...
import com.adpilot.cortb.clientapi.rest.items.Item._
import com.adpilot.cortb.clientapi.rest.items.ItemMeta.Property.BasicProperty
import com.adpilot.cortb.clientapi.rest.items.ItemMeta.{ItemProperty, PropertyType}
...
abstract class ItemRepresentationSupport[I, E :TypeTag] extends ItemRepresentation[I, E] {
	...
	protected implicit def self = this


	protected implicit def itemAsOne[I2, E2](item :Item[I2])(implicit repr :ItemRepresentation[I2, E2]) :One[E2] =
		repr.one(item)

	protected implicit def itemAsMaybe[I2, E2](item :Item[I2])(implicit repr :ItemRepresentation[I2, E2]) :Reference[Option[E2]] =
		repr.maybe(item)

	protected def itemsAsMany[I2, E2, C<:Iterable[E2]](items :Items[I2])(implicit repr :ItemRepresentation[I2, E2], cbf :CanBuildFrom[_, E2, C]) :Many[C] =
		repr.many(items)
...
//		repr.items(ref)
	protected implicit def itemOptionAsOneOption[I2, E2](opt :Option[Item[I2]])(implicit repr :ItemRepresentation[I2, E2]) :Option[One[E2]] =
		opt.map(itemAsOne(_))

	protected implicit def oneOptionAsItemOption[I2, E2](opt :Option[One[E2]])(implicit repr :ItemRepresentation[I2, E2]) :Option[Item[I2]] =
		opt.map(oneAsItem(_))



	override def one(ref: Item[I]): One[E] = ref match {
		case ValueItem(item) => One(entity(item))
...
//		case ValueItem(item) => IdRef.maybe[E](entity(item))
	...
	override def many[T<:Iterable[E]](ref: Items[I])(implicit cbf :CanBuildFrom[_, E, T]): Many[T] = ref match {
		...
		case ItemsAll => Many.All[T]()
		case ItemsAbsent => Many[T]()
	...
	override def items[T <: Iterable[E]](ref: Reference[T]): Items[I] = ref match {
		...
		case Unknown() => Items()
		case PropertyReference(property, value) =>
			mapping(property.asInstanceOf[ObjectProperty[E, _]]).items(value)
		case PropertyMultiReference(property, values) =>
			mapping(property.asInstanceOf[ObjectProperty[E, _]]).itemsMulti(values)
	...
	override def property(prop: Property[I, _]): ObjectProperty[E, _] = prop match {
...
//		}
	...
	override def foreignProperty(prop: ItemProperty[I, _]): (E) => One[Any] = prop match {
		case SingleRefMapping(p) => p.onEntity
		case _ => throw new IllegalArgumentException(s"passed property $prop was not created by this instance ($self)")
//		case _ => property(prop.name).map(_.onEntity.asInstanceOf[E=>One[_]]) getOrElse {
//			throw new IllegalArgumentException(s"Couldn't map property $prop to an entity property")
//		}
	}
	...
	override def mapProperty(property: ObjectProperty[E, _]): Property[I, _] = mapping(property)

	private def mapping(prop: ObjectProperty[E, _]): PropertyMapping[_, _] =
	...
	trait PropertyMapping[+X, Y] extends ItemMeta.Property[I, Y] {
		...
		protected[ItemRepresentationSupport] lazy val property = ObjectProperty(onEntity)
		...
		protected[ItemRepresentationSupport] def one(value :Any) :One[E] =
			One(property).Empty(entityValue(value.asInstanceOf[Y]))

		protected[ItemRepresentationSupport] def maybe(value :Any) :Reference[Option[E]] =
			PropertyReference(property).maybe(entityValue(value.asInstanceOf[Y]))

		protected[ItemRepresentationSupport] def many[T<:Iterable[E]](value :Any) :Reference[T] =
			PropertyReference(property).as[T](entityValue(value.asInstanceOf[Y]))

		protected[ItemRepresentationSupport] def manyMulti[T<:Iterable[E]](values :Seq[Any]) :Reference[T] =
			PropertyReference(property).as[T](values.map(v => entityValue(v.asInstanceOf[Y])))
		...
		override def toString = Try({s"[$self.$name<~>${property.name}]"}).toOption.getOrElse(s"[$self.$name]")
	}


	class SingleRefMapping[X, Y](val onEntity :E=>One[X], onItem :I=>Item[Y], name :String)(
			implicit override val target :ItemRepresentation[Y, X]
		) extends ItemProperty(onItem, name)(target) with PropertyMapping[One[X], Item[Y]]
	{
		protected[this] val itemValue = target.item(_:One[X])
		...
		override def andThen[Z](next: Property[Y, Z]): Property[I, Z] = {
			...
			def toEntity(key :Z) = target.one(Item(next, key)) match {
				case UniquePropertyReference(_, res) => res
			...
			def toItem(key :Any) = target.item(One(prop).Empty(key)) match {
		...
		) extends BasicProperty(onItem, name) with PropertyMapping[Reference[T], Items[Y]]
	{
		def this(onEntity :E=>Reference[T], onItem :I=>Items[Y])(
			...
			this(onEntity, onItem, ObjectProperty(onItem).name)
	...
	protected def prop[X](entityProperty :E=>X, itemProperty :I=>X)(implicit tpe :PropertyType[X], tag :TypeTag[I]) :PropertyMapping[X, X] =
		prop(entityProperty, itemProperty, ObjectProperty(itemProperty).name)
	...
	protected def map[X, Y:PropertyType](entityProperty :E=>X, itemProperty :I=>Y)(implicit toEntity :Y=>X, toItem :X=>Y, tag :TypeTag[I]) :PropertyMapping[X, Y] =
		map(entityProperty, itemProperty, ObjectProperty(itemProperty).name)




	protected def one[X, Y](entityProperty :E=>One[X], itemProperty :I=>Item[Y], name :String)(implicit repr :ItemRepresentation[Y, X]) :SingleRefMapping[X, Y] =
		new SingleRefMapping(entityProperty, itemProperty, name)

	protected def one[X, Y](entityProperty :E=>One[X], itemProperty :I=>Item[Y])(implicit repr :ItemRepresentation[Y, X], tag :TypeTag[I]) :SingleRefMapping[X, Y] =
		one[X, Y](entityProperty, itemProperty, ObjectProperty(itemProperty).name)
...
//		map(entityProperty, itemProperty, name)(ItemMeta.propertyType[Items[Y]], target.many[T], target.items(_))
...
//		map(entityProperty, itemProperty)(ItemMeta.propertyType[Items[Y]], target.many[T], target.items(_), tag)
	...
	protected lazy val propertyMappings = properties.groupBy(_.property).map {
		...
		}.toMap[ObjectProperty[E, _], PropertyMapping[_, _]]
