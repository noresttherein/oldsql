...
import com.adpilot.cortb.clientapi.prototype.repository.entities.Model.{HasId, Id, IdRef}
...
import com.adpilot.cortb.clientapi.prototype.repository.plain.mapping.ColumnType._
import com.adpilot.cortb.clientapi.prototype.repository.plain.mapping.EntityMapping.{ColumnKey, ForeignKeyType}
import com.adpilot.cortb.clientapi.prototype.repository.plain.mapping.{ColumnType, EntityMapping}
...
//		Boxed[Seq[S], Seq[T]](_.map(mapping.map))(_.map(mapping.unmap))
	...
	implicit val byteArrayOptionType = ColumnType[Option[Array[Byte]]](
		SetParameter[Option[Array[Byte]]]{ case (bytes, params) => params.setBytesOption(bytes) }, GetResult[Option[Array[Byte]]](res => res.nextBytesOption())
	)
...
//	implicit val urlType = Boxed((s:String) => Try{new URL(s)}.toOption.orNull)(u => if (u==null) null else u.toString)
	...
	implicit val appnexusCreativeCategories = safe((s:String) => AppnexusCreativeData.Category(s.toLong))(_.id.toString)
	implicit val creativeImage = safe[Creative.Image, Array[Byte]](Creative.Image)(_.data)


	implicit val countryType = safe(Country)(_.code)
	implicit val biddingStrategyType = safe(BiddingStrategy(_:String))(_.name)

	implicit val networkType = safe(Network)(_.name)
	implicit val hourType = safe(Hour.fromString(_:String))(Hour.format(_))
...
//			case NothingKey(key :Id) => key
	...
	def optionalSelect[T](implicit opt :ColumnType[Option[T]]) :ColumnType[One[T]] =
		ColumnType.mapped[One[T], Option[T]](_.map(One(_)) getOrElse One())(_.toOpt)(new NullValue[One[T]](ColumnKey()), ColumnType[Option[T]])
}
