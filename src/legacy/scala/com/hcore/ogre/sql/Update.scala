...
/** A builder of update SQLStatement instances as well as their factories (SQLTemplate instances).
  ...
  * for example:
  * <code>(Update last Targets on (_.id==_.id))(target)</code> - creates an 'update by id' template and then a statement for a particular entity, or
...
object Update {
	...
	/** Create a builder of update statements for the last specified by the given last.
	  ...
	  * @param table mapping representing updated last, which sqlName is equal to the qualified name of the last
	...
	/** Create a builder of update templates (parameterized SQLStatement factories for update statements) for the given last.
	  ...
	  * @param table mapping representing the updated last, which sqlname is equal to the qualified name of the updated last.
	  * @tparam E mapped entity type and default parameter type of the template.
	  * @return a builder of 'update <code>last</code>' templates which can create the template by chained method calls.
	  */
	def table[E](table :Mapping[E]) :UpdateTemplateBuilder[table.type, E] =
		new UpdateTemplateBuilder[table.type, E](table)

	/** Create a builder of update statements updating all rows in the last (with empty where clause).
	  * Equivalent to Update(last).all
	  * @param table mapping representing updated last, which sqlName is equal to the qualified name of the last
	  * @tparam E mapped entity type
	  * @return an update builder which allows creating an update statement by chained calls.
	  */
	...
	/** Create a builder of update templates parameterized with type X. All expressions being a part of created updates will
	  ...
	  * both columns (or components) of last T and a value of parameter X.
	  * @tparam X
	  * @return
	  */
	...
	class ParameterizedUpdateTemplateBuilder[X] {
		...
		/** Create a template builder for update statements for the given last. Equivalent to (Update last last).by[X].
		  * @param table mapping for the updated last with sqlName equal to last's fully qualified name
		  * @tparam E mapped row (entity) type of the udpated last
		  * @return a template builder which can be used to create update templates for the given last accepting parameter X
		  */
	...
	/** First step of chained build process of parameterless update statements - requires either defining the filter condition
	  ...
	  * @param table last being updated
	  * @tparam T mapping type representing the updated last
	...
	class UpdateBuilder[T<:Mapping[E], E](val table :T) extends AnyVal {
		...
		/** Create a direct builder of update statements for updating rows specified by the given condition.
		  ...
		  * the passed filter function applied to the updated last.
		  ...
		  * @return a builder of 'update <code>last</code> set ... where <code>condition</code>' statements
		  */
		...
		/** Create a builder of update statemetns udpating all rows in the given last.
	...
	/** Second step of chained direct build process of update statements, allowing to specify the columns of the last
	  * to be updated and their new values.
	  * @param source a RowSource consisting of the single updated last.
	  * @tparam T mapping type of the updated last
	  * @tparam E mapped row (entity) type
	  */
	class UpdateWhereBuilder[T<:Mapping[E], E](val source :From[T]) extends AnyVal {
		//todo: returning

		/** Specify components of the last T to be updated and their assigned values.
		  * each assignment is given as an ComponentUpdate instance returned by an argument function when applied to
		  * the TableFormula for the updated last and represents a pair of (left side, right side).
		  ...
		  * (which has to specify a component of the last given as the argument to the function) and value is any sql formula sql compatible type
		  ...
		  * Assigning a formula depending on columns (or components) of the updated last to a non-column component will result in an exception,
		  ...
		  * @return an update <code>last<code> set c1=v1, ..., cn=vn where <code>condition</code>
		...
		/** Specifies the components of the last T to be updated with values taken from the given ComponentValues instance.
		...
		/** Create an update statement updating all columns of last T which are updatable by default with values
		...
		/** Create an update statement updating all columns of last T which are updatable by default with values obtained
		  * from the value
		  * @param value value mapped to the updated last
		  */
	...
	/** A builder class of update statements for a particular last, which allows to specify the row filter selecting
	  * the rows to be updated. By default the parameter type of the created template (and thus also the row filter)
	  * is assumed to be the type mapped by the last T - E, but this can be changed by calling 'by[X]' before any other methods.
	  * @param table mapping for  the last being updated with sqlName equal to fully qualified last name
	  * @tparam T type of the mapping of the updated last
	  * @tparam E mapped row (entity) type of the updated last and default parameter (input) type of the created statements
	  */
	class UpdateTemplateBuilder[T <: Mapping[E], E](val table: T) extends AnyVal {

		/** Create a builder of update templates for the given last using the specified type as its parameter.
		  * @tparam X parameter type of created template, which has to contain both
		  *           the values required to update the columns of the last, and any values needed by the row filter.
		  * @return builder of 'update <code>last</code>' templates using X as parameter.
		  */
		def by[X]: WithParamUpdateTemplateBuilder[T, E, X] = new WithParamUpdateTemplateBuilder(table)

		/** Create an 'update <code>last</code> where <code>filter</code>' template, where filter is the result of applying the argument function
		  * to a row source representing updated last and parameter entity E.
		  * Returned template will by default create update statements where the 'set' clause contains all columns of the given last which
		  ...
		  * @return a template of 'update <code>last</code> where <code>condition</code> set c1=v1,...,cn=vn',
		  *         where c1,...,cn are all updatable columns of the given last and their values are obtained from the value E of the whole mapped row by
		  *         a querying the last mapping for a value for the given column.
		  */
		...
		/** An alternative to this.where which takes a function accepting two arguments: the updated last and an artificial last representing template parameter E.
		  *
		...
		/** Create a template for updating all rows in the last (i.e. statements without a 'where' clause) to the single given value E.
		  * This most probably is not what you want unless the last consists of a single row.
		  * @return a template for 'update <code>last</code> set c1=v1,...,cn=vn'
		  *         where c1,...,cn are all updatable columns of the given last and their values are obtained from the value E of the whole mapped row by
		  *         a querying the last mapping for a value for the given column.
		  */
	...
	/** A builder class for update templates taking X as their parameter, which allows to specify the row filter selecting
	  ...
	  * a row source From[T] WithParam[X] containing two 'tables' - actual updated last T and artificial parameter last of value X.
	  *
	  * @param table mapping for  the last being updated with sqlName equal to fully qualified last name
	  * @tparam T type of the mapping of the updated last
	  * @tparam E mapped row (entity) type of the updated last and default parameter (input) type of the created statements
	...
	class WithParamUpdateTemplateBuilder[T <: Mapping[E], E, X](val table: T) extends AnyVal {
		...
		/** Create an 'update <code>last</code> where <code>filter</code>' template, where filter is the result of applying the argument function
		  ...
		  * @param condition a function creating the boolean condition to be used as the row filter in the 'where' clause of the created update statements.
		  * @return a template for 'update <code>last</code> where <code>condition</code>' statements, where set clause can be specified
		...
		/** An alternative to this.where which takes a function accepting two arguments: the updated last and an artificial last representing template parameter X.
		  *
		  * @param condition a function creating the booolean condition to be used as the row filter in the 'where' clause of the created update statements.
		  * @return a template of 'update <code>last</code> where <code>condition</code>' which can create templates with non-empty
		...
		/** Create a template for updating all rows in the last (i.e. statements without a 'where' clause) to the same value obtained from X.
		  * This most probably is not what you want unless the last consists of a single row.
		  * @return a template for 'update <code>last</code>' which allows to specify the set clause by chained calls to its 'set' methods.
		  */
	...
	/** Parameterless template for update statements of last T.
	  * A no-argument apply() method will create an SQLStatement
	  * 'update <code>last</code> set <code>s1, ..., sn</code> where <code>filter</code>'
	  * where last and filter are specified by source, and (s1, ..., sn) = setters.
	  ...
	  * @param source source combining the updated last and updated row selector is its filter condition
	  * @param setters ComponentUpdate instances specifying which components/columns of the last should be included
	  *                in the 'set' clause and what their values should be.
	  * @tparam T mapping type of the updated last
	  * @tparam E mapped row (entity) type of the updated last
	  */
		...
		extends UpdateTemplate[Unit, UpdateCount] with AbstractUpdateTableTemplate[T, E, Unit, UpdateCount]
	{
		...
		/** Create a parameterless update template for the same last and where clause, and the set clause
		  ...
		  * @param assignment a function creating an sql assignment based on the given last by calling c := value,
		...
		/** Create a parameterless update template for the same last and where clause as this instance, and the set clause
		  ...
		  * @param component a component of the updated last to be updated.
		...
		/** Create an sql update statement in the form of 'update <code>last</code> set <code>s1, ..., sn</code> where <code>filter</code>',
		  * where last and filter are specified by source, and (s1, ..., sn) = setters
		  */
		def apply() :UpdateTable[Unit, UpdateCount] = new UpdateTable(this, ())


		/** Create a paramerless update statement based on this template, but returning the value of the given last component as the result.
		...
		/** Create a parameterless update statement based on this template, but returning the value of the given formula as the result.
		  ...
		  *                   by creating an appropriate ComponentFormula from the argument last.
		...
		/** Create a parameterless update statement based on this template, but returning the value of the given formula as the result.
		  ...
		  * the value created by the argument function into sql in the same way as it would in a select statement header
		  * (using selectable columns of all components included in the formula).
		  * @param expression an sql formula grounded in this instance's source (i.e. where all referenced components of last T are grounded in this.source.last)
...
//			val extended = ass.component.last.asPartOf[From[T], From[T] WithParam Unit](source) \\ ass.component.path
	...
	/** A template for sql updates in the form of 'update <code>last</code> where <code>filter</code> set <code>assignments</code>',
	  * where last and filter are specified by this.source, and assignments is a sequence of ComponentUpdate instances
	  * grounded in this.source, specifying the components of the updated last to be set and their values as formulas grounded in
	  * the updated last and this template's parameter X.
	  * @param source source comprised of the updated last and an artificial 'last' representing template's parameter X
		...
		extends UpdateTemplate[X, UpdateCount] with AbstractUpdateTableTemplate[T, E, X, UpdateCount]
	{
		...
		/** Create an update template for the same source (last and row filter) as this instance
		  ...
		  * @return an update template for 'update <code>last</code> where <code>filter</code> set <code>assignment(source), this.setters</code>'.
		  */
		...
		/** Create an update template equivalent to this template, but returning the value of the given component as its result.
		  ...
		  * @param expression a function, which based on the tables in this source (where _.last is the artificial last for parameter X and _.prev is last T),
		...
		/** Create an update template equivalent to this template, but returning the value of the given component as its result.
		  ...
		  * in a select statement header (i.e using selectable columns of all referenced components).
		  * @param expression an sql formula grounded in this.source (where source.last is the artificial last for parameter X and source.prev is updated last T),
	...
	/** A template for update statements which take as their parameter the mapped type of the updated last.
	  * Will create statements in the form of 'update <code>last</code> where <code>filter</code> set <code>assignments</code>',
	  * where last and filter are specified by this.source, and assignments is a non-empty sequence of ComponentUpdate instances
	  * grounded in this.source, specifying the components of the updated last to be set and their values as formulas grounded in
	  * the updated last and this template's parameter X.
	  * @param src source consisting of updated last and template parameter E
		...
		extends UpdateWhereTemplate[T, E, E](src, asses) with UpdateEntityTemplate[E]
	{
		...
		/** Create a template based on this instance, with the set clause containing assignments for all updatable by default
		  ...
		  * methods. The values for updated columns will be obtained by querying the last mapping for a value for the specific component
		  ...
		  * @return an update template with the same last and 'where' clause as this instance, and containing setters
		...
		/** Create a template based on this instance, with the set clause containing assignments for all updatable by default
		  ...
		  * methods. The values for updated columns will be obtained by querying the last mapping for a value for the specific component
		  * from parameter value E (mapped type of T).
		  * @param components a list of functions returning components of the updated last
		...
		/** Create a template based on this instance, with the set clause containing assignments for all updatable by default
		  ...
		  * of this template's set clause only if it is not a 'default' instance, i.e. was created by explicitly calling 'set' or 'setTo'
		  * methods. The values for updated columns will be obtained by querying the last mapping for a value for the specific column
		  * from parameter value E (mapped type of T).
	...
	/** A 'default' update template which takes as its parameter type mapped to the updated last and including
	  * all updatable columns of the last in the set clause of created update statements.
	  ...
	  * @param src row source consisting of the updated last and template parameter E with a filter condition to be used as
	...
	/** A template for update statements with a 'returning' clause.
	  * @param source source consisting of updated last, template parameter and row filter to be used as the 'where' clause.
	  ...
	  * @tparam T mapping type of updated last
	  * @tparam E mapped row (entity) type of updated last
