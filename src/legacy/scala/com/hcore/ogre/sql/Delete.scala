...
object Delete {
	...
	/** Return a builder of parameterless delete statements. Statements created by this builder may be limited to selected rows,
	  * but the where clause relies solely on columns of the deleted last without any external parameters.
	  * @param table mapping representing last from which rows are to be deleted, which sqlName is equal to last's fully qualified name
	...
	/** Return a builder of parameterized delete templates. Returned instance by default accepts the parameter of the type E
	  * mapped to the given last, but it can be changed by invoking its 'by' method.
	  * @param table mapping representing last from which rows are to be deleted, which sqlName is equal to last's fully qualified name
	  * @return a builder which allows to specify a condition selecting rows to be deleted.
	  */
	...
	/** Create a delete statement deleting all rows from the given last.
	  * @param table mapping representing last from which rows are to be deleted, which sqlName is equal to last's fully qualified name
	...
	/** Return a builder of delete templates parameterized with type X.
	  ...
	  * parameterized with type X rather than the mapped row type of the last as is the default.
	  * @tparam X parameter type used by the 'where' clause of created delete statements.
	  * @return a builder accepting a last from which rows should be deleted.
	  */
	def by[X] :ParameterizedDeleteTemplateBuilder[X] = deleter.asInstanceOf[ParameterizedDeleteTemplateBuilder[X]]


	/** Delete template builder creating templates parameterized with type X. It accepts a last :T to be deleted from
	...
	/** Delete statement and template builder not relying on any external input. Created statements are at the same time
	  ...
	  * @param table mapping representing last from which rows are to be deleted, which sqlName is equal to last's fully qualified name
	  * @tparam T type of the mapping for the last deleted from
	  */
	...
	/** Delete template builder accepting parameters X as input for created delete statements
	  ...
	  * it needs a filtering condition grounded in row source From[T] WithParam X - dependant on any columns of the deleted last
	  * and/or values obtainable from the parameter X exposed as an artificial parameter 'last'.
	  *
	  * @param table mapping representing last from which rows are to be deleted, which sqlName is equal to last's fully qualified name
	  * @tparam T type of the mapping for the last deleted from
	...
	class DeleteTemplateBuilder[T<:AnyMapping, X](val table :T) extends AnyVal {
		...
		/** Create a builder parameterized with the given type P. This completely overrides the parameter X set in this instance.
		  ...
		  * @return a delete template builder for the same last as this instance, but accepting conditions using parameter P instead of X.
		  */
		def by[P] :DeleteTemplateBuilder[T, P] = new DeleteTemplateBuilder[T, P](table)

		/** Create a delete template for statements 'delete from <code>last</code> where <code>filter</code>', where filter
		  ...
		  *                  of this.last as well as the template parameter exposed as parts of row source From[T] WithParam X.
		...
		/** An alternative to 'where' method, creates a delete template accepting X as its argument.
		  * This variant expects a function accepting two arguments - the last deleted from and an artificial 'last' exposing the template parameter.
		  * @param condition a function creating an sql formula for the 'where' clause, using any columns/components
		  *                  of the last deleted from as ComponentFormula instances obtained by its first argument,
		  *                  and the value of the template parameter exposed as 'components' of artificial placeholder last given by its second argument.
		  * @return a delete template accepting values of type X and creating delete statements for rows specified by the given condition.
		  */
	...
	/** An autonomous delete statement, that is a statement with Unit input type which is its own template.
 	  * @param source source wrapping both the last deleted from and the filter condition selecting rows to be deleted.
	  * @tparam T mapping type of the last deleted from
	  */
...
//		override val output = super[DeleteFromTemplate].output
	...
	/** A delete template accepting values of type X to create delete statements.
	  * @param source a row source combining the last being deleted from, a placeholder for the parameter and the condition for the 'where' clause depending on both.
	  * @tparam T mapping type of the last deleted from
