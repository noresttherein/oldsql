...
//		override def unapply(root :S) :Option[T] = pick(root)
	...
	trait ConcatPath[X<:AnyMapping, Y<:AnyMapping, Z<:AnyMapping] extends MappingPath[X, Z] {
		...
		private[mapping] def ascending :ConcatPath[X, _<:AnyMapping, Z]
		private[mapping] def descending :ConcatPath[X, _<:AnyMapping, Z]
	...
	private[mapping] class TypedConcatPath[X <: AnyMapping, Y<:AnyMapping{ type ResultType=W }, W, Z<:AnyMapping{ type ResultType=V }, V] private[mapping] (
...
//		override def surepick = for (f <- prefix.surepick; g <- suffix.surepick) yield (f andThen g)
		...
		private[mapping] lazy val ascending :ConcatPath[X, _<:AnyMapping, Z] =
			if (isAscending) this
			else
		...
		private[mapping] lazy val descending :ConcatPath[X, _<:AnyMapping, Z] =
