...
import com.hcore.clientapi.entities.Model
import com.hcore.clientapi.entities.Model.One
import com.hcore.ogre.mapping.ColumnType.MappedType
import com.hcore.ogre.mapping.EntityMapping.{JoinByFK, JoinKey, JoinResult}
...
import scala.slick.jdbc.{GetResult, PositionedResult, SetParameter, StaticQuery => Q}
...
trait EntityMapping[E, PK] extends TableMapping[E] with MappingWithPK[E, PK] { mapping =>
	...
	object ForeignKeyComponent {
		...
		private def get[T](ref :One[T]) :Option[mapping.Component[One[T]]] = ref match {
			case JoinKey(JoinByFK(_, Some((fk :Mapping[_], prev)))) if prev.table == mapping =>
	...
	private def joinMock(implicit path :JoinByFK[E, PK]) :E =
		joinMock() getOrElse apply(ComponentValues(this :this.type)(_.scoutValue)) //todo

	protected def joinMock() :Option[E] = None


	private def mockValue[T](col :Component[T])(implicit path :JoinByFK[E, PK]) = ComponentValues(col :col.type)(col.nullValue match {
		case Some(fk:JoinKey[_, _]) =>
	...
	protected[mapping] def joinSelectStatement(whereCondition :Option[String], refs :(E=>One[Any])*) :(String, GetResult[E]) = {
		...
		val joins :Seq[FromItem] = { //Seq[(join, table occurence, row already joined earlier in the list)]
			val expanded =
				root +: refs.map(_(mock)).flatMap{ case JoinKey(join) if join.joinedWith(this) => join.expand() }

			def indexed(tables :Seq[FromItem] = expanded.map((_, 0, false))) :Seq[FromItem] = tables match {
				case Seq() => Seq()
				case Seq(head @ (join, count, false), tail @ _*) =>
					head +: indexed(tail.map {
		...
		val select = {
			val columns =
				for ((join, alias) <- ordered; column <-join.table.selectable)
					yield s"$alias.${column.sqlName.get}"
		...
		val where = {
			def conditions(tables :Seq[FromItem]=joins) :Seq[String] = tables match {
				...
				case Seq(_, (JoinByFK(_, None), _, false), t @_*) => //second table is the root and starts a new join chain
					conditions(tables.tail)
				case Seq((prev, idx1, _), (next, idx2, false), t @_*) if next.prev.isDefined =>
					...
					val keysEqual = (next.prev.get._1.querable zip next.table.PK.querable).map {
						case (lcol, rcol) => s"$ltable.${lcol.sqlName.get} = $rtable.${rcol.sqlName.get}"
					}.mkString(" and ")
...
//		)
		extends MappedType[One[E], FK](_ => ???, _ => ???)
	{
		...
		private val referencing = Single(PropertyEquals(table.pk _))
...
//		override val unmap = (ref :One[E]) => toFK(referencing.forceKeyOutOf(ref))
	...
	object ForeignKeyType {
		...
		def emptyKey[PK :ColumnType, E :TypeTag](pk :E=>PK) = {
			val factory = Single(PropertyEquals(pk))
...
*/
	...
	private case class JoinByFK[E, PK] private (table :EntityMapping[E, PK], prev :Option[(Mapping[One[E]], JoinByFK[_, _])])
	{
		...
		def extend[X, P](key :Mapping[One[X]], parent :EntityMapping[X, P]) :JoinByFK[X, P] =
	...
	private object JoinByFK {
		def apply[C, P, PK](prev :JoinByFK[C, _], key :Mapping[One[P]], table :EntityMapping[P, PK]) :JoinByFK[P, PK] =
