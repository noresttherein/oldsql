...
object ItemMeta {
	...
	trait Property[E, T] {
		...
		def resultTypeName = {
			val fullName = tpe.toString
			fullName.substring(fullName.lastIndexOf('.')+1)
		}

		override def toString = s"$name :$resultTypeName"
	...
	object Property {
		...
		protected[ItemMeta] def apply[E :TypeTag, T :PropertyType](property :E=>T) :Property[E, T] =
			new BasicProperty(property, PropertyChain(property).name)//, propertyType[T], ItemMeta[E])
...
//			ItemsProperty(compose(prop.apply), append(prop.name))
	...
	object ItemProperty {
		...
		protected[ItemMeta] def apply[E :TypeTag, T :ItemMeta](property :E=>Item[T]) :ItemProperty[E, T] =
			new ItemProperty(property, PropertyChain(property).name)
...
//			private val cast = as[T](identity)
		...
		implicit val FloatProperty = BigDecimalType.map(_.toFloat)(f => BigDecimal(f.toDouble))
