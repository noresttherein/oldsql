...
object LogicalFormula {
	...
	//todo: rename to Not
	case class NotFormula[-F <: FromClause](formula :SQLFormula[F, Boolean]) extends LogicalFormula[F] {
		...
		override def map[S <: FromClause](mapper: SQLRewriter[F, S]) = NotFormula(mapper(formula))
	}
	...
	case class And[-F <: FromClause] private(protected val parts :List[SQLFormula[F, Boolean]]) extends LogicalFormula[F] {
		...
		override def map[S <: FromClause](mapper: SQLRewriter[F, S]) = new And(parts.map(mapper(_)))
	...
	object And {
		def apply[F <: FromClause](parts :SQLFormula[F, Boolean]*) :And[F] =
			new And(parts.toList.reverse)

		def unapply[F <: FromClause](sql :SQLFormula[F, Boolean]) :Option[Seq[SQLFormula[F, Boolean]]] = sql match {
	...
	case class Or[-F <: FromClause] private(protected val parts :List[SQLFormula[F, Boolean]]) extends LogicalFormula[F] {
		def conditions :Seq[SQLFormula[F, Boolean]] = parts.reverse
		...
		override def map[S <: FromClause](mapper: SQLRewriter[F, S]) = new Or(parts.map(mapper(_)))
	...
	object Or {
		...
		def apply[F <: FromClause](conditions :SQLFormula[F, Boolean]*) :Or[F] = new Or(conditions.toList.reverse)
