...
/** A Concept of assigning a value to a component of a table, either directly as a set clause of an update statement,
  ...
  * @tparam F row source providing values used by the formula on the right side - might contain a source parameter
  *           and/or table `T`.
  * @tparam T the mapping of the table being updated/inserted to.
  * @tparam E the subject type of table `T`.
  * @tparam C the mapping of a subcomponent of the table `T` which is being set.
  * @tparam O a tag type distinguishing individual tables in `F`.
  * @tparam L the value type of the component which is being assigned to.
  * @tparam R the value type of the assigned expression on the right hand side.
  * @tparam X the type to which both left and right side are auto converted on the scala side.
...
*/
	...
	/** Create a list of assignments for the set clause of an update statement from a given SetComponent instance.
	  ...
	  */
	def UpdateForms[T<:Mapping, C<:Mapping, RV, V, P](
		                                                                                         ...
		                                                                                         table :TableFormula[L, T], setter :SetComponent[R, T, C, V, E], param :RowSourceParam[R, P], mode :ColumnFilter) :Seq[SetForm[L, R, T, _<:Mapping, P]] =
	{
		...
		def columnSetter[X](column :C#Component[X]) :SetForm[L, R, T, _<:Mapping, P] = {
			val left = table \\ (setter.path :\ column)
			val columnValue = ComponentPath.direct(comp, column).pick
			def pick(p :P) :Option[column.Subject] = {
	...
	case class SetColumnForm[-L<:FromClause, -R<:FromClause, T<:Mapping, C<:Mapping, P]
		...
		extends SetForm[L, R, T, C, P]
	{
		val value = placeholders(pick)((component.selectForm && component.writeForm(mode).asInstanceOf[SQLWriteForm[C#Subject]]).asOpt)
		...
		val form = {
			...
			columnForm.iflatMap{ p :P => pick(p).asInstanceOf[Option[column.Subject]] }
		}
	}
