...
import net.noresttherein.oldsql.sql.ast.{AdaptedColumnSQL, AdaptedSQL, AliasedSQL, BoundParam, ChainSQL, ComponentSQL, ConvertedSQL, DecoratedSQL, EditedComponentSQL, InlineSQL, IndexedSQL, MappingTerm, SelectIdSQL, SQLTerm, UnboundParamSQL}
...
import net.noresttherein.oldsql.sql.ast.IndexedSQL.{LabeledColumnSQL, IndexItem, LabeledValueSQL, MatchAnyIndexed}
...
/** A synthetic adapter of an [[net.noresttherein.oldsql.sql.SQLExpression SQLExpression]]`[Domain, S, O]`
  ...
  *   - [[net.noresttherein.oldsql.sql.ast.IndexedSQL IndexedSQL]] expressions use instead
...
//todo: SQLMapping should be use site covariant in its subject; this will help in select clause/grouping
...
/** An [[net.noresttherein.oldsql.sql.SQLMapping SQLMapping]] adapting
  * a [[net.noresttherein.oldsql.sql.ast.IndexedSQL IndexedSQL]] expression for use
  ...
  * are columns and other `IndexedSQL` expressions, this means that every column of this mapping
...
object ListingSQLMapping {
	...
	//we don't need an OperationView because IndexedSQL cannot contain non-column components
	def apply[F <: RowProduct, S >: Grouped <: Single, T <: Listing, O](expr :IndexedSQL[F, S, T])
	...
	{ outer =>
			...
			with CaseAnyColumn[F, Extractors] with CaseAnyColumnMappingTerm[Extractors]
		{
			...
			{   //any changes must be reflected also in the AssemblerComposer to retain consistency
				...
				val component :TypedMapping[V, G] = e.anchored
...
object TypedListingSQLMapping {
	...
	def apply[F <: RowProduct, S >: Grouped <: Single, T <: Listing, O]
	         (expr :IndexedSQL[F, S, T], columnPrefix :String) :TypedListingSQLMapping[F, S, T, O] =
	...
	private class ListingTupleMapping[F <: RowProduct, S >: Grouped <: Single, V <: Listing, O]
	                                 (columnPrefix :String, override val expr :IndexedSQL[F, S, V])
		extends TypedListingSQLMapping[F, S, V, O]
		   with ExportMapping with ExportMappingTemplate[c[F]#c[S]#Mapping, c[F]#c[S]#Column]
	{ outer =>
		def this(expr :IndexedSQL[F, S, V]) = this("", expr)

		override type Domain = F
		override type Scope  = S
		type Item[T] = IndexItem[F, Grouped, _ <: Label, T]
		...
		private[this] val subexpressions = {
			@inline def itemToComponent[K <: Label, T](e :IndexItem[F, S, K, T]) = {
				val prefix = if (columnPrefix.length == 0) e.key else columnPrefix + "_" + e.key
				Assoc[Item, ExpressionComponent, T](e, e.value.listingMapping[O](prefix))
			}
			expr.items.view.map(itemToComponent(_ :IndexItem[F, S, _ <: Label, _])) to NaturalMap
		}
		...
		private class AssemblerComposer
			extends BaseAnyExpressionVisitor[F, Assembler] with MatchAnyIndexed[F, Assembler]
		{
			...
			override def emptyIndexed = { val res = Got(@~); _ => res }
		}
		...
		private class ComponentsCollector
			extends BaseAnyExpressionVisitor[F, Extracts] with MatchAnyIndexed[F, Extracts]
		{
			override def indexedItem[C >: Grouped <: Single, I <: Listing, K <: Label, L]
