...
trait ColumnSQL[-F <: RowProduct, -S >: Grouped <: Single, V]
	extends SelectableSQL[F, S, V]
	   with ConvertingColumnTemplate[F, S, V, SQLExpression.from[F]#rows[S]#C]
	   with VariantGroundingTemplate[F, S, V, ({ type E[-f <: RowProduct] = ColumnSQL[f, S, V] })#E]
{
...
//todo: priority (binding strength to avoid unnecessary usage of '(', ')'
	...
	/** Attaches a label to this column in the form of a `String` literal type `N` for the purpose of indexing it
	  * inside an indexed SQL [[net.noresttherein.oldsql.sql.ast.LabeledSQL tuple]], allowing to access
...
object ColumnSQL {
	...
	trait ConvertingColumnTemplate[-F <: RowProduct, -S >: Grouped <: Single, V, +EC[v] <: ConvertibleColumn[F, S, v, EC]]
		extends ConvertingTemplate[F, S, V, EC]
	{ this :EC[V] with ConvertingColumnTemplate[F, S, V, EC] =>
			...
			if (this eq other)
				...
				other match {
					case column :ColumnSQL[F2, S2, V2] if reform.compatible(this, column) =>
						(leftResult(this), rightResult(other))
					case _ =>
						super.reform[F1, S1, F2, S2, V2, EC2, U](this)(reform, passCount)
				}
...
//			visit[F_, S_, C, T, Y](visitor)
	...
	type ConvertibleColumn[-F <: RowProduct, -S >: Grouped <: Single, V,
	                       +E[v] <: ColumnSQL[F, S, v] with ConvertingColumnTemplate[F, S, v, E]] =
		ColumnSQL[F, S, V] with ConvertingColumnTemplate[F, S, V, E]
