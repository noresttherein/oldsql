...
import net.noresttherein.oldsql.exceptions.{InseparableExpressionException, InvalidSQLException, MisalignedExpressionException, NoSuchComponentException, OldSQLException, UndefinedShapeException, rethrow}
...
import net.noresttherein.oldsql.schema.{ColumnForm, RelVar, Relation, SQLForm, SQLReadForm, SQLWriteForm, Table}
...
import net.noresttherein.oldsql.slang.{OptionGuardExtension, innerClassNameOf, mappingMethods}
...
import net.noresttherein.oldsql.sql.Query.{AbstractQuery, QueryReformingTemplate, QueryTemplate}
...
import net.noresttherein.oldsql.sql.SQLDialect.{DefaultSpelling, SQLSpelling, SpellingScope}
...
import net.noresttherein.oldsql.sql.SQLExpression.{CaseAnyExpression, ConvertibleSQL, GroundingTemplate, Grouped, ReorderingTemplate, Single}
...
import net.noresttherein.oldsql.sql.mechanics.{Alignment, AlignableColumns, QueryReform, Reform, SQLOrdering, SpelledSQL}
...
object SQLDialect {
	...
	trait SpellingScope extends Serializable {
		...
		/** An allowed column column is one which is, or can be, included in this scope as a part of the SQL
		  * for an owning component.
		  */
		def isAllowed(column :TypedColumn[_, _]) :Boolean

		/** A prohibited column is one which can never be included in this scope as a part of the SQL
		  * for an owning component.
		  */
		def isProhibited(column :TypedColumn[_, _]) :Boolean = !isAllowed(column)
	...
	object SpellingScope {
			...
			extends SpellingScope
		{
			...
			override def mandatoryColumns[Comp[T, Q] <: TypedMapping[T, Q], Col[T, Q] <: TypedColumn[T, Q] with Comp[T, Q], O]
					...
					:Unique[Col[_, O]] =
				if (root == component)
					view.mandatoryColumns(root)
				else
					view.mandatoryColumns(root, component)

			override def isAllowed(column :TypedColumn[_, _])    :Boolean = view.Prohibited.inactive(column)
		...
		private object TopLevel extends SpellingScope {
			...
			override def isAllowed(column :TypedColumn[_, _]) = false
...
//		val OperandScope :SpellingScope = DefaultScope
	...
	/** A strategy class used to render all classes representing some SQL/DML/DDL syntax features or database objects
	  ...
	  * such as [[net.noresttherein.oldsql.sql.SQLExpression SQLExpression]]`.`[[net.noresttherein.oldsql.sql.SQLExpression.realign reform]]
	...
	trait SQLSpelling extends Decorable[SQLSpelling] { //spell methods are kind of redundant given TopScope, but at least take no extra params
		...
		def spell[P, V](query :Query[P, V]) :SpelledSQL[P] =
			top(query.reformed(this))(newContext)
		...
		def spell[V](query :QuerySQL[RowProduct, V]) :SpelledSQL[@~] =
			top(query.reformed(this))(Dual, newContext, Parameterization.paramless)
...
//			withReform(queryReform.subreform(query)(this))
		...
		//These methods are not needed that much for the reforming itself, but for the spelling, as defaultSpelling
		// does not accept
...
//		}
		...
		//This is a bit awkward, because we always call spelling.queryReform(query)(query).
		// An alternative would be to have a DefaultQueryReform which on every call delegates to query.defaultReform
		/** The strategy used to unify select clauses of member selects in a compound select. */
		def queryReform :QueryReform = QueryReform.default
		...
		/** Specifies the restrictions to reforming that must be observed when reforming the argument query.
		  ...
		  * @return `query.`[[net.noresttherein.oldsql.sql.Query.QueryReformingTemplate.allowedReform allowedReform]]
		  */
		def allowedReforms[R](query :AbstractQuery[R]) :Permissions = query.`->allowedReforms`(self)

		/** Reorders columns of the given expression by creating a proxy overriding methods like
		  * [[net.noresttherein.oldsql.sql.SQLExpression.selectForm selectForm]] and
		  * [[net.noresttherein.oldsql.sql.SQLExpression.split split]].
		  * @return `expression.`[[net.noresttherein.oldsql.sql.SQLExpression.ConvertingOps.reform(reordering* reform]]`(reordering)(this)`.
		  */ //Not `reform` as it might conflict with Reform getter.
//		def reorder[F <: RowProduct, S >: Grouped <: Single, V, E[v] <: ConvertibleSQL[F, S, v, E]]
//		           (expression :ConvertibleSQL[F, S, V, E], reordering :Rearrangement) :E[V] =
		def realign[F <: RowProduct, S >: Grouped <: Single, V, Same <: SQLExpression[F, S, V]]
		           (expression :ReorderingTemplate[F, S, V, Same], reordering :Rearrangement) :Same =
			expression.`->realign`(reordering)(this)
		...
		/** A collection of all ''select'' clauses of member ''selects'' in `query`, in the order of their appearance,
		  ...
		  * @return `query.`[[net.noresttherein.oldsql.sql.Query.QueryTemplate.alignment alignment]].
		  */
		def alignment[R, Q[X] <: QueryTemplate[X, Q]](query :QueryTemplate[R, Q], permissions :Permissions) :Alignment =
			query.`->alignment`(permissions)(self)
		...
		/** A correct form to use for this spelling's [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.scope scope]].
		  * If the scope equals [[net.noresttherein.oldsql.sql.SQLDialect.SpellingScope.SelectScope SelectScope]],
		  * its column set is defined by the mapping's [[net.noresttherein.oldsql.schema.Mapping.selectForm selectForm]].
		  * For write scopes, the column set is defined by [[net.noresttherein.oldsql.schema.Mapping.writeForm writeForm]]
		  * for the appropriate operation type. The complementing reader/writer form is created from column forms
		  * for the included columns, which might make it raise errors on use.
		  * @see [[net.noresttherein.oldsql.sql.SQLDialect.SpellingScope.termForm]]
		  */
		def mappingForm[F <: RowProduct, S >: Grouped <: Single, M[A] <: MappingAt[A], V]
		                 (expression :MappingSQL[F, S, M, V]) :SQLForm[M[Unit]#Subject] =
			expression.`->mappingForm`(this)

		/** A correct form to use for this spelling's [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.scope scope]].
		  * Depending on the scope, the reader or writer form may non functional
		  * (not [[net.noresttherein.oldsql.schema.forms.UnspecifiedForm.isUniversal universal]]) - set / return `null`
		  * values or throw an exception.
		  */
		def effectiveForm[F <: RowProduct, S >: Grouped <: Single, V](expression :SQLExpression[F, S, V]) :SQLForm[V] =
			expression.`->effectiveForm`(this)
		...
		/** A correct form to use for this spelling's [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.scope scope]].
		  ...
		  * values or throw an exception.
		  */
		def effectiveForm[F <: RowProduct, S >: Grouped <: Single, V](expression :ColumnSQL[F, S, V]) :ColumnForm[V] =
			expression.`->effectiveForm`(this)

		/** Lists all potential columns of the argument expression (including those which are excluded by default)
		  ...
		  * process can apply to them. The relative order of
		  * [[net.noresttherein.oldsql.sql.mechanics.AlignableColumn.isDefault default]] columns within
		  * the returned list is consistent with [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.split split]],
		  * [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.shape shape]] and
		  * [[net.noresttherein.oldsql.sql.SQLExpression.selectForm selectForm]]
		  */ //todo: add potentialColumnsCount method
		def potentialColumns[F <: RowProduct, S >: Grouped <: Single]
		                    (expression :SQLExpression[F, S, _], permissions :Permissions) :AlignableColumns =
			expression.`->potentialColumns`(permissions)(self)
		...
		/** Lists all potential columns of the argument expression (including those which are excluded by default)
		  ...
		  * [[net.noresttherein.oldsql.sql.mechanics.AlignableColumn.isDefault default]] columns within
		  * the returned list is consistent with [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.split split]],
		  * and [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.shape shape]] methods.
		  */ //todo: add potentialColumnsCount method
		def potentialColumns[P, V](query :Query[P, V], permissions :Permissions) :AlignableColumns =
			query.`->potentialColumns`(permissions)(self)

		/** The number of columns listed in
		  * [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.potentialColumns potentialColumns]].
		  */
		def potentialColumnsCount[F <: RowProduct, S >: Grouped <: Single](expression :SQLExpression[F, S, _]) :Int =
			expression.`->potentialColumnsCount`(self)

		/** The number of columns listed in
		  * [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.potentialColumns potentialColumns]].
		  */
		def potentialColumnsCount[P, V](query :Query[P, V]) :Int =
			query.`->potentialColumnsCount`(self)
...
//				case single if single.sizeIs == 1 => single.head
...
*/
	...
	object SQLSpelling {
			...
			extends AbstractDecorable[SQLSpelling](redactor) with SQLSpelling
		{
			...
			override def redecorate(decorators :SQLSpelling => SQLSpelling) :SQLSpelling =
...
//				author.queryReform(query)
			...
			override def allowedReforms[R](query :AbstractQuery[R]) :Permissions = author.allowedReforms(query)

			override def realign[F <: RowProduct, S >: Grouped <: Single, V, Same <: SQLExpression[F, S, V]]
		                        (expression :ReorderingTemplate[F, S, V, Same], reordering :Rearrangement) :Same =
				author.realign(expression, reordering)

			override def alignment[R, Q[X] <: QueryTemplate[X, Q]]
			                      (query :QueryTemplate[R, Q], permissions :Permissions) :Alignment =
				author.alignment(query, permissions)

			override def mappingForm[F <: RowProduct, S >: Grouped <: Single, M[A] <: MappingAt[A], V]
			                        (expression :MappingSQL[F, S, M, V]) :SQLForm[M[Unit]#Subject] =
				author.mappingForm(expression)

			override def effectiveForm[F <: RowProduct, S >: Grouped <: Single, V]
			                          (expression :SQLExpression[F, S, V]) :SQLForm[V] =
				author.effectiveForm(expression)

			override def effectiveForm[F <: RowProduct, S >: Grouped <: Single, V]
			                          (expression :ColumnSQL[F, S, V]) :ColumnForm[V] =
				author.effectiveForm(expression)

			override def potentialColumns[F <: RowProduct, S >: Grouped <: Single]
			                             (expression :SQLExpression[F, S, _], permissions :Permissions) :AlignableColumns =
				author.potentialColumns(expression, permissions)

			override def potentialColumns[P, V](query :Query[P, V], permissions :Permissions) :AlignableColumns =
				author.potentialColumns(query, permissions)
