...
/** In its basic use, a `FromClause` is a representation of the ''FROM'' and ''WHERE'' clauses in an SQL ''SELECT''
  ...
  * [[net.noresttherein.oldsql.sql.GroupedFrom GroupedFrom]], the latter tree containing in particular
...
//	}
	...
	/** A marker trait for all non empty clauses, i.e. those which contain at least one relation.
	  ...
	  * All [[net.noresttherein.oldsql.sql.GroupedFrom grouped]] clauses are automatically subtypes
	  * of this type, too.
	  */
