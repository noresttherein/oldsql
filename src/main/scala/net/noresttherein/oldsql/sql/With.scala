...
import scala.collection.{mutable, Factory, IterableOps}
...
import net.noresttherein.oldsql.slang.repeat
import net.noresttherein.oldsql.sql.CommonTableExpression.{AnonymousName, CommonTableAlias}
...
//}
//type WithSome >: Dual <: RowProduct { val withClause <: WithSome }
//trait WithFrom[T[O] <: MappingAt[O]] extends SelectFrom[Dual, T]
//type RowProductPreamble >: EmptyRowProduct <: RowProduct
//type With[M[O] <: MappingAt[O]] >: Dual <: RowProductPreamble
//
//type AndWith[+L <: RowProductPreamble, R[O] <: MappingAt[O]] >: Dual <: L { val withClause :L; type Generalized >: withClause AndWith R}
//
//type SelectWith[+L <: NonEmptyRowProduct, R[O] <: MappingAt[O]] <: L {}
...
object With {
	//todo: ParamTables
	//todo: recursive
//	def apply[A <: Label :ValueOf] = new CTEName[A](valueOf[A])
...
//	def recursive
//	def apply[H](alias :String, query :QuerySQL[RowProduct, H]) :With

///*
//	def apply[CTEs :CTETuple](ctes :CTEs) :WithCTEs[CTEs] = new WithCTEs(new With[CTEs](ctes))
//
//	class WithCTEs[CTEs](private val clause :With[CTEs]) {
//		def apply[Q](f :With[CTEs] => Q) :Q = f(clause)
//		def apply[Q](f :(With[CTEs], With[CTEs]) => Q) = f(clause, clause)
//		def apply[Q](f :(With[CTEs], With[CTEs], With[CTEs]) => Q) = f(clause, clause, clause).define(clause.withClause)
//	}
//	class With[CTEs](ctes :CTEs) {
//		def apply[A <: Label](name :A)(implicit get :GetCTE[A, CTEs]) :StaticTable[A, get.M] = get(ctes)
//	}
//
//	trait GetCTE[A <: Label, CTEs] {
//		type M[O] <: MappingAt[O]
//		def apply(ctes :CTEs) :StaticTable[A, M] = ???
//	}
//*/



	class WithQuerySQL[H] private[With] (private[With] val query :QuerySQL[RowProduct, H]) extends AnyVal {
//		def from[Q](f :Table[MappingOf[H]#TypedProjection] => Q) :Q
		def apply[Q](f :Table[MappingOf[H]#TypedProjection] => Q) :Q = //todo: remove the cast when Query will preserve the subject type
			f(CommonTableExpression[query.RowMapping, H](
				query :QuerySQL[RowProduct, H] { type RowMapping[O] = query.RowMapping[O] }
			).asInstanceOf[Table[MappingOf[H]#TypedProjection]])

		def as[A <: Label](alias :A) :WithGenerator[StaticTable[A, MappingOf[H]#TypedProjection]] =
			//because the extension method for query doesn't currently preserve the result type H
			new WithGenerator(CommonTableAlias[A, query.RowMapping, H](
				alias, query :QuerySQL[RowProduct, H] { type RowMapping[O] = query.RowMapping[O] }
			).asInstanceOf[StaticTable[A, MappingOf[H]#TypedProjection]])
	}

	class WithMappingQuerySQL[M[O] <: MappingAt[O]] private[With] (private val query :MappingQuerySQL[RowProduct, M])
		extends AnyVal
	{
		def apply[Q](f :Table[M] => Q) :Q = f(CommonTableExpression[M, M[Unit]#Subject](query))

		def as[A <: Label](alias :A) :WithGenerator[StaticTable[A, M]] =
			new WithGenerator(CommonTableAlias[A, query.RowMapping, M[Unit]#Subject](alias, query))
	}
	...
	class CTEName[N <: Label](private val name :N) extends AnyVal {
		def as[M[O] <: MappingAt[O], V](select: => QuerySQL[RowProduct, V] { type RowMapping[O] = M[O] })
				:CommonTableAlias[N, M] =
			CommonTableExpression(select) as name
//		def as[M[O] <: MappingAt[O]](select: => MappingQuerySQL[RowProduct, M]) :CommonTableAlias[N, M] =
//			CommonTableExpression.as(name, select)
//
//		def as[V](select: => QuerySQL[RowProduct, V]) :CommonTableAlias[N, MappingOf[V]#TypedProjection] =
//			CommonTableExpression.as(name, select)
	}


	object implicits {
		implicit class CTENameExtension[N <: Label](private val name :N) extends AnyVal {
			@inline def as[M[O] <: MappingAt[O], V](query: => QuerySQL[RowProduct, V] { type RowMapping[O] = M[O] })
					:CommonTableAlias[N, M] =
				CommonTableAlias[N, M, V](name, query)
		}
	}
}
	...
	extends TableExpression[M] with NamedRelation[M] with RelationTemplate[M, CommonTableExpression[M]]
{
	...
	def as[A <: Label](alias :A) :CommonTableAlias[A, M] =
		CommonTableAlias[A, query.RowMapping, Row](alias, query)
...
object CommonTableExpression {
	...
	def apply[M[O] <: MappingAt[O], V]
	         (alias :String, select : => QuerySQL[RowProduct, V] { type RowMapping[O] = M[O] })
			:CommonTableExpression[M] =
		new BaseCommonTableExpression[V, M](alias, select)

	//why doesn't this signature work here when it works in TableExpression?
	def apply[M[O] <: MappingAt[O], V]
	         (select : => QuerySQL[RowProduct, V] { type RowMapping[O] = M[O] }) :CommonTableExpression[M] =
		new BaseCommonTableExpression[V, M](AnonymousName, select) {
			override val name = {
				val hash32 = System.identityHashCode(this)
				val hash16 = hash32 ^ (hash32 >> 16) & 0xffff
				AnonymousName + "@" + Integer.toHexString(hash16)
			}
	...
	class WithTableAsFactory[A <: Label] private[CommonTableExpression](private val alias :A) extends AnyVal {
		def as[M[O] <: MappingAt[O], V]
		      (select : => QuerySQL[RowProduct, V] { type RowMapping[O] = M[O] }) :CommonTableAlias[A, M] =
			CommonTableAlias[A, M, V](alias, select)
	}
		...
		extends StaticTable[A, M] with CommonTableExpression[M] with RelationTemplate[M, CommonTableAlias[A, M]]
	{
		...
		protected override def alter[O](includes :Unique[TypedMapping[_, O]], excludes :Unique[TypedMapping[_, O]])
			...
			if (includes.isEmpty && excludes.isEmpty)
				...
				{
//					override val table      = CommonTableAlias.this
	...
	object CommonTableAlias {
		def apply[A <: Label, M[O] <: MappingAt[O], V]
		         (alias :A, select : => QuerySQL[RowProduct, V] { type RowMapping[O] = M[O] })
				:CommonTableAlias[A, M] =
			new BaseCommonTableExpression[V, M](alias, select) with CommonTableAlias[A, M] {
	...
	private class BaseCommonTableExpression[V, +M[O] <: MappingAt[O]]
...
//	private class AnonymousCommonTableExpression
...
//		else (new WithClauseClosureBuilder(this).addLocal(table) ++= table.withClause).result()
	...
	/** Adds the given common table expression to this clause's [[net.noresttherein.oldsql.sql.WithClause.outer outer]]
	  ...
	  * to the default column selection are abstracted over by this class.
	  */
	def +:(table :CommonTableExpression.__)  :WithClause //= table.withClause ++ this
...
//	private class DerivedLocalWithClauseBuilder(original :WithClause) extends DerivedWithClauseBuilder(original) {
//		private[this] val outer = original.outer
//
//		override def addOne(elem :CommonTableExpression.*) :this.type =
//			if (outer.contains(elem)) addOuter(elem)
//			else super.addOne(elem)
//	}
		...
		extends WithClauseClosureBuilder with Builder[CommonTableExpression.__, WithClause]
	{
		...
		override def addOne(elem :CommonTableExpression.__) :this.type =
			if (local.contains(elem)) {
				if (localBuilder.add(elem.default)) {
		...
		override def result() :WithClause =
			if (suppressedBuilder.isEmpty)
				if (original.suppressed.isEmpty)
					...
					val res = fromParts(localBuilder, outerBuilder, original.suppressed)
					clear()
					res
				}
			else {
...
//		override def ++:(tables :WithClause) = tables
	...
	private trait OuterWithClause extends WithClause {
		override def local      :WithClause = empty
		override def outer      :WithClause = this
		override def suppressed :WithClause = empty
		...
		override def local_+(table :CommonTableExpression.__) :WithClause = {
			...
			val outer =
				...
				else if (clause.sizeIs == 1) toUnique
...
//		override def closure = this
	...
	{ //consider: we could have a more efficient implementation if it is complete and checks arguments for completeness
		...
		override lazy val local :WithClause =
			...
			else if (localCTEs.isEmpty && suppressed.isEmpty) empty
...
//		override def closure = this
		...
		override def local_++(tables :WithClause) :WithClause =
			if (tables.isEmpty && tables.suppressed.isEmpty)
				...
				val outer = addAbsent(outerCTEs, tables, empty, empty)
		...
		override def ++(tables :WithClause) :WithClause =
			if (tables.isEmpty && tables.suppressed.isEmpty)
				this
			else if (isEmpty && suppressed.isEmpty)
				tables
			else {
...
//				else new FlatOuterWithClause(Unique.single(tables.head))
...
//				new FlatOuterWithClause(suppressed to Unique)
		}
		if (isEmpty(local))
			if (isEmpty(outer))
