...
import net.noresttherein.oldsql.sql.RowShape.{JDBCTypeExtensions, Nulls, Indefinite}
...
object RowShape {
	...
	object Indefinite {
		...
		private final val IndefiniteCacheSize = 32
	}
...
//		override def canEqual(that :Any) :Boolean = that.isInstanceOf[Nulls]
	...
	implicit class JDBCTypeExtensions(private val tpe :JDBCType) extends AnyVal {
		...
		def <:<(other :JDBCType) :Boolean = tpe == other //todo:
		def <:>(other :JDBCType) :Boolean = tpe == other
