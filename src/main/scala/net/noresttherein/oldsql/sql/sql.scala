...
package object sql {
	...
	/** Represents the multipurpose 'everything' wildcard `*` in SQL. Can be used as the argument to
	  ...
	  * It is useful because it allows to use `select` as an infix method, rather than nullary `.select()`:
	  * {{{
	  *     From(Hamsters) select *
	  * }}}
	  */
...
//	type GlobalColumn[-]
	...
	/** Creates a dedicated [[net.noresttherein.oldsql.schema.Relation Relation]] for a synthetic parameter mapping
	  ...
	  * without a label, as well as an implicit conversion adding
	...
	/** Adds a [[net.noresttherein.oldsql.sql.NamedParamRelationFromLabel.?: ?:]] method to `String` literals
	  ...
	  */ //consider: another place for it, so all implicits can be imported at once
	implicit def extension_?:[N <: Label](name :N) :NamedParamRelationFromLabel[N] =
		new NamedParamRelationFromLabel[N](name)
...
*/
	...
	object GroupByVal {
				...
				:F GroupByVal S =
		{
			...
			val last = GroupingSQL[G, RowProduct AndByVal S, Group[S]#M, S](
				grouping.upcast, 0
			)
	...
	object GroupByOne {
		...
		{
			val grouping = GroupingRelation(group.anchor(from.generalized))
			val last = GroupingSQL[from.Generalized, RowProduct AndByOne S, Group[S]#C, S](
				grouping.upcast, 0
			)
	...
	/** An implicit argument used to 'seal' methods to package `sql`. A method `private[scope]` or `protected[scope]`
	  ...
	  * of `Seal` (a value of which is always available) ensures that the method cannot be overridden, as `Seal`
	...
	/** A value wrapper with visibility restricted to package `sql`, ensuring that any definition including it
	  * can neither be used nor overridden by extending classes from outside this package. A declaration
