...
import net.noresttherein.oldsql.sql.ast.{denullify, BoundColumnParam, BoundParam, ColumnComponentSQL, ColumnLValueSQL, ComponentSQL, GenericColumnComponentSQL, LValueSQL, SQLNull}
...
trait ComponentSetter[-L <: RowProduct, -R <: RowProduct] extends Serializable { self =>
	...
	/** Splits this assignment into assignments for individual columns of this component.
	  ...
	  * by `lvalue.`[[net.noresttherein.oldsql.sql.SQLExpression.realign reform]]`(rvalue)`. This is a symmetrical
			...
			:Seq[ColumnSetter[From[M], RowProduct]] =
	{
		...
		columns.view.map { column =>
			def filter[T](column :TypedColumn[T, RowProduct AndFrom M]) :ColumnSetter[From[M], RowProduct] = {
				...
				if (t.isDefined) domain.last \ column := BoundColumnParam(column.form, t.get)
