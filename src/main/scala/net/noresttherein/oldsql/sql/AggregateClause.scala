...
import net.noresttherein.oldsql.sql.RowProduct.{AggregateOfGeneralized, ExpandedBy, GroundFrom, NonEmptyFrom, PartOf, PrefixOf, RowProductTemplate}
...
sealed trait Aggregated[+F <: FromSome] extends DecoratedFrom[F] with AggregateClause { thisClause =>
	...
	override type SelectedFrom[+P <:  NonEmptyFrom] = Aggregated[clause.SelectedFrom[P]]
...
//		spelling(this)(context)
	...
	protected override def defaultSpelling[P](context :SQLContext[P], params :Parameterization[P, Generalized])
	                                         (implicit spelling :SQLSpelling) :SpelledSQL[P] =
	{
		val res = clause.spell(context, params.fromClause[clause.Generalized])
		SpelledSQL(res.sql, res.context.aggregate, res.setter)
	}

	override def spellingContext(implicit spelling :SQLSpelling) :SQLContext[Any] = clause.spellingContext.aggregate
	...
	protected override def groupingSpellingContext[P]
		...
		groupingSpellingContext(clause)(position + fromClause.size, context.reset(groupings = -1), params.fromClause)
