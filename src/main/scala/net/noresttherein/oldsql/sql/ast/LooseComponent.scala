...
import net.noresttherein.oldsql.sql.mechanics.{=~=, RelationCount, SpelledSQL, SQLConversion}
...
class LooseComponent[F <: RowProduct, M[A] <: BaseMapping[V, A], V] private[ast]
	...
	extends LValueSQL[F, M, V] with SelectableMappingSQL[F, M]
...
*/
	...
	override def substitute[E <: F]
	                       (substitutes :Iterable[ColumnSetter[FromLast, E]]) :MappingSQL[E, Single, M, Subject] =
...
//	@throws[UnsupportedOperationException]("if this expression is not a ColumnSQL.")
	...
	protected override def shape(implicit spelling :SQLSpelling) :RowShape =
		throw new UndefinedShapeException(
			"Not anchored component " + this + " has undetermined columns."
		)
