...
import net.noresttherein.oldsql.sql.ast.AdaptedColumnSQL.AdaptedColumnOnlySQL
...
import net.noresttherein.oldsql.sql.mechanics.{SpelledSQL, SQLAdaptation, SQLNumber}
import net.noresttherein.oldsql.sql.mechanics.SpelledSQL.{Parameterization, SQLContext}
import net.noresttherein.oldsql.sql.mechanics.SQLAdaptation.{ArbitraryAdaptation, ColumnAdaptation}
import net.noresttherein.oldsql.sql.mechanics.SQLNumber.{SQLFraction, SQLInteger}
import net.noresttherein.oldsql.sql.mechanics.SQLTransformation.ArbitraryColumnTransformation
...
object ArithmeticSQL {
	...
	type __ = ArithmeticSQL[_ <: RowProduct, _ >: Grouped <: Single, _]


	sealed abstract class AbstractUnaryOperation(val symbol :String) extends Serializable {
		def apply[V :SQLNumber](value :V) :Opt[V] = Lack

		def apply[F <: RowProduct, S >: Grouped <: Single, V:SQLNumber]
		         (value :ColumnSQL[F, S, V]) :ArithmeticSQL[F, S, V] 
		...
		override def equals(that :Any) :Boolean = that match {
			...
			case other :AbstractUnaryOperation if other canEqual this => symbol == other.symbol
			case _ => false
		}
		def canEqual(that :Any) :Boolean = that.isInstanceOf[AbstractUnaryOperation]
		override def hashCode :Int = symbol.hashCode
		override def toString :String = symbol
	}


	private abstract class UnaryOperationAdaptation[V :SQLNumber](val operation :AbstractUnaryOperation)
		extends SQLAdaptation[V, V]
//		extends ColumnAdaptation[V, V] with ArbitraryColumnTransformation[V, V]
	{
		...
		override def apply(value :V) :V = operation(value) orIllegal "Cannot " + operation + " " + value + "."
	}
	...
	class UnaryOperationSQL[-F <: RowProduct, -S >: Grouped <: Single, V]
		...
		extends AdaptedColumnOnlySQL[F, S, V, V] with ArithmeticSQL[F, S, V]
	{
		...
		override def transformation :SQLAdaptation[V, V] =
			//We don't extend SQLFormIdentity because it extends SQLValueIdentity and someone may match against it
			new UnaryOperationAdaptation(operation) with SelfColumnAdaptation[ColumnSQL] {
				override def applyString(arg :String) :String = "(" + this.operation.symbol + " " + arg + ")"
			}
...
//			visitor.unaryArithmetic(this)
	...
	object UnaryOperationSQL {
		...
		class UnaryOperation(symbol :String) extends AbstractUnaryOperation(symbol) {
			override def apply[F <: RowProduct, S >: Grouped <: Single, V :SQLNumber]
			                  (value :ColumnSQL[F, S, V]) :UnaryOperationSQL[F, S, V] =
				new UnaryOperationSQL(this, value)

			override def unapply[F <: RowProduct, S >: Grouped <: Single, V]
			                    (e :SQLExpression[F, S, V]) :Opt[ColumnSQL[F, S, V]] =
				e match {
			...
			override def canEqual(that :Any) :Boolean = that.isInstanceOf[UnaryOperation]
		}
...
//			UnaryOperationVisitor[F, R]
		...
		extends AdaptedColumnOnlySQL[F, S, V, V] with ArithmeticSQL[F, S, V]
	{
		...
		//We don't extend SQLFormIdentity because it extends SQLValueIdentity and someone may match against it
			new UnaryOperationAdaptation(operation) with SelfColumnAdaptation[ColumnSQL] {
				override def applyString(arg :String) :String = "(" + arg + " " + this.operation.symbol + ")"
			}
...
//			visitor.unaryPostfix(this)
	...
	object UnaryPostfixOperationSQL {
		...
		class UnaryPostfixOperation(symbol :String) extends AbstractUnaryOperation(symbol) {
			override def apply[F <: RowProduct, S >: Grouped <: Single, V :SQLNumber]
			...
			override def canEqual(that :Any) :Boolean = that.isInstanceOf[UnaryPostfixOperation]
		}
...
//			UnaryPostfixOperationVisitor[F, R]
		...
		extends BinaryCompositeColumn[F, S, V, V] with ArithmeticSQL[F, S, V]
	{
		...
		override def universalForm :Opt[ColumnForm[V]] = left.universalForm
		...
		protected override def effectiveForm(implicit spelling :SQLSpelling) :ColumnForm[V] = spelling.effectiveForm(left)
