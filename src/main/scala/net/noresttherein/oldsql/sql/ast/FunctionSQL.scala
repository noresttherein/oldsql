...
import net.noresttherein.oldsql.sql.SQLExpression.{AnyExpressionVisitor, Single, Grouped, SpecificExpressionVisitor}
...
trait FunctionSQL[-F <: RowProduct, -S >: Grouped <: Single, X <: Chain, Y] extends CompositeSQL[F, S, Y] {
	...
	protected override def split(implicit spelling :SQLSpelling) :Seq[ColumnSQL[F, S, _]] =
		throw new InseparableExpressionException(
			this, "Cannot split a non-column function call " + this + " into columns."
		)
...
//consider: moving to package ast level
