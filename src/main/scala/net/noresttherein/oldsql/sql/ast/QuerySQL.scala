...
import net.noresttherein.oldsql.sql.ast.CompoundSelectColumnMapping.CompoundSelectColumnMappingVisitor
import net.noresttherein.oldsql.sql.ast.CompoundSelectMappingSQL.{CaseCompoundSelectMapping, CompoundSelectMappingVisitor}
import net.noresttherein.oldsql.sql.ast.CompoundSelectColumn.{CaseCompoundSelectColumn, CompoundSelectColumnVisitor}
import net.noresttherein.oldsql.sql.ast.CompoundSelectSQL.CaseCompoundSelect
import net.noresttherein.oldsql.sql.ast.SelectAs.{CaseSelectMapping, SelectMappingVisitor}
import net.noresttherein.oldsql.sql.ast.SelectColumn.{CaseSelectColumn, SelectColumnVisitor}
import net.noresttherein.oldsql.sql.ast.SelectColumnAs.SelectColumnMappingVisitor
import net.noresttherein.oldsql.sql.ast.SelectSQL.{CaseSelect, SelectVisitor}
...
//		def as[A <: Label](alias :A) :With[M] As A = With(alias, this) //todo: With for MappingQuerySQL
	...
	/** An SQL query, that is an SQL [[net.noresttherein.oldsql.sql.ast.SelectColumn select]] or
	  * a [[net.noresttherein.oldsql.sql.ast.CompoundSelectColumn compound select]] on them, which returns
	  * a single column.
	  */
...
//			matcher.query(this)
	...
	/** An SQL query, that is an SQL [[net.noresttherein.oldsql.sql.ast.SelectAs select]] or
	  * a [[net.noresttherein.oldsql.sql.ast.CompoundSelectMappingSQL compound select]] on them,
...
//			matcher.mappingQuery(this)
	...
	/** An SQL query returning a single column of some relation.
	  ...
	  * @see [[net.noresttherein.oldsql.sql.ast.CompoundSelectColumnMapping]]
	  */
...
//			matcher.mappingQuery(this)
		...
		extends MatchColumnMappingQuery[F, Y]
	{
		override def selectMapping[H[O] <: ColumnMapping[V, O], V]
		                          (e :SelectColumnAs[F, H, V]) :Y[GlobalScope, Rows[V]] =
			mappingQuery(e)
		...
		extends MatchMappingQuery[F, Y]
	{
		override def selectMapping[H[O] <: MappingAt[O]]
		                          (e :SelectAs[F, H]) :Y[GlobalScope, Rows[H[Unit]#Subject]] =
			mappingQuery(e)
...
object CompoundSelectSQL {
	...
	private class BaseCompoundSelectSQL[-F <: RowProduct, V]
...
object CompoundSelectColumn {
	...
	private class BaseCompoundSelectColumn[-F <: RowProduct, V]
	                                      (override val left :ColumnQuery[F, V], override val operator :SetOperator,
	                                       override val right :ColumnQuery[F, V])
...
object CompoundSelectMappingSQL {
	...
	private class BaseCompoundSelectMappingSQL[-F <: RowProduct, M[O] <: MappingAt[O]]
	                                          (override val left :MappingQuerySQL[F, M],
...
object CompoundSelectColumnMapping {
	...
	private class BaseCompoundSelectColumnMapping[-F <: RowProduct, M[O] <: ColumnMapping[V, O], V]
	                                             (override val left :ColumnMappingQuery[F, M, V],
	                                              override val operator :SetOperator,
	                                              override val right :ColumnMappingQuery[F, M, V])
