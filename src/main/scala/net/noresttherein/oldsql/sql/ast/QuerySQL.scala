...
import net.noresttherein.oldsql.schema.{ColumnForm, ColumnReadForm, ColumnWriteForm, SQLForm, SQLReadForm, SQLWriteForm, Table}
...
import net.noresttherein.oldsql.slang.{cast2TypeParams, classNameMethods}
import net.noresttherein.oldsql.sql.{ColumnSQL, CommonTableExpression, RowProduct, RowShape, SQLDialect, SQLExpression, StandardSQL, WithClause}
import net.noresttherein.oldsql.sql.ColumnSQL.{ColumnShape, VariantColumnGroundingTemplate}
...
import net.noresttherein.oldsql.sql.Query.{DecoratorQueryTemplate, QueryReformingTemplate, QueryTemplate, SingleQueryTemplate}
...
import net.noresttherein.oldsql.sql.SQLExpression.{AnyExpressionVisitor, Grouped, ReorderingTemplate, SQLShape, Single, SingleRowSQLTemplate, SpecificExpressionVisitor}
...
import net.noresttherein.oldsql.sql.mechanics.{=~=, AlignableColumns, Alignment, QueryReform, Reform, ReformPermissions, SQLConversion, SQLTransformation, SpelledSQL}
...
trait QuerySQL[-F <: RowProduct, R]
	extends SQLExpression[F, Single, Rows[R]]
	   ...
	   with ReorderingTemplate[F, Single, Rows[R], QuerySQL[F, R]]
	   with QueryTemplate[R, ({ type Q[r] = QuerySQL[F, r] })#Q]
	   with QueryReformingTemplate[R, QuerySQL[F, R]]
{
	...
	/** Converts this ''select'' to an expression for its returned value, representing its usage in SQL assuming
	  ...
	  */
	def single :SQLExpression[F, Single, R] = to[R]
...
//
//	private[sql] def commonTableExpression :CommonTableExpression[RowMapping] =
//		CommonTableExpression.arbitrary(this)
//
//	private[sql] def commonTableExpression[N <: Label](alias :N) :CommonTableAlias[N, RowMapping] =
//		CommonTableAlias.arbitrary(this)
...
*/
	...
	/** Reforms this query (as the second/right one in a pair) with the given argument (the first/left query)
	  ...
	  * from `this.`[[net.noresttherein.oldsql.sql.ast.QuerySQL.realign reform]] by
	...
	protected def reform_:[E <: F](first :SelectSQL[E, R])(reform :QueryReform)
	                              (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
		((first :SingleQuerySQL[E, R]) reform_: this)(reform)
	...
	protected override def realign(reordering :Rearrangement)(implicit spelling :SQLSpelling) :QuerySQL[F, R] =
		if (reordering.isIdentity)
			this
		else
			throw new UnsupportedOperationException(
				"Query `" + this + "` cannot be reordered to " + reordering + "."
			)
...
private[ast] sealed abstract class Rank2QuerySQLImplicits {
	...
	//todo: replace type projection with projection once we get rid of references to BaseMapping in API
	implicit def arbitraryTableExpression[V](query :QuerySQL[RowProduct, V]) :Table[MappingOf[V]#TypedProjection] =
		TableExpression[query.RowMapping, V](query).asInstanceOf[Table[MappingOf[V]#TypedProjection]]
//		TableExpression(query.supertyped)

}
private[ast] sealed abstract class Rank1QuerySQLImplicits extends Rank2QuerySQLImplicits {
	implicit def singleColumnTableExpression[V](query :ColumnQuery[RowProduct, V]) :Table[MappingOf[V]#ColumnProjection] =
...
//		def toCTE      :CommonTableExpression[M] = self.commonTableExpression
//		def toCTE[N <: Label](alias :N) :CommonTableAlias[N, M] = self.commonTableExpression(alias)

//		def as[A <: Label](alias :A) :M Aliased A = self.toTable as alias
		def as[A <: Label](alias :A) :CommonTableAlias[A, M] = CommonTableAlias(alias :A, self)//(CommonTableExpression.mappingOfQuery[M, V])
...
//		def as[A <: Label](alias :A) :With[M] As A = With(alias, this) //todo: With for MappingQuerySQL
	...
	implicit def tableExpression[M[O] <: MappingAt[O]](query :MappingQuerySQL[RowProduct, M]) :Table[M] =
		TableExpression[M, M[Unit]#Subject](query)

	implicit def commonTableExpressionByName[N <: Label](alias :N) :CTEName[N] = new CTEName(alias)
	...
	type ReformableQuerySQL[-F <: RowProduct, R, +Reformed] = QuerySQL[F, R] with QueryReformingTemplate[R, Reformed]
...
*/
	...
	trait SingleQuerySQL[-F <: RowProduct, R]
		extends QuerySQL[F, R]
		   ...
		   with SingleQueryTemplate[R, ({ type Q[X] = SingleQuerySQL[F, X] })#Q, QuerySQL[F, R]]
	{
		...
		protected override def reform[E <: F](second :QuerySQL[E, R])(reform :QueryReform)
		                                     (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
			(this `->reform_:` second)(reform) //calls this reform_: second

		protected override def reform_:[E <: F](first :SingleQuerySQL[E, R])(reform :QueryReform)
		                                       (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
		{
			val validator = reform.reform.prohibitAll
			validator[Nothing, Grouped, R, SQLShape, Nothing, Grouped, R, SQLShape, R](first.selectClause, selectClause)(
		...
		protected override def reform_:[E <: F](first :SelectSQL[E, R])(reform :QueryReform)
		                                       (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
		{
			val reformFirst = reform.reform.prohibitReformRight
			type Converted[V] = SQLExpression[first.From, Grouped, V]
			val reformed = reformFirst[first.From, Grouped, R, Converted, Nothing, Grouped, R, SQLShape, R](
	...
	trait DecoratorQuerySQL[-F <: RowProduct, R]
		extends QuerySQL[F, R] with DecoratorQueryTemplate[R, ({ type Q[X] = QuerySQL[F, X] })#Q]
	{
		...
		override def isGround = query.isGround
	...
	/** The value type of [[net.noresttherein.oldsql.sql.ast.QuerySQL QuerySQL]] instances
	  ...
	  */ //todo: rename to Selected/Cursor or something and move out; at the very least should be in Query with templates
	...
	private[QuerySQL] sealed abstract class Rank3RowsImplicits {
	...
	private[QuerySQL] sealed abstract class Rank2RowsImplicits extends Rank3RowsImplicits {
		implicit def form[T :SQLForm] :SQLForm[Rows[T]] =
			SQLForm.map("Rows")(Rows.single[T])(_.one)
	}

	private[QuerySQL] sealed abstract class Rank1RowsImplicits extends Rank2RowsImplicits {
		implicit def columnReadForm[T :ColumnReadForm] :ColumnReadForm[Rows[T]] =
			ColumnReadForm.map("Rows")((t :T) => Rows(t))

		implicit def columnWriteForm[T :ColumnWriteForm] :ColumnWriteForm[Rows[T]] =
	...
	object Rows extends Rank1RowsImplicits {
		...
		val empty :Rows[Nothing] = MultipleRows(Nil)

		implicit def columnForm[T :ColumnForm] :ColumnForm[Rows[T]] =
			ColumnForm.map("Rows")(Rows.single[T])(_.one)
...
trait ColumnQuery[-F <: RowProduct, R]
	extends QuerySQL[F, R]
	   with ColumnSQL[F, Single, Rows[R]]
	   with SingleRowSQLTemplate[F, Rows[R], ({ type Q[-f <: RowProduct] = ColumnQuery[f, R] })#Q]
	   with VariantColumnGroundingTemplate[F, Single, Rows[R], ({ type Q[-f <: RowProduct] = ColumnQuery[f, R] })#Q]
//	   with ReorderingTemplate[F, Single, Rows[R], ColumnQuery[F, R]]
		//I don't think we should force ColumnQuery result on reformed
	   with QueryTemplate[R, ({ type Q[X] = ColumnQuery[F, X] })#Q]
{
...
//	override def reformed(implicit spelling :SQLSpelling) :ColumnQuery[F, R] = spelling.queryReform(this)
//	protected override def reformed(reform :QueryReform)(implicit spelling :SQLSpelling) :ColumnQuery[F, R] = this
...
//			matcher.query(this)
	...
	protected abstract override def reform[E <: F](second :QuerySQL[E, R])(reform :QueryReform)
	                                              (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
		second match {
			case column :ColumnQuery[E, R] if reform.compatible(this, column) => (this, second)
			case _ => super.reform(second)(reform)
		}
	protected abstract override def reform_:[E <: F](first :SingleQuerySQL[E, R])(reform :QueryReform)
	                                                (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
		first match {
			case column :ColumnQuery[E, R] if reform.compatible(column, this) => (column, this)
			case _ => super.reform_:(first)(reform)
		}
	protected abstract override def reform_:[E <: F](first :SelectSQL[E, R])(reform :QueryReform)
	                                                (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
		first match {
			case column :ColumnQuery[E, R] if reform.compatible(column, this) => (column, this)
			case _ => super.reform_:(first)(reform)
	...
	protected override def realign(reordering :Rearrangement)(implicit spelling :SQLSpelling) :ColumnQuery[F, R] = {
		super[ColumnSQL].realign(reordering) //validate
		this
	}
...
object ColumnQuery {
	...
	trait ColumnSingleQuery[-F <: RowProduct, R]
		extends SingleQuerySQL[F, R] with ColumnQuery[F, R]
		   with SingleRowSQLTemplate[F, Rows[R], ({ type Q[-f <: RowProduct] = ColumnSingleQuery[f, R] })#Q]
		   with SingleQueryTemplate[R, ({ type Q[X] = ColumnSingleQuery[F, X] })#Q, SingleQuerySQL[F, R]]
...
//	   with ReorderingTemplate[F, Single, Rows[M[Unit]#Subject], MappingQuerySQL[F, M]] //todo: SelectAs for any MappingSQL
{
...
//	private[sql] override def commonTableExpression :CommonTableExpression[M] = CommonTableExpression(this)
//	private[sql] override def commonTableExpression[N <: Label](alias :N) :CommonTableAlias[N, M] =
//		CommonTableAlias(alias, this)
...
trait ColumnMappingQuery[-F <: RowProduct, M[O] <: BaseColumn[V, O], V] //todo: replace TypedColumn with ColumnAt
	extends ColumnQuery[F, V] with MappingQuerySQL[F, M]
	   with SingleRowSQLTemplate[F, Rows[V], ({ type Q[-f <: RowProduct] = ColumnMappingQuery[f, M, V] })#Q]
		//todo: SelectColumnMapping for any ColumnMappingSQL
//	   with ReorderingTemplate[F, Single, Rows[V], ColumnMappingQuery[F, M, V]]
{
