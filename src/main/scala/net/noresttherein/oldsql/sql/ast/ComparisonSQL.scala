...
import net.noresttherein.oldsql.sql.ast.DominationComparisonSQL.{AnyDominationVisitor, SpecificDominationVisitor}
import net.noresttherein.oldsql.sql.ast.NullDominationSQL.{AnyNullDominationVisitor, SpecificNullDominationVisitor}
...
trait ComparisonSQL[-F <: RowProduct, -S >: Grouped <: Single, T]
	extends ConditionSQL[F, S] //not a BinaryCompositeColumnSQL because the operands might be multi column
	   with BinaryCompositeSQL[F, S, T, T, Boolean]
	   with BinaryCompositeTemplate[F, S, T, T, Boolean,
	                                ({ type E[-f <: RowProduct, -s >: Grouped <: Single] = ColumnSQL[f, s, Boolean] })#E]
{
...
//		val operatorSQL = " " + cmpSpelling.operator(symbol) + " "
	...
	override def hashCode :Int = super[BinaryCompositeTemplate].hashCode * 31 + operator.hashCode
...
//		private def comparisonMatchAny :Int = cmpResult
	...
	trait SpecificComparisonVisitor[+F <: RowProduct, +S >: Grouped <: Single, X, +Y]
		extends SpecificOrderComparisonVisitor[F, S, X, Y] with SpecificNullOrderComparisonVisitor[F, S, X, Y]
		   with SpecificDominationVisitor[F, S, X, Y] with SpecificNullDominationVisitor[F, S, X, Y]
		...
		extends MatchSpecificComparison[F, S, X, Y]
	{
		...
		override def domination[V](e :DominationComparisonSQL[F, S, V])(implicit isBoolean :X =:= Boolean) :Y =
			comparison(e)
		...
		override def nullOrder[V](e :NullOrderComparisonSQL[F, S, V])(implicit isBoolean :X =:= Boolean) :Y =
			comparison(e)
		override def nullDomination[V](e :NullDominationComparisonSQL[F, S, V])(implicit isBoolean :X =:= Boolean) :Y =
			comparison(e)
	}
//
//
...
//	}
	...
	trait AnyComparisonVisitor[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]]
		extends AnyOrderComparisonVisitor[F, Y] with AnyNullOrderComparisonVisitor[F, Y]
		   with AnyDominationVisitor[F, Y] with AnyNullDominationVisitor[F, Y]
	...
	trait CaseAnyComparison[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]] extends MatchAnyComparison[F, Y] {
		...
		override def domination[S >: Grouped <: Single, X](e :DominationComparisonSQL[F, S, X]) :Y[S, Boolean] =
			comparison(e)
		...
		override def nullOrder[S >: Grouped <: Single, X](e :NullOrderComparisonSQL[F, S, X]) :Y[S, Boolean] =
			comparison(e)
		override def nullDomination[S >: Grouped <: Single, X](e :NullDominationComparisonSQL[F, S, X]) :Y[S, Boolean] =
			comparison(e)
	}
}
...
/** Comparison of magnitude on all columns. `left `''operator''` right` is equivalent to
  * {{{
  *     left.split.zip(right.split).foldLeft(True) { (sql, columns) => sql && columns._1 <= columns._2 }
  * }}}
  * where `<=` is used to stand for `this.operator`. It is an error if expressions have different numbers of columns.
  */ //todo: create factory methods in SQLExpression
case class DominationComparisonSQL[-F <: RowProduct, -S >: Grouped <: Single, T]
           (left :SQLExpression[F, S, T], operator :OrderingOperator, right :SQLExpression[F, S, T])
           (implicit val ordering :SQLOrdering[T])
	extends ComparisonSQL[F, S, T]
{
	override def groundValue :Opt[Boolean] =
	...
	protected override def reapply[E <: RowProduct, C >: Grouped <: Single]
	                              (left :SQLExpression[E, C, T], right :SQLExpression[E, C, T]) :ComparisonSQL[E, C, T] =
		new DominationComparisonSQL(left, operator, right)

	protected override def defaultSpelling[P](from :F, context :SQLContext[P], params :Parameterization[P, F])
	                                         (implicit spelling :SQLSpelling) :SpelledSQL[P] =
		columnwiseSpelling(" " + spelling.operator(operator.symbol) + " ")(from, context, params)

	protected override def visit[Y[-_ >: Grouped <: Single, _]]
	                            (visitor :AnyColumnVisitor[F, Y]) :Y[S, Boolean] = visitor.domination(this)

	protected override def visit[Y](visitor :SpecificColumnVisitor[F, S, Boolean, Y]) :Y = visitor.domination(this)
}


object DominationComparisonSQL {
	trait SpecificDominationVisitor[+F <: RowProduct, +S >: Grouped <: Single, X, +Y] {
		def domination[V](e :DominationComparisonSQL[F, S, V])(implicit isBoolean :X =:= Boolean) :Y
	}

	type MatchSpecificDomination[+F <: RowProduct, +S >: Grouped <: Single, X, +Y] = SpecificDominationVisitor[F, S, X, Y]
	type CaseSpecificDomination[+F <: RowProduct, +S >: Grouped <: Single, X, +Y] = SpecificDominationVisitor[F, S, X, Y]

	trait AnyDominationVisitor[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]] {
		def domination[S >: Grouped <: Single, X](e :DominationComparisonSQL[F, S, X]) :Y[S, Boolean]
	}

	type MatchAnyDomination[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]] = AnyDominationVisitor[F, Y]
	type CaseAnyDomination[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]] = AnyDominationVisitor[F, Y]
}
...
object NullOrderComparisonSQL {
	...
	private[ast] def spell[P](left :SpelledSQL[P], operator :OrderingOperator, right :SpelledSQL[P])
	                         (implicit spelling :SQLSpelling) :SpelledSQL[P] =
		if (operator.isStrict)
			"(" +: (left + (" " + spelling.operator(operator.symbol) + " ") + right + spelling._OR_ +
				left + (spelling._IS_ + spelling.NULL + spelling._AND_) +
					right + (spelling._IS_ + spelling.NOT + " " + spelling.NULL + ")"))
		else
			"(" +: (left + (" " + spelling.operator(operator.symbol) + " ") + right + spelling._OR_ +
...
case class NullDominationComparisonSQL[-F <: RowProduct, -S >: Grouped <: Single, T]
           (left :SQLExpression[F, S, T], operator :OrderingOperator, right :SQLExpression[F, S, T])
           (implicit val ordering :SQLOrdering[T])
	extends ComparisonSQL[F, S, T]
{
	override def groundValue :Opt[Boolean] = Lack
//		for (l <- left.groundValue; r <- right.groundValue) yield operator(l, r)

	protected override def reapply[E <: RowProduct, C >: Grouped <: Single]
	                              (left :SQLExpression[E, C, T], right :SQLExpression[E, C, T]) :ComparisonSQL[E, C, T] =
		new NullDominationComparisonSQL(left, operator, right)
	...
	protected override def columnSpelling[P](left :SpelledSQL[P], right :SpelledSQL[P])
	                                        (implicit spelling :SQLSpelling) :SpelledSQL[P] =
		NullOrderComparisonSQL.spell(left, operator, right)

	protected override def visit[Y[-_ >: Grouped <: Single, _]]
	                            (visitor :AnyColumnVisitor[F, Y]) :Y[S, Boolean] = visitor.nullDomination(this)

	protected override def visit[Y](visitor :SpecificColumnVisitor[F, S, Boolean, Y]) :Y = visitor.nullDomination(this)
//
//	protected override def visit[F_ <: F, S_ >: Grouped <: S,
//	                             E >: ColumnSQL[F, S, Boolean] <: SQLExpression[F_, S_, Boolean],
//	                             R[-s >: Grouped <: Single, v, -e <: SQLExpression[F_, s, v]]]
//	                            (visitor :ColumnVisitor[F, R]) :R[S_, Boolean, E] =
//		visitor.domination(this)
}


object NullDominationSQL {

	trait SpecificNullDominationVisitor[+F <: RowProduct, +S >: Grouped <: Single, X, +Y] {
		def nullDomination[V](e :NullDominationComparisonSQL[F, S, V])(implicit isBoolean :X =:= Boolean) :Y
	}
	type MatchSpecificNullDomination[+F <: RowProduct, +S >: Grouped <: Single, X, +Y] =
		SpecificNullDominationVisitor[F, S, X, Y]
	type CaseSpecificNullDomination[+F <: RowProduct, +S >: Grouped <: Single, X, +Y] =
		SpecificNullDominationVisitor[F, S, X, Y]
//
//	trait DominationVisitor[+F <: RowProduct, +R[-S >: Grouped <: Single, V, -E <: SQLExpression[F, S, V]]] {
//		def domination[S >: Grouped <: Single, N](e :DominationSQL[F, S, N]) :R[S, Boolean, DominationSQL[F, S, N]]
//	}
//	type MatchEqualaity[+F <: RowProduct, +R[-S >: Grouped <: Single, V, -E <: SQLExpression[F, S, V]]] =
//		DominationVisitor[F, R]
//	type CaseEqualaity[+F <: RowProduct, +R[-S >: Grouped <: Single, V, -E <: SQLExpression[F, S, V]]] =
//		DominationVisitor[F, R]

	trait AnyNullDominationVisitor[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]] {
		def nullDomination[S >: Grouped <: Single, X](e :NullDominationComparisonSQL[F, S, X]) :Y[S, Boolean]
	}
	type MatchAnyNullDomination[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]] = AnyNullDominationVisitor[F, Y]
	type CaseAnyNullDomination[+F <: RowProduct, +Y[-_ >: Grouped <: Single, _]] = AnyNullDominationVisitor[F, Y]
}
	...
	extends ComparisonSQL[F, S, T]
{
	...
	protected override def reapply[E <: RowProduct, C >: Grouped <: Single]
	                              (left :SQLExpression[E, C, T], right :SQLExpression[E, C, T]) :ComparisonSQL[E, C, T] =
		new NullEqualitySQL(left, right)
