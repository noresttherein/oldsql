...
import net.noresttherein.oldsql.sql.ast.CompositeSQL.{BinaryCompositeTemplate, UnaryCompositeSQL, UnaryCompositeTemplate}
...
trait ConditionSQL[-F <: RowProduct, -S >: Grouped <: Single] extends CompositeColumnSQL[F, S, Boolean] {
	override def universalForm :Opt[ColumnForm[Boolean]] = Got(ColumnForm[Boolean])
	override def selectForm    :ColumnReadForm[Boolean] = ColumnForm[Boolean]

	protected override def effectiveForm(implicit spelling :SQLSpelling) :ColumnForm[Boolean] = ColumnForm[Boolean]
...
object ConditionSQL {
	type __ = ConditionSQL[_ <: RowProduct, _ >: Grouped <: Single]
...
private[ast] trait NullCheck[-F <: RowProduct, -S >: Grouped <: Single, T]
	extends ConditionSQL[F, S]
	   with UnaryCompositeSQL[F, S, T, Boolean] //theoretically not needed for anything, other than consistency
	   with UnaryCompositeTemplate[F, S, T, Boolean, //not UnaryCompositeColumn because value is not a column
	                               ({ type E[-f <: RowProduct, -s >: Grouped <: Single] = ColumnSQL[f, s, Boolean] })#E]
{
	...
	override def rephrase[E <: RowProduct](mapper :SQLScribe[F, E]) :ColumnSQL[E, S, Boolean] = {
		val v = mapper(value)
		if (v eq value) this.asInstanceOf[ColumnSQL[E, S, Boolean]] //this.castFrom[ColumnSQL[F, S, Boolean], ColumnSQL[E, S, Boolean]]
		else reapply(v)
	}
