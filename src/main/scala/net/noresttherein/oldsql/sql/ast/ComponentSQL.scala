...
import net.noresttherein.oldsql.morsels.{permutation, superPermutation}
import net.noresttherein.oldsql.pixies.Rearrangement
import net.noresttherein.oldsql.schema.{ColumnForm, ColumnMappingExtract, ColumnReadForm, Mapping, MappingExtract, Relation, SQLForm, SQLReadForm}
...
import net.noresttherein.oldsql.sql.{Adjoin, ColumnSQL, ColumnSetter, Expanded, RowProduct, RowShape, SQLExpression, Seal, Select, WithClause}
import net.noresttherein.oldsql.sql.ColumnSQL.{AnyColumnVisitor, ColumnReformingDefaults, InvariantColumnGroundingTemplate, SpecificColumnVisitor, VariantColumnGroundingTemplate}
...
import net.noresttherein.oldsql.sql.SQLExpression.{AnyExpressionVisitor, ConvertibleSQL, Grouped, InvariantGroundingTemplate, Single, SpecificExpressionVisitor}
...
import net.noresttherein.oldsql.sql.ast.ComponentSQL.{GenericComponentSQLTemplate, TypedComponentSQL, TypedComponentSQLTemplate}
...
import net.noresttherein.oldsql.sql.mechanics.{=~=, AlignableColumns, Reform, ReformPermissions, RelationCount, RelationOffset, SQLConversion, SQLTransformation, SpelledSQL}
...
import net.noresttherein.oldsql.sql.mechanics.ReformPermissions.Permissions.{MayExclude, MayInclude, NoReform}
import net.noresttherein.oldsql.sql.mechanics.SpelledSQL.{Parameterization, SQLContext}

//implicits
import net.noresttherein.oldsql.slang.mappingMethods
import net.noresttherein.oldsql.slang.{cast2TypeParams, castTypeParam, saferCasting}
...
/** An SQL [[net.noresttherein.oldsql.sql.SQLExpression expression]] representing a whole, potentially multi column,
  ...
  * individual column expressions may be split, for example in column-wise equality comparisons between two components.
  *
  ...
  *
  * It is defined as [[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling spelling]]`.`[[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.scope scope]]`.`[[net.noresttherein.oldsql.sql.SQLDialect.SpellingScope.defaultColumns[o](mapping:MappingAt[O],component:MapppingAt[O])* defaultColumns]]`(this.`[[net.noresttherein.oldsql.sql.ast.ComponentSQL.ComponentSQLTemplate.mapping mapping]]`, this.`[[net.noresttherein.oldsql.sql.ast.ComponentSQL.ComponentSQLTemplate.origin origin]]`.`[[net.noresttherein.oldsql.sql.ast.JoinedRelation.anchored anchored]]`)`.
  * A component is always included in its default form (for `anchored` mapping, so possibly after modifications
  * from its nominal mapping `M`). In particular, if it contains an `Explicit`/`NonDefault` buff, it is ignored
  * for the purpose of this expression. For example, an
  * [[net.noresttherein.oldsql.schema.bits.ForeignKeyMapping.inverse inverse foreign key]] component, defining a link
  * from one entities primary key to another one's foreign key, has said primary key as its sole subcomponent,
  * which normally exists independently of the reference component. For this reason the latter is non default,
  * so that a ''select'' against the table doesn't include those column(s) multiple times. However, when said
  * reference component is used explicitly as an expression, for example in a comparison with another component,
  * then its columns will be included as if the `NonDefault` buff was not present.
  *
...
trait ComponentSQL[-F <: RowProduct, M[A] <: MappingAt[A]]
	extends LValueSQL[F, M, M[Unit]#Subject] with SelectableMappingSQL[F, M]
	   with GenericComponentSQLTemplate[F, M, ({ type E[-f <: RowProduct] = ComponentSQL[f, M] })#E]
{ self =>

//	override type FromLast = origin.FromLast
//	override type Origin   = origin.Origin
	...
	override def universalForm :Opt[SQLForm[M[Unit]#Subject]] = {
		val read  = origin.anchored.selectForm(anchored)
		val write = origin.anchored.writeForm(anchored, origin.anchored.selectedByDefault(anchored))
		Got(read <> write)
	}
...
//	/** Returns `this`. */
//	override def underlying :LValueSQL[F, M, M[Unit]#Subject] = this
	...
	override def defaultColumns(implicit spelling :SQLSpelling) :Unique[TypedColumn[_, Origin]] =
		defaultColumns(anchored)

	override def mandatoryColumns(implicit spelling :SQLSpelling) :Unique[TypedColumn[_, Origin]] =
		mandatoryColumns(anchored)

	override def columns(component :MappingAt[Origin])(implicit spelling :SQLSpelling) :Unique[TypedColumn[_, Origin]] =
		nominalCounterparts(spelling.scope.applicableColumns(origin.anchored, component))

	override def defaultColumns(component :MappingAt[Origin])
	                           (implicit spelling :SQLSpelling) :Unique[TypedColumn[_, Origin]] =
		nominalCounterparts(spelling.scope.defaultColumns(origin.anchored, component))

	override def mandatoryColumns(component :MappingAt[Origin])
	                             (implicit spelling :SQLSpelling) :Unique[TypedColumn[_, Origin]] =
		nominalCounterparts(spelling.scope.mandatoryColumns(origin.anchored, component))

	private def nominalCounterparts(columns :Unique[TypedColumn[_, Origin]]) :Unique[TypedColumn[_, Origin]] = {
		val counterparts = mapping.columns.view.collect {
			case c if export.contains(origin.anchored.export(c)) => (origin.anchored.export(c), c)
		}.toMap
		columns.map(counterparts)
	}
...
//		new ComponentReformer[reform.LValue, E, C, X, U](other)(reform, passesAllowed)
	protected override def reform[F2 <: RowProduct, S2 >: Grouped <: Single, V2, EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
		...
		if (this eq other)
			...
			origin.`->reformComponent`[M, F, F2, S2, V2, EC2, U](this, other)(reform, passCount)//(leftResult, rightResult, spelling)

	protected override def reformer[F2 <: RowProduct, S2 >: Grouped <: Single, V2,
                                    EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
                                   (other :ConvertibleSQL[F2, S2, V2, EC2])(reform :Reform, passCount :PassCount)
                                   (implicit leftResult  :SQLTransformation[Subject, U],
                                             rightResult :SQLTransformation[V2, U], spelling :SQLSpelling)
			:SpecificExpressionVisitor
			 [F2, S2, V2, (leftResult.Expression[F, Single, LValueSQL[F, M, U]], rightResult.Expression[F2, S2, EC2[U]])] =
		//fixme: we must handle ParamSQL components the same way as BoundParams, not like components.
		new ComponentReformer[F2, S2, V2, EC2, U, leftResult.Expression, rightResult.Expression](
		                      other)(reform, passCount)(leftResult, rightResult, spelling)

	private[sql] def `->reformer`[F2 <: RowProduct, S2 >: Grouped <: Single, V2,
                                  EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
                                 (other :ConvertibleSQL[F2, S2, V2, EC2])(reform :Reform, passCount :PassCount)
                                 (implicit leftResult  :SQLTransformation[Subject, U],
                                           rightResult :SQLTransformation[V2, U], spelling :SQLSpelling)
			:SpecificExpressionVisitor
			 [F2, S2, V2, (leftResult.Expression[F, Single, LValueSQL[F, M, U]], rightResult.Expression[F2, S2, EC2[U]])] =
		reformer(other)(reform, passCount)


	protected class ComponentReformer[F2 <: RowProduct, S2 >: Grouped <: Single, V2,
	                                  EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U,
	                                 ...
	                                 (other :ConvertibleSQL[F2, S2, V2, EC2])(reform :Reform, passCount :PassCount)
	                                 (implicit leftResult  :SQLTransformation.Into[Subject, U, LR],
...
*/
		...
		override def mappingTerm[T[A] <: BaseMapping[V2, A]](e :MappingTerm[T, V2]) :Result =
			if (passCount.secondTime)
				if (reform.mayAlterRight && rightResult.isReversible)
					...
					if (isAlterable && reform.mayAlterLeft) //todo: reforming of ComponentSQL with a MappingTerm
						???
		...
		override def indexed[L <: Listing](e :IndexedSQL[F2, S2, L])(implicit isListing :V2 =:= L) :Result =
			if (!passCount.secondTime)
				fallback
			else {
//				val rightRes = isListing.substituteCo[({ type T[A] = SQLTransformation[A, U]#Into[RR] })#T](rightResult)
				...
				val labelsMatchDefaultColumns =
					defaultColumns.size == items.length && defaultColumns.corresponds(items) {
				...
				if (labelsMatchDefaultColumns)
				...
				} else if (isAlterable) try {  //consider: reforming right
					...
					if (reformedColumns.view.map(_.name) == keyOrder)
						...
						if (reform.mayReorderLeft) {
							...
							val permutation = superPermutation(self.mapping.columns, nominalColumns)
...
//		override def labeled[L <: Listing](e :IndexedSQL[E, C, L])(implicit isListing :X =:= L)
...
//	}
	...
	protected override def split(implicit spelling :SQLSpelling) :Seq[GenericColumnComponentSQL.AnyIn[F]] = {
		def columnComponent[T](column :TypedColumn[T, Origin])
				:GenericColumnComponentSQL[F, MappingOf[T]#TypedColumnProjection, T] =
	...
	protected override def mappingForm(implicit spelling :SQLSpelling) :SQLForm[M[Unit]#Subject] =
		spelling.scope.termForm(origin.anchored, anchored)
	...
	protected override def explodedSpelling[P](independent :Boolean)
		...
		origin.`->spellExploded`(this, independent)(from, context, params)
	...
	//todo: we should use extract.toString instead of mapping.mappingName
...
object ComponentSQL {
	...
	private[ast] sealed abstract class Rank1FactoryImplicits {
		implicit def componentFactory[M <: MappingOf[S], S](implicit project :OriginProjection[M, S])
			...
			new Factory[M] {
				override type Result[-F <: RowProduct] = ComponentSQL[F, project.WithOrigin]
				override type TypedResult[F <: RowProduct, T[A] <: BaseMapping[R, A], R, L <: RowProduct] =
					TypedComponentSQL[F, T, R, project.WithOrigin, S, L]
	...
	object Factory extends Rank1FactoryImplicits {
		implicit def columnFactory[M <: BaseColumn[S, _], S]
			...
			new Factory[M] {
				override type Result[-F <: RowProduct] = GenericColumnComponentSQL[F, project.WithOrigin, S]
				override type TypedResult[F <: RowProduct, T[A] <: BaseMapping[R, A], R, L <: RowProduct] =
					TypedColumnComponentSQL[F, T, R, project.WithOrigin, S, L]
...
//		def entity   :Entity[Origin] = origin.mapping
		...
		/** A component is alterable ''iff''
		  * its [[net.noresttherein.oldsql.sql.ast.ComponentSQL.ComponentSQLTemplate.origin origin]] is alterable.
		  * This is not the same as [[net.noresttherein.oldsql.sql.ast.ComponentSQL.ComponentSQLTemplate.isFinal isFinal]]:
		  * a [[net.noresttherein.oldsql.sql.ast.ParamSQL ParamSQL]] is not alterable, but is not necessarily final,
		  * either.
		  */
		override def isAlterable :Boolean = origin.isAlterable
		...
		{
			val d = default
			d.alter(includes.withOrigin[d.Origin], excludes.withOrigin[d.Origin])
		}
...
//			this +- components
		...
		def alter(columns :Unique[TypedColumn[_, Origin]]) :Same = {
			...
			val permutation = superPermutation(mapping.columns, exports)
...
//			exclude(Unique.single(component(mapping)))
		...
		//todo: docs
		def alter(reordering :Rearrangement) :Same =
			if (!isCustom && reordering.isIdentity)
				this
			else if (!reordering.isSurjection) //we can't add null columns here
				throw new IllegalArgumentException(
					"Cannot alter `" + this + "` according to " + reordering +
						" because it would involve adding null columns."
				)
			else
				alter(
					reordering.reorder(mapping.columns.toSeq,
						throw new IllegalArgumentException(
							"Cannot alter `" + this + "` according to " + reordering +
								" because it would involve adding null columns."
						)
		...
		def reorder(precedes :(TypedColumn[_, Origin], TypedColumn[_, Origin]) => Boolean) :Same = //<- not in JoinedRelationTemplate
			reorder(permutation(mapping.columns.toIndexedSeq)(precedes))
...
////		override def asLast :ComponentSQL[FromLast, M] //not implemented here because of linearization issues in RelationSQL subtypes.
	...
	/** Main 'template' mixin trait of [[net.noresttherein.oldsql.sql.ast.ComponentSQL ComponentSQL]]
	  * and [[net.noresttherein.oldsql.sql.ast.GenericColumnComponentSQL GenericColumnComponentSQL]].
	  * Aside from the API inherited from template traits of super types of the former, it provides
	  * methods for transplanting a component to another relation. This trait uses a type constructor
	  * as its 'self' type, which is contravariant in its `RowProduct` type parameter, so this interface
	  * cannot be extended by types other than the mentioned pair. It's role is taken over in a sense
	  * by [[net.noresttherein.oldsql.sql.ast.ComponentSQL.InvariantComponentSQLTemplate InvariantComponentSQLTemplate]],
	  * in that it also declares methods returning components grounded in other clauses.
	  */
	trait GenericComponentSQLTemplate[-F <: RowProduct, M[O] <: MappingAt[O],
	                                 +Same[-f <: RowProduct] <: ComponentSQL[f, M] with ComponentSQLTemplate[f, M, Same[f]]]
		extends LValueSQLTemplate[F, M, M[Unit]#Subject, Same, Same[F]]
		   with ComponentSQLTemplate[F, M, Same[F]]
	{ self :Same[F] with GenericComponentSQLTemplate[F, M, Same] =>

		/* This one cannot just convert to another relation type because type F will be likely incompatible,
		 * and can't convert to the same either, because JoinedGrouping's grouped from clause's type changes.
		 */
		/** Substitutes the [[net.noresttherein.oldsql.sql.ast.ComponentSQL.ComponentSQLTemplate.relation relation]]
		  * of this component for the argument. As the row mapping of the given relation will be a different instance
		  * if `relation` and `this.relation` are different instances, an effort is made to find a counterpart
		...
		@throws[IncompatibleMappingsException]("If the component cannot be anchored because no counterpart " +
		                                       "in the argument's mapping can be found.")
		def anchor(relation :Relation[Entity]) :Same[F]
		...
		/** An expression for the same component, but with the given relation used as its
		  ...
		  */
		@throws[IncompatibleMappingsException](
			"If the component cannot be grafted because no counterpart component in the argument's mapping can be found.")
		def graft[C <: RowProduct](relation :JoinedRelation[C, Entity]) :Same[C]
		...
		def moveTo[C <: RowProduct](offset :RelationOffset[C, Entity] { type First = FromLast }) :Same[C]
//			graft(origin.moveTo(offset))
//
//		/** This method is equivalent to
//		  * `this.`[[net.noresttherein.oldsql.sql.ast.ComponentSQL.TypedComponentSQLTemplate.expand expand]]`[E]`,
...
//		  */
//		def asIn[E <: RowProduct](implicit expansion :Origin PrefixOf E) :Same[E] =
...
//			moveTo(RelationOffset.unsafe[FromLast, FromLast, origin.position.Rel, Entity](0))
	...
	/** An interface trait shared by [[net.noresttherein.oldsql.sql.ast.JoinedRelation JoinedRelation]]
	  * and actual implementations of [[net.noresttherein.oldsql.sql.ast.ComponentSQL ComponentSQL]].
	  * Performs the same function as [[net.noresttherein.oldsql.sql.SQLExpression.GroundingTemplate GroundingTemplate]]
	  * family in other expressions, but due to invariance of `JoinedRelation` being critical,
	  * define all the grounding methods as returning except for
	  * [[net.noresttherein.oldsql.sql.ast.ComponentSQL.InvariantComponentSQLTemplate.asIn asIn]], which is based
	  * on invariant evidence. A most notable exception here is that of
	  * [[net.noresttherein.oldsql.sql.ast.ComponentSQL.GenericComponentSQLTemplate.graft graft]], which changes
	  * [[net.noresttherein.oldsql.sql.ast.LValueSQL.LValueSQLTemplate.FromLast FromLast]] type of the moved component.
	  * @tparam F $F
	  * @tparam M $M
	  * @tparam Same the self type of this instance - the type of component expressions produced by various copy/modify
	  *              methods of this type, typically also extended by any trait or class extending this trait.
	  */
	trait InvariantComponentSQLTemplate[F <: RowProduct, M[A] <: MappingAt[A], +Same[f <: RowProduct] <:
	                                                  ComponentSQL[f, M] with InvariantComponentSQLTemplate[f, M, Same]]
		extends ComponentSQL[F, M]
//		   with InvariantGroundingTemplate[F, Single, M[Unit]#Subject, Same]
	{ self :Same[F] =>
		//We can't implement or even redeclare graft here, because it changes FromLast type and Same does not allow it.
		// Without graft most of these methods can't be implemented, but at least we have a polymorphic interface.
		override type Origin = F

		//changes FromLast in TypedComponentSQL
//		override def alterLike[C <: RowProduct](template :JoinedRelation[C, M]) :Same[F]

		override def anchor(relation :Relation[Entity]) :Same[F]

		override def expand[U <: F, E <: RowProduct]
                   (base :E)(implicit expansion :U ExpandedBy E, isSingle :Single <:< Single) :Same[_ >: E] =
			expand[E]

		override def expand[E <: RowProduct](implicit expansion :F ExpandedBy E) :Same[_ >: E]

		override def basedOn[U <: F, E <: RowProduct](base :E)(implicit expansion :U PartOf E) :Same[_ >: E] =
			expand(base)(expansion.asExpandedBy, implicitly[Single <:< Single])

		override def moveTo[C <: RowProduct](offset :RelationOffset[C, Entity] { type First = self.FromLast }) :Same[C]

		override def asIn[E <: RowProduct](implicit expansion :F PrefixOf E) :Same[E]
	}
//
//	trait InvariantComponentSQL[F <: RowProduct, M[A] <: MappingAt[A]]
//		extends ComponentSQL[F, M]
//		   with InvariantComponentSQLTemplate[F, M, ({ type E[f <: RowProduct] = InvariantComponentSQL[f, M] })#E]



	/** An implementation mixin narrowing down the return type of methods defined in
	  * [[net.noresttherein.oldsql.sql.ast.ComponentSQL.GenericComponentSQLTemplate GenericComponentSQLTemplate]] and
	  * [[net.noresttherein.oldsql.sql.ast.ComponentSQL.InvariantComponentSQLTemplate InvariantComponentSQLTemplate]].
	  * As in the latter, its self type `Same` is invariant in its clause (first) parameter. It contains implementations
	  * of a great bulk of inherited methods, taking advantage of the fact of stronger typing than on the level
	  * of `ComponentSQL` and its direct mixins, in particular the constraint that
	  * `this.`[[net.noresttherein.oldsql.sql.ast.ComponentSQL.TypedComponentSQLTemplate.FromLast FromLast]]`=:=this.[[net.noresttherein.oldsql.sql.ast.ComponentSQL.ComponentSQLTemplate.origin origin]]`.FromLast`.
	  * They in general assume `origin != this`, so most of them is overridden in
	  * [[net.noresttherein.oldsql.sql.ast.RelationSQL RelationSQL]],
	  * but it still provides an interface allowing for polymorphism between proper components and relations.
	  * @tparam F    $F
	  * @tparam M    $M
	  * @tparam V    $V
	  * @tparam L    $L
	  * @tparam Same self type of this component, accepting its `RowProduct` and `FromLast` type parameters (`F` and `L`
	  *              in this trait).
	  * @define V    the [[net.noresttherein.oldsql.schema.Mapping.Subject Subject]] type of the component's mapping
	  *              and the value type of this expression.
	  * @define L    [[net.noresttherein.oldsql.sql.ast.LValueSQL.LValueSQLTemplate.FromLast FromLast]] type of this component.
	  * @define Coll `TypedComponentSQLTemplate`
	  */
	trait TypedComponentSQLTemplate[F <: RowProduct, M[A] <: BaseMapping[V, A], V, L <: RowProduct,
	                                    +Same[f <: RowProduct, l <: RowProduct] <:
	                                          ComponentSQL[f, M] with TypedComponentSQLTemplate[f, M, V, l, Same]]
		extends ComponentSQL[F, M]
		   with InvariantComponentSQLTemplate[F, M, ({ type E[f <: RowProduct] = Same[f, L] })#E]
	{ self :Same[F, L] =>
		...
		override def default :Same[F, L] =
			if (isDefault) this else graft(origin.default)

		override def defaultWith(includes :Unique[TypedMapping[_, F]], excludes :Unique[TypedMapping[_, F]]) :Same[F, L] = {
		...
		@throws[IllegalArgumentException]("if any of the given components is not a component of this mapping.")
		override def include(components :Iterable[TypedMapping[_, F]]) :Same[F, L] = {
		...
		@throws[IllegalArgumentException]("if any of the given components is not a component of this mapping.")
		@throws[UnsupportedOperationException]("if this component is a column and the exclude list is non empty.")
		override def exclude(components :Iterable[TypedMapping[_, F]]) :Same[F, L] = {
			validateExcludes(components)
			graft(origin.exclude(components))
		}

		override def alter(includes :Iterable[TypedMapping[_, F]], excludes :Iterable[TypedMapping[_, F]]) :Same[F, L] = {
			validateComponents(includes)
			validateExcludes(excludes)
			graft(origin.alter(includes, excludes))
		}

		//Copy&paste from ComponentSQLTemplate, could be extracted to another interface if needed.
		//We can't move it to MappingSQLTemplate because LooseComponent can't handle it.
		override def alter(columns :Unique[TypedColumn[_, Origin]]) :Same[F, L] = {
			val exports = columns.map(mapping.export(_))
			val permutation = superPermutation(mapping.columns, exports)
			defaultWith(exports, mapping.columns.filterNot(exports.contains)).reorder(permutation)
		}

		override def alterLike[C <: RowProduct](template :JoinedRelation[C, M]) :Same[F, L] = {
			type Rel[f <: RowProduct] = RelationSQL[f, Entity, EntitySubject, L]
		...
		override def alter(reordering :Rearrangement) :Same[F, L] =
			if (!isCustom && reordering.isIdentity)
				this
			else if (!reordering.isSurjection) //we can't add null columns here
				throw new IllegalArgumentException(
		...
		override def reorder(permutation :IndexedSeq[Int]) :Same[F, L] =
			if (!isCustom && permutation == permutation.indices) //!isCustom checks if we are not already reordered
				...
				val newOrigin = origin.custom { (relation, originCounterpart) =>
					if (relation == originCounterpart && relation == mapping)//we are a RelationSQL homomorphic to relation
		...
		override def makeCustom :Same[F, L] = graft(origin.makeCustom)

		protected override def makeFinal :Same[F, L] = graft(origin.`->makeFinal`)

		override def aliased(aliases :Map[TypedColumn[_, F], String]) :Same[F, L] =
			graft(origin.aliased(aliases))

		override def anchor(from :F) :Same[F, L] = graft(origin.anchor(from))

		override def anchor(relation :Relation[Entity]) :Same[F, L] = graft(origin.anchor(relation))

		override def expand[U <: F, E <: RowProduct]
                   (base :E)(implicit expansion :U ExpandedBy E, isSingle :Single <:< Single) :Same[_ >: E, L] =
			{ val res = graft(origin.expand(base).origin); res } //type inference without an expected type

		override def expand[E <: RowProduct](implicit expansion :F ExpandedBy E) :Same[_ >: E, L] =
			{ val res = graft(origin.expand[E](expansion).origin); res }

		override def basedOn[U <: F, E <: RowProduct](base :E)(implicit expansion :U PartOf E) :Same[_ >: E, L] =
			expand(base)(expansion.asExpandedBy, implicitly[Single <:< Single])

		/* In order to have these methods defined here, rather than TypedComponentSQL, we would need origin :RelationSQL
		 * in order to know that this.FromLast =:= origin.FromLast. It would prevent us however from extending
		 * this trait from JoinedRelationTemplate, which has origin :Rel[F]. It's an either/or situation,
		 * and moving them down results in lesser repetition.
		 */
		override def graft[C <: RowProduct](relation :JoinedRelation[C, Entity]) :Same[C, relation.FromLast]

		override def moveTo[C <: RowProduct](offset :RelationOffset[C, Entity] { type First = FromLast }) :Same[C, L] =
			graft(origin.moveTo(offset))
//		/** Converts this $this to an expression based on clause `E[F]`, which expands `F` by a single relation. */
//		def asIn[E[+L <: F] <: L Expanded M forSome { type M[O] <: MappingAt[O]}] :Cons[E[F]] =
//			asIn[E[F]](PrefixOf.expand)
//
		override def asIn[E <: RowProduct](implicit expansion :F PrefixOf E) :Same[E, L] =
			moveTo(origin.position + expansion)

		override def asLast :Same[FromLast, FromLast] =
			moveTo(RelationOffset.unsafe[FromLast, FromLast, origin.position.Rel, Entity](0))
...
//			override type isSelectable = true
	...
	trait TypedComponentSQL[F <: RowProduct, T[A] <: BaseMapping[R, A], R,
		...
		extends ComponentSQL[F, M]
		   ...
		   with TypedComponentSQLTemplate[F, M, V, L, ({ type E[f <: RowProduct, l <: RowProduct] = TypedComponentSQL[f, T, R, M, V, l] })#E]
	{ self =>
...
//				:MappingSQL[E, Single, M, V] =
			...
			if (substitutes.isEmpty)
				...
				EditedComponentSQL[F, E, T, R, M, V, included.FromLast](included)(setters)
			}
...
//				:TypedComponentSQL[P, T, R, M, V, L] =
//			graft(origin.moveTo(offset))
//
//		override def asIn[E <: RowProduct](implicit expansion :F PrefixOf E) :TypedComponentSQL[E, T, R, M, V, L] =
//			moveTo(origin.position + expansion)
//
//		override def asLast :TypedComponentSQL[FromLast, T, R, M, V, FromLast] =
//			moveTo(RelationOffset.unsafe[FromLast, FromLast, origin.position.Rel, Entity](0))
...
//			SelectSQL.subselect[B, from.type, M, V](from, this)
	...
	object TypedComponentSQL {
		...
		{ self =>
			...
			protected override lazy val form :SQLForm[S] =
				selectForm <> origin.anchored.writeForm(anchored, origin.anchored.selectedByDefault(anchored))
...
trait GenericColumnComponentSQL[-F <: RowProduct, M[A] <: BaseColumn[V, A], V]
	extends ComponentSQL[F, M]
	   with ColumnReformingDefaults[F, Single, V, ({ type E[v] = ColumnLValueSQL[F, M, v] })#E]
	   with ColumnLValueSQL[F, M, V] with SelectableColumnMappingSQL[F, M, V]
	   with VariantColumnGroundingTemplate[F, Single, V,
	                                       ({ type E[-f <: RowProduct] = GenericColumnComponentSQL[f, M, V] })#E]
	   with GenericComponentSQLTemplate[F, M, ({ type E[-f <: RowProduct] = GenericColumnComponentSQL[f, M, V] })#E]
//	   with SQLReformerOverride[F, Single, V, ({ type E[v] = ColumnLValueSQL[F, M, v] })#E]
{ self =>
	override type Subject = V
	override def component  :GenericColumnComponentSQL[F, M, V] = this
//	override def underlying :GenericColumnComponentSQL[F, M, V] = this
...
//
//	protected override def realign(reordering :Rearrangement)
//	                              (implicit spelling :SQLSpelling) :GenericColumnComponentSQL[F, M, V] =
//	{
//		super[ColumnLValueSQL].realign(reordering) //for validation
//		this
//	}
	...
	protected override def reform[F2 <: RowProduct, S2 >: Grouped <: Single, V2, EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
		...
		if (this eq other)
			...
			origin.`->reformComponent`[M, V, F, F2, S2, V2, EC2, U](this, other)(reform, passCount)

	protected override def potentialColumns(permissions :Permissions)(implicit spelling :SQLSpelling) :AlignableColumns =
		AlignableColumns(this, if (isFinal) NoReform else permissions)
...
object GenericColumnComponentSQL {
	...
	trait TypedColumnComponentSQL[F <: RowProduct, T[A] <: BaseMapping[R, A], R,
		...
		extends TypedComponentSQL[F, T, R, M, V, L]
           with GenericColumnComponentSQL[F, M, V]
//		   with InvariantColumnGroundingTemplate
//                [F, Single, V, ({ type E[f <: RowProduct] = TypedColumnComponentSQL[f, T, R, M, V, L] })#E]
		   with TypedComponentSQLTemplate
                [F, M, V, L, ({ type E[f <: RowProduct, l <: RowProduct] = TypedColumnComponentSQL[f, T, R, M, V, l] })#E]
	{ self =>
...
//			graft(origin.aliased(aliases))
//
		override def alter(reordering :Rearrangement) :TypedColumnComponentSQL[F, T, R, M, V, L] =
			if (reordering.columnCount == 1 && reordering.isIdentity)
				this
			else
				throw new IllegalArgumentException("Cannot reorder a single column according to " + reordering + ".")
				...
				:MappingSQL[E, Single, M, V] { type Origin = F } =
		{
			...
			if (substitutes.isEmpty)
				...
				if (origin.export.export(substitutes.head.lvalue.mapping.refine.withOrigin[Origin]) == export)
					EditedComponentSQL[F, E, T, R, M, V, L](this)(substitutes.toSeq)
				else
					reject()
				...
				EditedComponentSQL[F, E, T, R, M, V, L](this)(last :: Nil)
			} else
				reject()
		}
...
//			graft(origin.anchor(relation))
		...
		override def graft[P <: RowProduct](relation :JoinedRelation[P, T])
				:TypedColumnComponentSQL[P, T, R, M, V, relation.FromLast] =
			if (relation eq origin)
				this.castFrom[
					TypedColumnComponentSQL[F, T, R, M, V, L], TypedColumnComponentSQL[P, T, R, M, V, relation.FromLast]
				]
			else if (relation.mapping == origin.mapping) //can safely use this.mapping
...
//				:TypedColumnComponentSQL[E, T, R, M, V, L] =
//			graft(origin.moveTo(offset))
//
//		override def asIn[E <: RowProduct](implicit expansion :F PrefixOf E) :TypedColumnComponentSQL[E, T, R, M, V, L] =
//			moveTo(origin.position + expansion)
//
//		override def asLast :TypedColumnComponentSQL[FromLast, T, R, M, V, FromLast] =
//			moveTo(RelationOffset.unsafe[FromLast, FromLast, origin.position.Rel, Entity](0))
...
//
		protected override def realign(reordering :Rearrangement)
		                              (implicit spelling :SQLSpelling) :TypedColumnComponentSQL[F, T, R, M, V, L] =
		{
			super[GenericColumnComponentSQL].realign(reordering) //validate
			this
		}
