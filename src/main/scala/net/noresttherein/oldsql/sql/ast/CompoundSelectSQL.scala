...
import net.noresttherein.oldsql.sql.{ColumnSQL, CompoundSelect, RowProduct, RowShape, SQLExpression, StandardSQL}
...
import net.noresttherein.oldsql.sql.RowProduct.ExpandedBy
...
import net.noresttherein.oldsql.sql.SQLExpression.{AnyExpressionVisitor, ConvertibleSQL, Grouped, Single, SingleRowSQLTemplate, SpecificExpressionVisitor, SQLShape}
...
trait CompoundSelectSQL[-F <: RowProduct, R]
	extends CompositeSQL[F, Single, Rows[R]] with QuerySQL[F, R]
	   with SingleRowSQLTemplate[F, Rows[R], ({ type Q[-f <: RowProduct] = CompoundSelectSQL[f, R] })#Q]
	   with QueryReformingTemplate[R, ({ type Q[X] = CompoundSelectSQL[F, X] })#Q]
	   with CompoundSelectTemplate[R, ({ type Q[X] = QuerySQL[F, X] })#Q]
{
//consider: a Shape type for SQLExpression: column/component/tuple/named tuple. This would allow having some kind of a select clause
	...
	override def parts             :Seq[QuerySQL[F, R]] = left::right::Nil
	override def selectForm        :SQLReadForm[Rows[R]] = left.selectForm
...
/*
	...
	protected override def reform_:[E <: F](first :SingleQuerySQL[E, R])(reform :QueryReform)
	                                       (implicit spelling :SQLSpelling) :(QuerySQL[E, R], QuerySQL[E, R]) =
	{
		val (firstWithLeft, leftWithFirst)   = reform(first :QuerySQL[E, R], left)
		val (firstWithRight, rightWithFirst) = reform(firstWithLeft :QuerySQL[E, R], right)
...
//			val self = reform.queryReform(this).apply(this)
	...
	protected override def reformed(reform :QueryReform)(implicit spelling :SQLSpelling) :CompoundSelectSQL[F, R] = {
	...
	@volatile @transient private[this] var _reformed        :CompoundSelectSQL[F, R] = _
	                                         ...
	                                         (implicit spelling :SQLSpelling) :SpelledSQL[P] =
	{
		...
		val CompoundSelectSQL(left, _, right) = spelling.queryReform(this)
	...
	def apply[F <: RowProduct, V]
		...
		(left, right, selectClause) match {
			case (l :ColumnQuery[F, V], r :ColumnQuery[F, V], s :ColumnSQL[_, _, V]) =>
				CompoundSelectColumn(l, operator, r, s)
			case _ =>
...
//		apply[F, V](left, operator, right, CompoundSelectIdSQL.selectClause(_))(reform)
	...
	private[sql] def reformed[F <: RowProduct, V]
		...
		(left, right, selectClause) match {
			...
			case _ =>
				val res = new ReformedCompoundSelectSQL[F, V](left, operator, right, selectClause)
				res.wasReformedBy((reform, spelling))
				res
		}

	private[sql] def reformed[F <: RowProduct, V](left :QuerySQL[F, V], operator :SelectOperator, right :QuerySQL[F, V],
	                                              selectClause :CompoundSelectSQL[F, V] => SQLShape[V])
	                                             (reform :QueryReform)(implicit spelling :SQLSpelling)
			:CompoundSelectSQL[F, V] =
	{
		val init = selectClause
		val res = new Impl[F, V](left, operator, right) {
...
*/
	...
	@inline def unapply[F <: RowProduct, V](e :SQLExpression[F, Grouped, Rows[V]])
			:Opt[(QuerySQL[F, V], SelectOperator, QuerySQL[F, V])] =
		e match {
			case op :CompoundSelectSQL[F @unchecked, V @unchecked] => Got((op.left, op.operator, op.right))
			case _ => Lack
		}

	private class Impl[-F <: RowProduct, V](override val left :QuerySQL[F, V], override val operator :SelectOperator,
	                                        override val right :QuerySQL[F, V])
		extends CompoundSelectSQL[F, V] with RowShapeCache
	{
		...
		override def selectForm = rowForm.map(Rows.single)

		override lazy val constituents = super.constituents
	}

	private class ReformedCompoundSelectSQL[-F <: RowProduct, V]
	              (l :QuerySQL[F, V], op :SelectOperator, r :QuerySQL[F, V], override val selectClause :SQLShape[V])
		extends Impl[F, V](l, op, r)
		   with ReformedCompoundSelectTemplate[V, ({ type Q[X] = QuerySQL[F, X] })#Q] with RowShapeCache
...
trait CompoundSelectColumn[-F <: RowProduct, R]
	extends CompositeColumnSQL[F, Single, Rows[R]] with CompoundSelectSQL[F, R] with ColumnQuery[F, R]
	   with SingleRowSQLTemplate[F, Rows[R], ({ type Q[-f <: RowProduct] = CompoundSelectColumn[f, R] })#Q]
	   with CompoundSelectTemplate[R, ({ type Q[X] = ColumnQuery[F, X] })#Q]
{
	override def selectForm        :ColumnReadForm[Rows[R]] = left.selectForm
	override def rowForm           :ColumnReadForm[R]       = left.rowForm
...
//				new StandardCompoundSelectColumn(left, operator, right, selectClause)
	...
	private class ReformedCompoundSelectColumn[-F <: RowProduct, V]
	                                          (l :ColumnQuery[F, V], op :SelectOperator, r :ColumnQuery[F, V],
	                                           override val selectClause :ColumnSQL[Nothing, Grouped, V])
		extends Impl(l, op, r)
	{
...
/** Implements a compound select combining the result sets of two
  ...
  * of the [[net.noresttherein.oldsql.sql.ast.CompoundSelectSQL.operator operator]] member property.
  */
trait CompoundSelectAs[-F <: RowProduct, M[O] <: MappingAt[O]]
...
//	override def constituents :Seq[SelectAs[F, M]] = left.constituents ++: right.constituents
	...
	override def anchor(from :F) :CompoundSelectAs[F, M] = {
		val l = left.anchor(from); val r = right.anchor(from)
		if ((l eq left) && (r eq right)) this else CompoundSelectAs(l, operator, r)
	}
//
...
object CompoundSelectAs {
	...
	def reformed[F <: RowProduct, M[O] <: MappingAt[O]]
	            (left :MappingQuerySQL[F, M], operator :SelectOperator, right :MappingQuerySQL[F, M],
	...
	private class Impl[-F <: RowProduct, M[O] <: MappingAt[O]]
	                  (override val left :MappingQuerySQL[F, M], override val operator :SelectOperator,
	                   override val right :MappingQuerySQL[F, M], override val rowForm :SQLReadForm[M[Unit]#Subject])
...
trait CompoundSelectColumnAs[-F <: RowProduct, M[O] <: BaseColumn[R, O], R]
	extends CompoundSelectColumn[F, R] with ColumnMappingQuery[F, M, R] with CompoundSelectAs[F, M]
...
//	override def constituents :Seq[SelectColumnAs[F, M, V]] = left.constituents ++: right.constituents
	...
	override def anchor(from :F) :CompoundSelectColumnAs[F, M, R] = {
		val l = left.anchor(from); val r = right.anchor(from)
		if ((l eq left) && (r eq right)) this else CompoundSelectColumnAs(l, operator, r)
	}
...
object CompoundSelectColumnAs {
	...
	def reformed[F <: RowProduct, M[O] <: BaseColumn[V, O], V]
	            (left :ColumnMappingQuery[F, M, V], operator :SelectOperator, right :ColumnMappingQuery[F, M, V],
	...
	private class Impl[-F <: RowProduct, M[O] <: BaseColumn[V, O], V]
	                  (override val left :ColumnMappingQuery[F, M, V], override val operator :SelectOperator,
	                   override val right :ColumnMappingQuery[F, M, V], override val rowForm :ColumnReadForm[V])
		extends CompoundSelectColumnAs[F, M, V] with RowShapeCache
	{
		override def selectForm = rowForm.map(Rows.single)
	}
