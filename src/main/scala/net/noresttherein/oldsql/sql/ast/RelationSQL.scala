...
import net.noresttherein.oldsql.exceptions.IncompatibleMappingsException
import net.noresttherein.oldsql.morsels.InferTypeParams
import net.noresttherein.oldsql.pixies.Rearrangement
import net.noresttherein.oldsql.schema.{ColumnForm, ColumnReadForm, MappingExtract, Relation, SQLForm, SQLReadForm, SQLWriteForm, Table}
...
import net.noresttherein.oldsql.sql.{AndBy, AndByOne, AndByVal, AndFrom, ByParam, ColumnSQL, ColumnSetter, ComponentSetter, Expanded, FromSome, GroundColumn, GroundSQL, GroupBy, GroupByClause, RowProduct, SQLExpression, Seal, Select, SingleColumn, SingleSQL, WithClause, WithParam}
...
import net.noresttherein.oldsql.sql.ParamClause.{ParamColumnExtract, ParamExtract, ParamMapping, ParamRelation, UnboundParam}
import net.noresttherein.oldsql.sql.RowProduct.{ExpandedBy, PrefixOf}
...
import net.noresttherein.oldsql.sql.ast.ComponentSQL.{ComponentSQLTemplate, InvariantComponentSQLTemplate, TypedComponentSQL, TypedComponentSQLTemplate}
...
import net.noresttherein.oldsql.sql.ast.MappingSQL.{RearrangedMappingSQL, UnalignedMappingSQL}
...
import net.noresttherein.oldsql.sql.ast.SQLTerm.SQLTermTemplate
import net.noresttherein.oldsql.sql.ast.TableSQL.{AnyTableVisitor, CustomTableSQL, SpecificTableVisitor}
import net.noresttherein.oldsql.sql.mechanics.{AlignableColumns, Reform, ReformPermissions, RelationOffset, SQLConversion, SQLTransformation, SpelledSQL}
...
import net.noresttherein.oldsql.sql.mechanics.ReformPermissions.Permissions.{MayAddNull, MayExclude, MayInclude, NoReform}
import net.noresttherein.oldsql.sql.mechanics.ReformPermissions.Permissions
import net.noresttherein.oldsql.sql.mechanics.SQLTransformation.{Composition, SQLDecoration}
...
//
	...
	protected def reformComponent[M[A] <: MappingAt[A], F1 <: F, F2 <: RowProduct, S2 >: Grouped <: Single, V2,
	                              EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
	                             (component :ComponentSQL[F1, M] { type Origin = F },
	                              other :ConvertibleSQL[F2, S2, V2, EC2])
	                             (reform :Reform, passCount :PassCount)
	                             (implicit leftResult  :SQLTransformation[component.Subject, U],
	                                       rightResult :SQLTransformation[V2, U], spelling :SQLSpelling)
			:(leftResult.Expression[F1, Single, LValueSQL[F1, M, U]], rightResult.Expression[F2, S2, EC2[U]]) =
		/* I don't like two things about this:
		 *   1. Direct call to component.reformer: breaks its encapsulation and is in general out of reforming order
		 *   1. The fact that we rely on package protected method here show that having them protected is too limited
		 *      for some valid use cases.
		 */
		if (component.origin != this)
			throw new IllegalArgumentException(
				"Cannot reform component `" + component + "` because it does not belong to relation `" + this + "`."
			)
		else if (component eq other)
			(leftResult(component), rightResult(other))
		else if (passCount.firstTime)
			other.`->reform`(component)(reform.swap, passCount.++).swap
		else
			component.`->reformer`(other)(reform, passCount).apply(other)

	protected def reformComponent[M[A] <: BaseColumn[X, A], X, F1 <: F, F2 <: RowProduct, S2 >: Grouped <: Single, V2,
	                              EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
	                             (component :GenericColumnComponentSQL[F1, M, X] { type Origin = F },
	                              other :ConvertibleSQL[F2, S2, V2, EC2])
	                             (reform :Reform, passCount :PassCount)
	                             (implicit leftResult  :SQLTransformation[X, U],
	                                       rightResult :SQLTransformation[V2, U], spelling :SQLSpelling)
			:(leftResult.Expression[F1, Single, ColumnLValueSQL[F1, M, U]], rightResult.Expression[F2, S2, EC2[U]]) =
		//same thoughts about it as of the ComponentSQL variant
		if (component.origin != this)
			throw new IllegalArgumentException(
				"Cannot reform component `" + component + "` because it does not belong to relation `" + this + "`."
			)
			...
			other match {
				case _ :ColumnSQL[F2, S2, V2] =>
					(leftResult(component), rightResult(other))
				case _ =>
					val right = component.`->reformer`(other)(reform.prohibitReformLeft, passCount).apply(other)._2
					(leftResult(component), right)
			}

	private[sql] def `->reformComponent`[M[A] <: MappingAt[A], F1 <: F, F2 <: RowProduct, S2 >: Grouped <: Single, V2,
	                                     EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
	                                    (component :ComponentSQL[F1, M] { type Origin = F },
	                                     other :ConvertibleSQL[F2, S2, V2, EC2])
	                                    (reform :Reform, passCount :PassCount)
	                                    (implicit leftResult  :SQLTransformation[M[Unit]#Subject, U],
	                                              rightResult :SQLTransformation[V2, U], spelling :SQLSpelling)
			:(leftResult.Expression[F1, Single, LValueSQL[F1, M, U]], rightResult.Expression[F2, S2, EC2[U]]) =
		reformComponent(component, other)(reform, passCount)

	private[sql] def `->reformComponent`[M[A] <: BaseColumn[X, A], X, F1 <: F, F2 <: RowProduct,
	                                     S2 >: Grouped <: Single, V2, EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
	                                    (component :GenericColumnComponentSQL[F1, M, X] { type Origin = F },
	                                     other :ConvertibleSQL[F2, S2, V2, EC2])
	                                    (reform :Reform, passCount :PassCount)
	                                    (implicit leftResult  :SQLTransformation[X, U],
	...
	protected def realign[M[A] <: MappingAt[A], F1 <: F]
	                     (component :ComponentSQL[F1, M] { type Origin = F }, reordering :Rearrangement)
	                     (implicit spelling :SQLSpelling)
			:MappingSQL[F1, Single, M, M[Unit]#Subject] =
		if (component.origin != this)
			throw new IllegalArgumentException(
				"Cannot reorder `" + component + "` with " + reordering +
				" because it comes from relation `" + component.origin + "` other than `" + this + "`."
			)
		else if (reordering.isIdentity)
			component
		else if (reordering.isSurjection) {
			val applicable = component.mapping.columns.filterNot { col =>
				spelling.scope.isProhibited(anchored.export(col))
			}
			if (reordering.columnCount != applicable.size)
				throw new IllegalArgumentException(
					"Cannot reorder `" + component + "` with " + reordering + " because the number " + applicable.size +
						" of applicable columns in " + spelling.scope + " differs from the reordered columnCount " +
						reordering.columnCount + "."
				)
			val reordered = Unique.tabulate(reordering.underlyingColumnCount) {
				i => applicable(reordering.inverse(i + 1) - 1)
			}
			component.alter(reordered)
		} else
			new RearrangedMappingSQL(component, reordering)

	private[sql] def `->realign`[M[A] <: MappingAt[A], F1 <: F]
	                            (component :ComponentSQL[F1, M] { type Origin = F }, reordering :Rearrangement)
	                            (implicit spelling :SQLSpelling)
			:MappingSQL[F1, Single, M, M[Unit]#Subject] =
		realign(component, reordering)

	protected def potentialColumnsOf[F1 <: F, M[A] <: MappingAt[A]]
	                                (component :ComponentSQL[F1, M] { type Origin = F }, permissions :Permissions)
	                                (implicit spelling :SQLSpelling) :AlignableColumns =
	{
		if (component.origin != this)
			throw new IllegalArgumentException(
				"Cannot list available columns for component `" + component + "` because it does not belong to `" +
					this + "`."
			)
		val effectivePermissions = //should we put these restrictions here, or in AlignableColumns, or perhaps reform?
			if (isFinal) NoReform
			else if (isCustom) permissions & MayAddNull
			else if (!isDefault) permissions - MayInclude - MayExclude
			else permissions
		AlignableColumns(component, effectivePermissions)
	}

	private[sql] def `->potentialColumnsOf`[F1 <: F, M[A] <: MappingAt[A]]
	                                       (component :ComponentSQL[F1, M] { type Origin = F }, permissions :Permissions)
	                                       (implicit spelling :SQLSpelling) :AlignableColumns =
		potentialColumnsOf(component, permissions)

	protected def potentialColumnsCountOf[F1 <: F, M[A] <: MappingAt[A]]
	                                     (component :ComponentSQL[F1, M] { type Origin = F })
	                                     (implicit spelling :SQLSpelling) :Int =
		component.mapping.columns.size

	private[sql] def `->potentialColumnsCountOf`[F1 <: F, M[A] <: MappingAt[A]]
	                                            (component :ComponentSQL[F1, M] { type Origin = F })
	                                            (implicit spelling :SQLSpelling) :Int =
		potentialColumnsCountOf(component)
	...
	private[sql] final def `->spell`[P, C <: F, M[O] <: MappingAt[O]]
	                                (component :ComponentSQL[C, M] { type Origin = F; type Entity[O] = T[O] },
	...
	private[sql] final def `->spellExploded`[P, C <: F, M[O] <: MappingAt[O]]
	                                        (component :ComponentSQL[C, M] { type Origin = F; type Entity[O] = T[O] },
	                                         independent :Boolean)
	                                        (from :C, context :SQLContext[P], params :Parameterization[P, C])
	                                        (implicit spelling :SQLSpelling) :Seq[SpelledSQL[P]] =
...
object JoinedRelation {
	...
	trait JoinedRelationTemplate[F <: RowProduct, T[O] <: MappingAt[O],
		...
		extends InvariantComponentSQLTemplate[F, T, ({ type E[f <: RowProduct] = JoinedRelation[f, T] })#E]
//		extends InvariantComponentSQLTemplate[F, T, Rel]
		   with ComponentSQLTemplate[F, T, Rel[F]]
	{ self :Rel[F] with JoinedRelationTemplate[F, T, Rel] =>
		...
		override def isAlterable :Boolean = !isFinal
		...
		//We could implement it directly like reorder, but for now the implementation from GenericComponentSQLTemplate
		// should be enough. We can't move it to MappingSQLTemplate because LooseComponent can't handle it.
		//This override is necessary because otherwise relations would inherit implementation
		//from TypedComponentSQLTemplate, which has narrower return types (in TableSQL and the rest).
		override def alter(columns :Unique[TypedColumn[_, Origin]]) :Rel[F] =
			super[ComponentSQLTemplate].alter(columns)

		override def alter(reordering :Rearrangement) :Rel[F] =
			super[ComponentSQLTemplate].alter(reordering)
			...
			if (!isCustom && permutation == permutation.indices)
				...
				custom { (relation, counterpart) =>
					if (relation == counterpart) //this will also cover relation being homomorphic with mapping
						...
						// is relative to the *nominal* mapping.
...
//							counterpart == mapping also checks for relation == mapping, which is the important case here
							if (counterpart == mapping || relation.contains(mapping.refine.withOrigin[Unit]))
								mapping.refine.castParam1[Subject].withOrigin[Unit]
							else
		...
		//Cannot return Rel[F] because 1) JoinedGrouping first type param is not preserved,
		// and 2) the result type may change between at least TableSQL and ParamSQL
//		override def anchor(relation :Relation[T]) :Rel[F]
		...
		override def moveTo[E <: RowProduct](offset :RelationOffset[E, T] { type First = FromLast }) :Rel[E] =
...
//			asIn[J[F, M]](PrefixOf.expand)
		...
		override def asIn[E <: RowProduct](implicit expansion :F PrefixOf E) :Rel[E] =
			copy(position + expansion, includes.withOrigin[E], excludes.withOrigin[E])
...
//				)
		...
		private[sql] def `->copy`[E <: RowProduct](index :Int,
		                                           includes :Unique[TypedMapping[_, F]] = this.includes,
		                                           excludes :Unique[TypedMapping[_, F]] = this.excludes) :Rel[E] =
			copy(
				RelationOffset.unsafe[E, FromLast, position.Rel, T](index),
				includes.withOrigin[E], excludes.withOrigin[E]
			)
	...
	extends JoinedRelation[F, T] with JoinedRelationTemplate[F, T, ({ type E[f <: RowProduct] = JoinedTable[f, T] })#E]
{
	...
	def anchor(table :Table[T]) :JoinedTable[F, T]
	   ...
	   with JoinedRelationTemplate[F, T, ({ type E[f <: RowProduct] = JoinedGrouping[U, f, T] })#E]
{
	...
	def anchor[C <: U](grouping :GroupingRelation[C, T]) :JoinedGrouping[C, F, T]
	...
	extends JoinedRelation[F, T] with JoinedRelationTemplate[F, T, ({ type E[f <: RowProduct] = JoinedParam[f, T] })#E]
{
	...
	def anchor(param :ParamRelation[T[Unit]#Subject]) :JoinedParam[F, ParamRelation[T[Unit]#Subject]#Param]
...
//	   with LValueTemplate[F, T, V, ({ type E[f <: RowProduct] = RelationSQL[f, T, V, L]})#E, RelationSQL[F, T, V, L]]
	   ...
	   with TypedComponentSQLTemplate[F, T, V, L, ({ type R[f <: RowProduct, l <: RowProduct] = RelationSQL[f, T, V, l] })#R]
{ self =>
//	override type Entity[O] = T[O]
//	override type EntitySubject = V
...
//		RelationSQL(relation, position) //not ideal, as it will not support non standard relations
	...
	override def expand[U <: F, E <: RowProduct]
		...
		base.relations(position + expansion.superPrefix[F]).typed

	override def expand[E <: RowProduct]
	                    (implicit expansion :F ExpandedBy E) :RelationSQL[_ >: E <: RowProduct, T, V, L] =
		asIn(expansion.superPrefix)

//	override def moveTo[E <: RowProduct](offset :RelationOffset[E, T] { type First = FromLast }) :RelationSQL[E, T, V, L] =
//		copy(offset, includes.withOrigin[E], excludes.withOrigin[E])
...
object RelationSQL {
	...
	{
		type Res = RelationSQL[F, T, V, F] forSome { type F <: RowProduct }
		relation match {
			case table :Table[T] =>
				TableSQL[AndFrom.LUB[T], T, V](table, 0) :Res
			case grouping :GroupingRelation[f, T] =>
				GroupingSQL[f, AndBy.Last[T], T, V](grouping, 0) :Res
			case param :ParamRelation[v] =>
				ParamSQL[WithParam.Last[v], v, WithParam.Last[v]](param, 0)
					.asInstanceOf[RelationSQL[RowProduct, T, V, RowProduct]] :Res
			case _ =>
				throw new IllegalArgumentException(
...
*/
		...
		extends RelationSQL[F, T, S, L]
	{
		...
		override def anchor(relation :Relation[T]) :RelationSQL[F, T, S, L] =
...
{
	override def form :SQLForm[V] = anchored.selectForm <> anchored.writeForm(anchored.selectedByDefault)
	...
	override def anchor(table :Table[T]) :TableSQL[F, T, V] =
		if (table == relation)
			this
		else if (table.row isomorphic mapping)
			TableSQL(
			...
			throw new IllegalArgumentException(
				"Cannot recreate a TableSQL " + fullString + " for table " + table + " because its mapping " +
					table.row + "is not isomorphic with this table's mapping " + mapping + "."
			)

	override def anchor(relation :Relation[T]) :TableSQL[F, T, V] = relation match {
		case table :Table[T] => anchor(table)
		case _ =>
			throw new IllegalArgumentException(
				"Cannot recreate a TableSQL " + fullString + " for relation " + relation + " :" + relation.className + "."
			)
	}
	...
	override def asGrouping[M[A] <: BaseMapping[S, A], S]
	                       (component :TypedComponentSQL[F, T, V, M, S, FromLast]) :GroupingRelation[F, M] =
		GroupingRelation[F, M, S](component)(component.projection.isomorphism)
...
object TableSQL {
		   ...
		   with CustomRelationSQL[F, T, S, RowProduct AndFrom T]
	{
		...
		override def anchor(table :Table[T]) :TableSQL[F, T, S] =
			if (relation == table) this
			else underlying.anchor(table).custom(customize)
		...
		{
			val moved = underlying.copy(position, underlying.includes.withOrigin[E], underlying.excludes.withOrigin[E])
			if (includes.nonEmpty || excludes.nonEmpty)
				new CustomTableSQL(moved, customize, includes, excludes)
			else
				new CustomTableSQL[E, T, S](moved, customize) {
...
{
	override def form :SQLForm[V] = relation.expr.selectForm <> SQLWriteForm.nulls(relation.expr.selectForm.columnCount)
	...
	override def makeCustom :GroupingSQL[U, F, T, V] =
		if (isCustom)
			...
			class UnalterableGroupingSQL[-u <: RowProduct, f <: RowProduct, t[A] <: BaseMapping[v, A], v]
                 (override val relation :GroupingRelation[u, t],
                  override val position :RelationOffset[f, t] { type First = RowProduct AndBy t },
                  override val includes :Unique[TypedMapping[_, f]] = Unique.empty[TypedMapping[_, f]],
                  override val excludes :Unique[TypedMapping[_, f]] = Unique.empty[TypedMapping[_, f]])
				extends GroupingSQL[u, f, t, v](relation, position, includes, excludes)
				   with CustomRelationMixin[f, t, ({ type R[ff <: RowProduct] = GroupingSQL[u, ff, t, v] })#R]
			new UnalterableGroupingSQL[U, F, T, V](relation, position, includes, excludes)
		}
	...
	override def anchor[C <: U](grouping :GroupingRelation[C, T]) :GroupingSQL[C, F, T, V] =
		if (grouping == relation)
			this
		else if (grouping.row isomorphic mapping)
			GroupingSQL[C, F, T, V](
				grouping, index,
				includes.map(relation.row[F].counterpart(mapping, _)),
				excludes.map(relation.row[F].counterpart(mapping, _))
			...
			throw new IllegalArgumentException(
				"Cannot recreate a GroupingSQL " + fullString + " for grouping " + grouping + " because its mapping " +
					grouping.row + " is not isomorphic with this GroupingSQL's mapping " + mapping + "."
			)
	...
	override def anchor(relation :Relation[T]) :GroupingSQL[_ <: RowProduct, F, T, V] = relation match {
		...
		case _ =>
			throw new IllegalArgumentException(
				"Cannot recreate a GroupingSQL " + fullString + " for grouping " + grouping + " :" +
...
object GroupingSQL {
	...
	//todo: move these out of companion objects in order to make them truly package private
		   ...
		   with CustomRelationSQL[F, T, S, RowProduct AndBy T]
	{
		...
		override def anchor[C <: D](grouping :GroupingRelation[C, T]) :GroupingSQL[C, F, T, S] =
			if (relation == grouping) origin
			else underlying.anchor(grouping).custom(customize).origin
		                                             ...
		                                             excludes :Unique[TypedMapping[_, E]]) :GroupingSQL[D, E, T, S] =
		{
			val moved = underlying.copy(position, underlying.includes.withOrigin[E], underlying.excludes.withOrigin[E])
			if (includes.nonEmpty || excludes.nonEmpty)
				new CustomGroupingSQL[D, E, T, S](moved, customize, includes, excludes)
			else
				new CustomGroupingSQL[D, E, T, S](moved, customize) {
...
{ self =>
	private[ast] type Component[X] = UnboundParamSQL[F, L, V, X]
	private[ast] type Column[X]    = UnboundColumnParam[F, L, V, X]

	override def form :SQLForm[V] = relation.form
...
//	override val origin :ParamSQL[F, X, L] = this
	...
	override def isAlterable :Boolean = false
	...
	override def alter(columns :Unique[TypedColumn[_, F]]) :ParamSQL[F, V, L] =
		throw new UnsupportedOperationException(
			"Cannot change column order of a ParamSQL `" + this + "` based on column parameter list " + columns + "."
		)

	override def alter(reordering :Rearrangement) :ParamSQL[F, V, L] =
		new ParamSQL[F, V, L](ParamRelation(relation.name)(relation.form.reorder(reordering)), position)
...
//		ParamSQL[F, V, L](ParamRelation(param.name)(param.form.reorder(permutation)), position)
	...
	override def reorder(permutation :IndexedSeq[Int]) :ParamSQL[F, V, L] =
		ParamSQL(ParamRelation(relation.name)(relation.form.reorder(permutation)), index)
	...
	override def anchor(param :ParamRelation[V]) :ParamSQL[F, V, L] =
		if (param == relation)
			this
		else
			ParamSQL(param, position.index)

	override def anchor(relation :Relation[ParamRelation[V]#Param]) :ParamSQL[F, V, L] =
		relation match {
			case param :ParamRelation[V @unchecked] =>
				anchor(param)
			case _ =>
				throw new IllegalArgumentException(
...
//		ParamSQL(relation, index + expansion.lengthDiff)
	...
	override def asGrouping[M[A] <: BaseMapping[S, A], S]
	                       (component :TypedComponentSQL[F, ParamRelation[V]#Param, V, M, S, L])
			:GroupingRelation[F, M] =
		GroupingRelation.param[F, M, S](component)(OriginProjection.isomorphism)

	protected override def reformComponent[M[A] <: MappingAt[A], F1 <: F, F2 <: RowProduct, S2 >: Grouped <: Single, V2,
	                                       EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
	                                      (component :ComponentSQL[F1, M] { type Origin = F },
	                                       other :ConvertibleSQL[F2, S2, V2, EC2])
	                                      (reform :Reform, passCount :PassCount)
		...
		if (component eq other)
			(leftResult(component), rightResult(other))
		else if (!passCount.otherHasPassed) //give at most a single chance to the other component.
			other.`->reform`(component)(reform.swap, passCount.++).swap
		else if (reform.compatible(component, other))
			(leftResult(component), rightResult(other))
		else
			component match {
				case mapping.components(param) =>
					val substitute = (param :UnboundParam[V, F]#ParamComponent[M[Unit]#Subject]) match {
						case column :UnboundParam[V @unchecked, F @unchecked]#ParamColumn[M[Unit]#Subject @unchecked] =>
							new UnboundColumnParam(this, column)
						case _ =>
							new UnboundParamSQL(this, param)
					}
					reform(substitute, other).castFrom[
						(leftResult.Expression[RowProduct, Single, GroundSQL[U]], rightResult.Expression[F2, S2, EC2[U]]),
						(leftResult.Expression[F, Single, LValueSQL[F, M, U]], rightResult.Expression[F2, S2, EC2[U]])
					]
				case _ =>
					throw new IllegalArgumentException(
						"Component expression's `" + component + "` mapping " + component.mapping + ": " +
						component.mapping.className + " is not a component of mapping " + mapping +
						" of unbound parameter " + this + "."
					)
			}

	protected override def reformComponent[M[A] <: BaseColumn[X, A], X, F1 <: F, F2 <: RowProduct, S2 >: Grouped <: Single, V2,
	                                       EC2[v] <: ConvertibleSQL[F2, S2, v, EC2], U]
	                                      (component :GenericColumnComponentSQL[F1, M, X] { type Origin = F },
	                                       other :ConvertibleSQL[F2, S2, V2, EC2])
	                                      (reform :Reform, passCount :PassCount)
		...
		if (component eq other)
			...
			component match {
				case mapping.components(param) =>
					reform(new UnboundColumnParam(this, param), other).castFrom[
						(leftResult.Expression[RowProduct, Single, GroundColumn[U]], rightResult.Expression[F2, S2, EC2[U]]),
						(leftResult.Expression[F, Single, ColumnLValueSQL[F, M, U]], rightResult.Expression[F2, S2, EC2[U]])
					]
				case _ =>
					throw new IllegalArgumentException(
						"Column expression's `" + component + "` mapping " + component.mapping + ": " +
						component.mapping.className + " is not a column of mapping " + mapping +
						" of unbound parameter " + this + "."
					)
			}

	protected override def realign[M[A] <: MappingAt[A], F1 <: F]
	                              (component :ComponentSQL[F1, M] { type Origin = F }, reordering :Rearrangement)
	                              (implicit spelling :SQLSpelling)
			:MappingSQL[F1, Single, M, M[Unit]#Subject] =
		if (reordering.columnCount != 0)
			throw new IllegalArgumentException(
				"Expected a zero column reordering for a param component `" + component + "`, but got " + reordering + "."
			)
		else
			new UnalignedMappingSQL(component)


	protected override def potentialColumnsOf[F1 <: F, M[A] <: MappingAt[A]]
	                                         (component :ComponentSQL[F1, M] { type Origin = F }, permissions :Permissions)
	                                         (implicit spelling :SQLSpelling) :AlignableColumns =
		//todo: a special case, which can reform both like a term by form swapping, and like a component based on extractors
		AlignableColumns.none(component)

	protected override def potentialColumnsCountOf[F1 <: F, M[A] <: MappingAt[A]]
	                                              (component :ComponentSQL[F1, M] { type Origin = F })
	                                              (implicit spelling :SQLSpelling) :Int =
		0
...
/** A temporary wrapping of a `ComponentSQL` for a `ParamSQL` in an `SQLTerm`, used to hijack the term reforming
  * process and use it to adapt the parameter component to forms from other expressions. Its `reform` method
  * returns the transformation applied to a `ComponentSQL` for mapping `component`, rather than itself.
  */
private class UnboundParamSQL[F <: RowProduct, L <: RowProduct, P, V]
                             (val param :ParamSQL[F, P, L], val component :UnboundParam[P, F]#ParamComponent[V])
	extends SQLTerm[V]
	   with SQLTermTemplate[V, ParamSQL[F, P, L]#Component, ParamSQL[F, P, L]#Column, GroundSQL]
{
	override def universalForm :Opt[SQLForm[V]] = Got(component.form)
	override def selectForm :SQLReadForm[V] = component.form

	protected def newComponent[X](f :V => X)(form :SQLForm[X]) :UnboundParam[P, F]#ParamComponent[X] =
		param.mapping.comp(component.extract andThen f)(form)

	protected def newColumn[X](f :V => X)(form :ColumnForm[X]) :UnboundParam[P, F]#ParamColumn[X] =
		param.mapping.col(component.extract andThen f)(form)

	protected override def convert[X](conversion :SQLConversion[V, X]) :GroundSQL[X] =
		if (conversion.isReversible)
			reform(conversion(component.form), conversion)
		else
			super.convert(conversion)

	override def reform[U](form :SQLForm[U], conversion :SQLConversion[V, U]) :UnboundParamSQL[F, L, P, U] =
		form match {
			case column :ColumnForm[U] =>
				reform(column, conversion)
			case _ =>
				new UnboundParamSQL(param, newComponent(conversion.apply)(form))
		}

	override def reform[U](form :ColumnForm[U], conversion :SQLConversion[V, U]) :UnboundColumnParam[F, L, P, U] =
		new UnboundColumnParam(param, newColumn(conversion.apply)(form))
	...
	protected override def reform[V2, U](form :SQLForm[V2],
		...
		form match {
			...
			case _ => leftResult match {
				//If leftResult is a conversion, we must return an LValueSQL for ParamComponent[V],
				// because we are only a forced placeholder for ComponentSQL for that mapping.
...
//					Got(reformed.asInstanceOf[leftResult.Expression[RowProduct, Single, GroundSQL[U]]])
				//If leftResult doesn't depend on the input expression type, then we can return a ComponentSQL
				// for ParamMapping[U] instead of a LValueSQL[_, ParamMapping[X], U], which we would have to otherwise.
				case SQLDecoration(identity, wrap :SQLTransformation.Into[U, U, leftResult.Expression])
					if !wrap.isTypeDependent
				=>
					val unifiedForm = rightResult(form)
					val component = param.mapping.comp(this.component.extract andThen (identity(_ :V)))(unifiedForm)
					Got(wrap.convert(ComponentSQL(param, component).castFrom[SingleSQL[F, U], GroundSQL[U]]))

				case Composition.DecoratedConversion(first, second :SQLTransformation.Into[U, U, leftResult.Expression])
					if !second.isTypeDependent
				=>
					val unifiedForm = rightResult(form)
					val component = param.mapping.comp(this.component.extract andThen (first(_ :V)))(unifiedForm)
					Got(second.convert(ComponentSQL(param, component).castFrom[SingleSQL[F, U], GroundSQL[U]]))

				//If rightResult is reversible, then we can replace the form in the component with the mapped argument.
				// We don't care that leftResult is not reversible, because spelling will happen in ComponentSQL,
				// not in LValueSQL.
...
//					val component = newComponent(rightResult.inverse compose leftResult.apply)(form)
					val component = param.mapping.comp(this.component.extract)(mappedForm)
					Got(leftResult(ComponentSQL(param, component).castFrom[SingleSQL[F, V], GroundSQL[V]]))

				//The last resort doesn't bode well: if the conversion is irreversible, then we can't map the given form
				case _ =>
					Lack
...
//					)(selectForm.nulls)
//					val component = param.mapping.comp(this.component.extract)(mappedForm)
//					Got(leftResult(ComponentSQL(param, component)))
			}
		}
//		if (!rightResult.isReversible)
//			Lack
//		else
//			super.reform(form, leftResult, rightResult)
	...
	protected override def reform[V2, U](form :ColumnForm[V2],
		...
		leftResult match {
			...
			=>
				val unifiedForm = rightResult(form)
				//We are using identity, not leftResult, as otherwise we'd apply leftResult twice because wrap==leftResult
				val component = param.mapping.col(this.component.extract andThen (identity(_ :V)))(unifiedForm)
				Got(wrap(ColumnComponentSQL(param, component).castFrom[SingleColumn[F, U], GroundColumn[U]]))

			case Composition.DecoratedConversion(first, second :SQLTransformation.Into[U, U, leftResult.Expression])
				if !second.isTypeDependent
			=>
				val unifiedForm = rightResult(form)
				val component = param.mapping.col(this.component.extract andThen (first(_ :V)))(unifiedForm)
				Got(second(ColumnComponentSQL(param, component).castFrom[SingleColumn[F, U], GroundColumn[U]]))
			...
			case _ if rightResult.isReversible =>
				...
				val component = param.mapping.col(this.component.extract)(mappedForm)
				Got(leftResult(ColumnComponentSQL(param, component).castFrom[SingleColumn[F, V], GroundColumn[V]]))
...
//				)(selectForm.nulls)
//				val component = param.mapping.comp(this.component.extract)(mappedForm)
//				Got(leftResult(ColumnComponentSQL(param, component)))
		}
//		if (!rightResult.isReversible)
//			Lack
//		else
//			super.reform(form, leftResult, rightResult)
	...
	protected override def realign(reordering :Rearrangement)(implicit spelling :SQLSpelling) :GroundSQL[V] =
		reform(component.form.reorder(reordering))

	protected override def effectiveForm(implicit spelling :SQLSpelling) :SQLForm[V] = component.form

	protected override def split(implicit spelling :SQLSpelling) :Seq[GroundColumn[_]] =
		throw new UnsupportedOperationException(
			"UnboundParamSQL `" + this + "`is a temporary expression and cannot be split."
		)

	protected override def defaultSpelling[Ps]
	                       (from :RowProduct, context :SQLContext[Ps], params :Parameterization[Ps, RowProduct])
	                       (implicit spelling :SQLSpelling) =
		throw new UnsupportedOperationException(
			"UnboundParamSQL `" + this + "`is a temporary expression and cannot be spelled."
		)

	protected override def explodedSpelling[Ps]
	                       (independent :Boolean)
	                       (from :RowProduct, context :SQLContext[Ps], params :Parameterization[Ps, RowProduct])
	                       (implicit spelling :SQLSpelling) =
		throw new UnsupportedOperationException(
	...
	override def toString :String = "UnboundParamSQL(" + param + ", " + component + ")"
}


private class UnboundColumnParam[F <: RowProduct, L <: RowProduct, P, V]
                                (param :ParamSQL[F, P, L], override val component :UnboundParam[P, F]#ParamColumn[V])
	extends UnboundParamSQL[F, L, P, V](param, component) with ColumnTerm[V]
	   with SQLTermTemplate[V, ParamSQL[F, P, L]#Component, ParamSQL[F, P, L]#Column, GroundColumn]
{
	override def universalForm :Opt[ColumnForm[V]] = Got(component.form)
	override def selectForm :ColumnReadForm[V] = component.form
	...
	protected override def reform[V2, U](form :SQLForm[V2],
		...
		form match {
			case column :ColumnForm[V2] =>
				super[UnboundParamSQL].reform(column, leftResult, rightResult)
			case _ =>
				Lack
		}
	...
	protected override def effectiveForm(implicit spelling :SQLSpelling) :ColumnForm[V] = component.form
	...
	override def toString :String = "UnboundColumnParam(" + param + ", " + component + ")"
}
...
/** Pattern matching SQL expressions representing unbound parameters and expressions directly derived from them
  ...
  */ //The object doesn't match UnboundParamSQL instances, I just reuse the name because of a lack of ideas.
object UnboundParamSQL {

	//these do not use Opt only because of a scalac bug regarding existentials and value types.
	def unapply[F <: RowProduct, T[A] <: BaseMapping[E, A], E, M[A] <: BaseColumn[V, A], V, L <: RowProduct]
	           (expr :TypedColumnComponentSQL[F, T, E, M, V, L])
			:Option[(UnboundParam[E, L], ParamColumnExtract[E, V, L], Int)] =
		expr.extract.export match {
			case param :UnboundParam[E @unchecked, L @unchecked]#ParamColumn[V @unchecked] =>
				Some((param.root, param.extract, expr.origin.index))
			case _ => None
		}
	def unapply[F <: RowProduct, T[A] <: BaseMapping[E, A], E, M[A] <: BaseMapping[V, A], V, L <: RowProduct]
	           (expr :TypedComponentSQL[F, T, E, M, V, L])
			:Option[(UnboundParam[E, F], ParamExtract[E, V, F], Int)] =
		expr.extract.export match {
			case param :ParamMapping[E @unchecked, V @unchecked, F @unchecked] =>
				Some((param.root, param.extract, expr.origin.index))
			case _ => None
		}

	def unapply[X](expr :SQLExpression[_, _, X]) //Opt causes a compiler error (scalac bug)
		...
		expr match {
			case TypedComponentSQL(table, extractor) if extractor.export.isInstanceOf[ParamMapping[_, _, _]] =>
				...
				Some((param.root, param.extract, table.index))
			case _ => None
		}
}
