...
import net.noresttherein.oldsql.sql.mechanics.{AlignableColumn, AlignableColumns, Reform, ReformPermissions, SpelledSQL, SQLAdaptation, SQLConversion, SQLTransformation}
...
//		else None
	...
	override def reapply[E <: RowProduct, C >: Grouped <: Single](e :SQLExpression[E, C, V]) :SQLExpression[E, C, V] =
		e match {
			case column :ColumnSQL[E, C, V] => reapply(column)
			case _ => e
		}
