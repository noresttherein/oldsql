...
import net.noresttherein.oldsql.sql.ColumnSQL.{ConvertibleColumn, ColumnConvertingTemplate}
...
sealed trait Interoperable[L, R] extends Serializable { self =>
	...
	/** Swaps the left and right sides of this relation. */
//	val swap :Interoperable[R, L] { type Unified = self.Unified }
	//Scala2 bug 12741
	type Swap <: Interoperable[R, L] { type Unified = Interoperable.this.Unified }
	val swap :Swap
//	val swap :Interoperable[R, L]#As[Unified]
...
//	@inline implicit def transformLeft[L, R](implicit left :SQLConversion[L, R]) :Interoperable[L, R] { type Unified = R } =
		left.asLeft

	@inline implicit def transformRight[L, R](implicit right :SQLConversion[R, L]) :Interoperable[L, R]#As[L] =
//	@inline implicit def transformRight[L, R](implicit right :SQLConversion[R, L]) :Interoperable[L, R] { type Unified = L } =
		right.asRight
}
...
//	@inline implicit def equivalence[L, R](implicit equiv :L =~= R) :Interoperable[L, R] { type Unified = equiv.Unified } =
		equiv

	def apply[L, R, U](left :SQLConversion[L, U], right :SQLConversion[R, U]) :Interoperable[L, R]#As[U] =
//	def apply[L, R, U](left :SQLConversion[L, U], right :SQLConversion[R, U]) :Interoperable[L, R] { type Unified = U } =
		left vs right

//	@inline def identity[V] :(V =~= V) { type Unified = V } = =~=.identity[V]
	@inline def identity[V] :(V =~= V)#Widen[V] = =~=.identity[V]
}
	...
	extends Interoperable[L, R]
{
	...
	override type Swap = Interoperable[R, L]#As[U]
}
...
@showAsInfix
sealed trait =~=[L, R] extends Interoperable[L, R] { self =>
	...
	/** Swaps the left and right sides of this relation. */
	override type Swap = (R =~= L)#Widen[Unified]
//	override val swap :(R =~= L) { type Unified = self.Unified }
//	override val swap :(R =~= L)#Widen[Unified]
...
//	@inline implicit def convertToLeft[L, R](implicit left :ReversibleConversion[L, R]) :(L =~= R) { type Unified = R } =
		left.asLeft

	@inline implicit def convertToRight[L, R](implicit right :ReversibleConversion[R, L]) :(L =~= R)#Widen[L] =
//	@inline implicit def convertToRight[L, R](implicit right :ReversibleConversion[R, L]) :(L =~= R) { type Unified = L } =
		right.asRight
}
...
//	def apply[L, R, U](left :ReversibleConversion[L, U], right :ReversibleConversion[R, U]): (L =~= R) { type Unified = U } =
		left =~= right
...
//		)
	...
	implicit def expressionsAreComparable
	             ...
	             (implicit unify :LV =~= RV)
			:AreComparable[ConvertingTemplate[LF, LS, LV, LE], ConvertingTemplate[RF, RS, RV, RE]] {
				type Left = LE[unify.Unified]; type Right = RE[unify.Unified]; type Unified = unify.Unified
			} =
		new Impl[ConvertingTemplate[LF, LS, LV, LE], LE[unify.Unified],
		         ConvertingTemplate[RF, RS, RV, RE], RE[unify.Unified], unify.Unified](
			unify.left(_), unify.right(_)
		)

	implicit def expressionAndLiteralAreComparable
	             [LF <: RowProduct, LS >: Grouped <: Single, LV, LE[v] <: ConvertibleSQL[LF, LS, v, LE], RV, U]
	             (implicit unify :(LV =~= RV)#Widen[U], form :SQLForm[U])
			:AreComparable[ConvertingTemplate[LF, LS, LV, LE], RV] {
				type Left = LE[unify.Unified]; type Right = GroundSQL[unify.Unified]; type Unified = U
			} =
		new Impl[ConvertingTemplate[LF, LS, LV, LE], LE[unify.Unified], RV, GroundSQL[unify.Unified], unify.Unified](
			unify.left(_), r => SQLTerm(unify.right(r))
		)
	implicit def columnSQLAndLiteralAreComparable
	             [LF <: RowProduct, LS >: Grouped <: Single, LV, LE[v] <: ConvertibleColumn[LF, LS, v, LE], RV, U]
	             (implicit unify :(LV =~= RV)#Widen[U], form :ColumnForm[U])
			:AreComparable[ConvertingColumnTemplate[LF, LS, LV, LE], RV] {
				type Left = LE[unify.Unified]; type Right = GroundColumn[unify.Unified]; type Unified = U
			} =
		new Impl[ConvertingColumnTemplate[LF, LS, LV, LE], LE[unify.Unified], RV, GroundColumn[unify.Unified], unify.Unified](
			unify.left(_), r => ColumnTerm(unify.right(r))
		)
	implicit def expressionAndMappingAreComparable
	             [LF <: RowProduct, LS >: Grouped <: Single, LV, LE[v] <: ConvertibleSQL[LF, LS, v, LE],
	              RF <: RowProduct, R <: Mapping, M[O] <: BaseMapping[RV, O], RV]
	             (implicit mappingType :InferTypeParams[R, M[RF], BaseMapping[RV, RF]], unify :LV =~= RV,
	              offset :RelationCount[RF, _ <: Numeral])
			:AreComparable[ConvertingTemplate[LF, LS, LV, LE], R] {
				type Left = LE[unify.Unified]; type Right = LValueSQL[RF, M, unify.Unified]; type Unified = unify.Unified
			} =
		new Impl[ConvertingTemplate[LF, LS, LV, LE], LE[unify.Unified], R, LValueSQL[RF, M, unify.Unified], unify.Unified](
			unify.left(_ :ConvertingTemplate[LF, LS, LV, LE]), (r :R) => unify.right(LooseComponent(mappingType(r)))
		)
