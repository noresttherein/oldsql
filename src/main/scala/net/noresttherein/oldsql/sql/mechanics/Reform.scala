...
import net.noresttherein.oldsql.sql.ast.{ComponentSQL, JoinedRelation, IndexedSQL, LValueSQL}
import net.noresttherein.oldsql.sql.mechanics.Reform.{PassCount, QueryReformOverrideDecorator, SwappedReform}
import net.noresttherein.oldsql.sql.mechanics.ReformPermissions.{BinaryReformPermissions, DetailedReformPermissions, ForbiddenReform, Permissions, ReformPermissionsDecorator}
import net.noresttherein.oldsql.sql.mechanics.ReformPermissions.Permissions.{MayAddNull, MayAddNullLeft, MayAddNullRight, MayAlter, MayAlterLeft, MayAlterRight, MayExclude, MayExcludeLeft, MayExcludeRight, MayInclude, MayIncludeLeft, MayIncludeRight, MayReform, MayReformLeft, MayReformRight, MayReorder, MayReorderLeft, MayReorderRight, NoReform}
...
trait ReformPermissions[Self <: ReformPermissions[Self]] extends Decorable[Self] { this :Self =>
	...
	def allowExcludeLeft  :Self = allow(mayExcludeLeft = true)
	...
	def allowIncludeLeft  :Self = allow(mayIncludeLeft = true)
	/** A reform of the same policy as this one, but allowed to additionally reorder the columns of the left
	  *  component expression in order to match the column set of the right expression. This call may have no effect
	  *  if an implementation does not support such fine grained control. */
	def allowReorderLeft  :Self = allow(mayReorderLeft = true)
	/** A reform of the same policy as this one, but allowed to additionally add null columns to the left expression
	  ...
	  * does not support such fine grained control. */
	def allowAddNullLeft  :Self = allow(mayAddNullLeft = true)
	...
	def allowAlterLeft   :Self =
		allow(mayExcludeLeft = true, mayIncludeLeft = true, mayReorderLeft = true)//, mayAddNullLeft = true)
	...
	def allowReformLeft  :Self =
		allow(mayExcludeLeft = true, mayIncludeLeft = true, mayReorderLeft = true, mayAddNullLeft = true)//, mayAddNullLeft = true)
	...
	/** A reform of the same policy as this one, but allowed to additionally exclude columns from the right expression
	  ...
	  * does not support such fine grained control. */
	def allowExcludeRight :Self = allow(mayExcludeRight = true)
	/** A reform of the same policy as this one, but allowed to additionally include columns of the right
	  ...
	  * if an implementation does not support such fine grained control. */
	def allowIncludeRight :Self = allow(mayIncludeRight = true)
	/** A reform of the same policy as this one, but allowed to additionally reorder the columns of the right
	  *  component expression in order to match the column set of the left expression. This call may have no effect
	  *  if an implementation does not support such fine grained control. */
	def allowReorderRight :Self = allow(mayReorderRight = true)
	/** A reform of the same policy as this one, but allowed to additionally add null columns to the right expression
	  ...
	  * does not support such fine grained control. */
	def allowAddNullRight :Self = allow(mayAddNullRight = true)
	/** A reform of the same policy as this one, but allowed to additionally include, exclude and reorder columns
	...
	/** Grants additional permissions to this $self. Not all implementations support enabling operations
	  * which aren't allowed, but, for those which do, the result's permissions will equal
	  * `this.`[[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permissions permissions]]` + permissions`.
	  * The result is a $Self of the same type, with the same decorators as this instance, but, possibly,
	  * with additional permissions. This means that in order to grant a permission, `permission`
	  * should have the appropriate flag set to `true`, but setting it to `false`
	  * will not revoke a currently possessed permission.
	  * @see [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permit]]
	  */
	def allow(permissions :Permissions) :Self = permit(permissions + this.permissions)

	/** Creates a $Self of the same type as this one, and with the same decorators applied,
	  * but with the given permissions granted. Not all implementations allow enabling not permitted operations,
	  * as, in most circumstances, this would lead to breaking the contract. Moreover, some may not offer possibility
	  * of setting permissions with this granularity, but on an all-or-nothing basis (`allowReformLeft`/`allowReformRight`).
	  * In order to grant a permission, `permission` should have the appropriate flag set to `true`,
	  * but setting it to `false` will not revoke a currently currently enabled permission.
	  * @see [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permit]]
	  */
	def allow(mayExcludeLeft :Boolean = mayExcludeLeft, mayIncludeLeft :Boolean = mayIncludeLeft,
...
//		))
	...
	/** Creates a $Self of the same type as this one, but with the given permissions revoked.
	  * In order to prohibit an individual operation, the corresponding parameter should be set to `false`.
	  * Note that this is not necessarily equivalent to prohibiting these permissions individually
	...
	/** Creates a $Self of the same type as this one, and with the same decorators applied,
	  * but with the given permissions revoked. If the implementation supports setting of permissions
	  * for individual operations (rather than all-or-nothing `prohibitReformLeft`/`prohibitReformRight`), the result's
	  * [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permissions permissions]]` == this.permissions & permissions`
	  * This means that in order to revoke a permission, `permission` should have the appropriate flag set to `false`,
	  * but setting it to `true` will not grant the permission if it is not currently enabled.
	  * @see [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permit]]
	  */
	def prohibit(permissions :Permissions) :Self = permit(permissions & this.permissions)

	/** Creates a $Self of the same type as this one, and with the same decorators applied,
	  * but with the specified permissions changes applied. Not all implementations allow enabling
	  * of not permitted operations, as, in most circumstances, this would lead to breaking the contract.
	  * Moreover, some may not offer possibility of setting permissions with this granularity,
	  * but on an all-or-nothing basis (`allowReformLeft`/`allowReformRight`).
	  * However, for those implementations which do support it, the result's permission flags will be set
	  * to the values of parameters with the same name. Omitting an argument leads to the permission's status
	  * remaining unchanged.
	  * @see [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permit]]
	  */
	def permit(mayExcludeLeft :Boolean = mayExcludeLeft, mayIncludeLeft :Boolean = mayIncludeLeft,
	           mayReorderLeft :Boolean = mayReorderLeft, mayAddNullLeft :Boolean = mayAddNullLeft,
	           mayExcludeRight :Boolean = mayExcludeRight, mayIncludeRight :Boolean = mayIncludeRight,
	           mayReorderRight :Boolean = mayReorderRight, mayAddNullRight :Boolean = mayAddNullRight) :Self =
		permit(Permissions(
	...
	/** Creates a $Self of the same type as this one, and with the same decorators applied,
	  * but with the given permission changes applied. If the implementation supports setting of permissions
	  * for individual operations (rather than all-or-nothing `prohibitReformLeft`/`prohibitReformRight`), the result's
	  * [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permissions permissions]] will equal the argument.
	  * Not all implementations, however, support enabling of additional permissions at all, in which case
	  * the method has the same effect as [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.prohibit prohibit]].
	  * @see [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permit]]
	  */
	def permit(permissions :Permissions) :Self
...
//	)
	...
	@inline private def thisReform :Self = this
	...
	protected def typeName :String = this.innerClassName
	override def toString :String =
		if (permissions == MayReform) typeName else typeName + "(" + permissionsString + ")"
}
...
object ReformPermissions {
	/** A light carrier for permissions information
	  ...
	  * The permissions exist in the context of aligning columns of two expressions or queries:
	  * there is a permission for every possible action which may be taken to unify their shapes has a permission,
	  * and it can be granted individually for the first/left and/or second/right expression/query.
	  * Occasionally however, this class is also used to describe permissions in singular, without distinguishing
	  * between the two sides. In those cases, a permission is understood to be granted ''iff'' it is granted
	  * for both left and right side.
	  *
	  * Each permission is represented by a single bit in the backing `Int`, with the lowest byte used
	  * for left permissions and the second lowest byte for right permissions.
	  * See the constants defined in the companion object for each individual permission.
	  * Logical operators work exactly as bitwise operators on the underlying `Int`.
	  *
	  * Instances can be created either through the factory method in the companion object,
	...
	class Permissions private[mechanics] (private[oldsql] val flags :Int) extends AnyVal {
		/** Logical negation of all existing permissions: a permission is given ''iff'' its not allowed by this instance. */
		@inline def unary_! :Permissions = new Permissions(~flags & 0xf0f)
		/** Logical negation of all existing permissions: a permission is given ''iff'' its not allowed by this instance. */
		@inline def unary_~ :Permissions = new Permissions(~flags & 0xf0f)
		/** An instance having a particular permission ''iff'' both `this` and `other` have that permission. */
		@inline def &(other :Permissions) :Permissions = new Permissions(flags & other.flags)
		/** An instance having a particular permission ''iff'' at least one of `this` or `other` has that permission. */
		@inline def |(other :Permissions) :Permissions = new Permissions(flags | other.flags)
		/** An instance having a particular permission ''iff'' at least one of `this` or `other` has that permission. */
		@inline def +(other :Permissions) :Permissions = new Permissions(flags | other.flags)
		/** An instance having a particular permission ''iff'' it is permitted by `this`, but not by `other`. */
		@inline def -(other :Permissions) :Permissions = new Permissions(flags & ~other.flags)
		/** An instance having a particular permission ''iff'' exactly one of `this` and `other` has that permission. */
		@inline def ^(other :Permissions) :Permissions = new Permissions(flags ^ other.flags)
		/** Clears all permissions to `NoReform` if `other` is `false`. */
		@inline def &(other :Boolean) :Permissions = if (other) this else NoReform
		/** Switches each permission from 'on' to 'off' or vice versa if `other` is `true`. */
		@inline def ^(other :Boolean) :Permissions = if (other) new Permissions(~flags & 0xf0f) else this

		/** Answers is these permissions include the specified permission(s). Same as `this >= other`. */
		@inline def apply(permission :Permissions) :Boolean =
			(flags & permission.flags) == permission.flags

		/** True if `other` allows all permissions of this instance. */
		def <=(other :Permissions) :Boolean = other(this)
		/** True if all permissions of `other` are allowed by this instance. */
		def >=(other :Permissions) :Boolean = apply(other)
		/** `this <= other && this != other` */
		def < (other :Permissions) :Boolean = flags != other.flags & other(this)
		/** `this >= other && this != other` */
		def > (other :Permissions) :Boolean = flags != other.flags & apply(other)

		/** When reforming a pair of expressions, optional columns of the first one can be added to the alignment. */
		def mayIncludeLeft  :Boolean = apply(MayIncludeLeft)
		/** When reforming a pair of expressions, optional columns of the first one can be removed from the alignment. */
		def mayExcludeLeft  :Boolean = apply(MayExcludeLeft)
		/** When reforming a pair of expressions, columns of the first one can be reordered. */
		def mayReorderLeft  :Boolean = apply(MayReorderLeft)
		/** When reforming a pair of expressions, `NULL` columns may be added to the first one,
		  * aligned with a column in the second expression. */
		def mayAddNullLeft  :Boolean = apply(MayAddNullLeft)
		/** When reforming a pair of expressions, columns of the first one can be reordered,
		  * and optional columns included or excluded. */
		def mayAlterLeft    :Boolean = apply(MayAlterLeft)
		/** When reforming a pair of expressions, the first one can be modified in any legal way to match the second one. */
		def mayReformLeft   :Boolean = apply(MayReformLeft)
		/** When reforming a pair of expressions, optional columns of the second one can be added to the alignment. */
		def mayIncludeRight :Boolean = apply(MayIncludeRight)
		/** When reforming a pair of expressions, optional columns of the second one can be removed from the alignment. */
		def mayExcludeRight :Boolean = apply(MayExcludeRight)
		/** When reforming a pair of expressions, columns of the second one can be reordered. */
		def mayReorderRight :Boolean = apply(MayReorderRight)
		/** When reforming a pair of expressions, `NULL` columns may be added to the second one,
		  * aligned with a column in the first expression. */
		def mayAddNullRight :Boolean = apply(MayAddNullRight)
		/** When reforming a pair of expressions, columns of the second one can be reordered,
		  * and optional columns included or excluded. */
		def mayAlterRight   :Boolean = apply(MayAlterRight)
		/** When reforming a pair of expressions, columns of the second one can be reordered,
		  * and optional columns included or excluded. */
		def mayReformRight  :Boolean = apply(MayReformRight)
		/** Optional columns may be removed from either expression. */
		def mayExclude      :Boolean = apply(MayExclude)
		/** Optional columns may be added to either of the expressions. */
		def mayInclude      :Boolean = apply(MayInclude)
		/** Columns can be reordered in any expressions. */
		def mayReorder      :Boolean = apply(MayReorder)
		/** `NULL` columns can be added before, between, or after columns of any expression. */
		def mayAddNull      :Boolean = apply(MayAddNull)
		/** Columns may be reordered, and optional columns can be included or excluded. */
		def mayAlter        :Boolean = apply(MayAlter)
		/** All permissions are granted. */
		def mayReform       :Boolean = apply(MayReform)
		...
		/** Substitutes right permissions for left permissions,
		  ...
		  * permissions. */
		@inline def asInLeft  :Permissions  = new Permissions((flags & 0xff) << 8 | flags & 0xff)

		/** Substitutes left permissions for right permissions,
		  * creating [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.Permissions.isSymmetrical symmetrical]]
		  * permissions. */
		@inline def asInRight :Permissions  = new Permissions(flags & ~0xff | flags >>> 8)

		/** Swaps the left permissions with right permissions, creating a mirror version of this object. */
		@inline def swap      :Permissions  = new Permissions(((flags << 8) | (flags >>> 8)) & 0xffff)

		/** True ''iff'' left permissions are equal right permissions. */
		@inline def isSymmetrical :Boolean = (flags & 0xff) == (flags >> 8)
	...
	object Permissions {
		/** A factory method for [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.Permissions Permissions]]
		  ...
		  */
		def apply(mayExcludeLeft :Boolean = false, mayIncludeLeft :Boolean = false,
		...
		/** When reforming a pair of expressions, optional columns of the second one can be added to the alignment. */
		final val MayExcludeRight = new Permissions(0x100)
		/** When reforming a pair of expressions, optional columns of the second one can be removed from the alignment. */
		final val MayIncludeRight = new Permissions(0x200)
		/** When reforming a pair of expressions, columns of the second one can be reordered. */
		final val MayReorderRight = new Permissions(0x400)
		/** When reforming a pair of expressions, `NULL` columns may be added to the second one,
		  * aligned with a column in the first expression. */
		final val MayAddNullRight = new Permissions(0x800)
		/** When reforming a pair of expressions, columns of the second one can be reordered,
		  * and optional columns included or excluded. */
		...
		/** When reforming a pair of expressions, no changes are allowed in the second one, but none if the first one. */
		final val NoReformLeft    = MayReformRight
		/** When reforming a pair of expressions, no changes are allowed in the first one, but none if the second one. */
		final val NoReformRight   = MayReformLeft
		/** No changes are allowed. */
		final val NoReform        = new Permissions(0)
		/** All changes are allowed. */
		final val All             = MayReform
	}
...
//		override def mayReformRight  :Boolean = decorated.mayReformRight
		...
		override def allow(permissions :Permissions) :Self =
			if (this.permissions(permissions)) self else decorated.allow(permissions)
		...
		override def permit(permissions :Permissions) :Self =
			if (this.permissions == permissions) self
			else decorated.permit(permissions)
...
//				                 mayExcludeRight, mayIncludeRight, mayReorderRight, mayAddNullRight)
	...
	trait BinaryReformPermissions[Self <: ReformPermissions[Self]] extends ReformPermissions[Self] { this :Self =>
		...
		override def allowAlterLeft    :Self =
			if (mayAlterLeft) self else permit(true, mayAlterRight).self
		...
		override def allowAlterRight   :Self =
			if (mayAlterRight) self else permit(mayAlterLeft, true).self
		...
		override def prohibitReformLeft   :Self =
			if (mayAlterLeft) permit(false, mayAlterRight).self else self
		...
		override def prohibitReformRight  :Self =
			if (mayAlterRight) permit(mayAlterLeft, false).self else self
		override def prohibitAll          :Self =
			if (mayAlterLeft || mayAlterRight) permit(false, false).self else self


		override def permit(permissions :Permissions) :Self =
			//we ignore mayAddNullLeft/mayAddNullRight arguments because they are hard coded to false
			if (permissions(MayAlterLeft) == mayAlterLeft && permissions(MayAlterRight) == mayAlterRight)
				self
			else
				permit(permissions(MayAlterLeft), permissions(MayAlterRight)).self

		protected def permit(mayReformLeft :Boolean, mayReformRight :Boolean) :Self
...
//		}
	...
	{ this :Self =>
		...
		override def permit(permissions :Permissions) :Self =
			if (this.permissions == permissions) self
			else constructor(permissions, wrap).self
//		override def allow(permissions :Permissions) :Self =
//			if (this.permissions(permissions)) self
//			else constructor(this.permissions + permissions, wrap).self
//
...
//			))
	...
	private[mechanics] trait ForbiddenReform[Self <: ReformPermissions[Self]] extends ReformPermissions[Self] {
		...
		override def permit(permissions :Permissions) :Self = self
...
//	type LValue[-F <: RowProduct, M[O] <: MappingAt[O], V] <: SQLExpression[F, Single, V]
	...
	/** Recursively adjusts the column sets of the `left` and  `right` `SQLExpression`.
	  ...
	  * [[net.noresttherein.oldsql.sql.SQLExpression.realign reform]]) of both expressions.
	  *
	...
	/** Checks if the argument expressions already have a unified form.
	  ...
	  * or if expression's [[net.noresttherein.oldsql.sql.SQLExpression.realign reform]] method cannot simply
	  ...
	  */
	//consider: do we really need the conversions here? On one hand, they may be useful, but on the other,
	// they make it impossible to use Reform for compatibility tests in multi expression alignments, which
	// couldn't possibly have this informatin for all column pairs.
	...
	def compatible[LF <: RowProduct, LS >: Grouped <: Single, LV, RF <: RowProduct, RS >: Grouped <: Single, RV, U]
		...
		try {
			(left eq right) || spelling.shape(left) <:> spelling.shape(right)
		} catch {
	...
	def default[LF <: RowProduct, LM[O] <: MappingAt[O], RF <: RowProduct, RM[O] <: MappingAt[O], U]
			...
			:(leftResult.Expression[LF, Single, LValueSQL[LF, LM, U]],
			  rightResult.Expression[RF, Single, LValueSQL[RF, RM, U]])
...
//	}
	 ...
	 * the structure of the tree (and, for example, accept two IndexedSQL expressions with different keys,
...
//	def subreform(implicit spelling :SQLSpelling) :Reform = self
	...
	/** A `QueryReform` to use if one of the reformed expressions
	  * is a [[net.noresttherein.oldsql.sql.ast.CompoundSelectSQL CompoundSelectSQL]]. By default it delegates
	  * to `spelling.`[[net.noresttherein.oldsql.sql.SQLDialect.SQLSpelling.queryReform queryReform]], imposing on it
	  * the restrictions coming from this reform's
	  * [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.permissions permissions]], but it can be overridden
	  * by user to return any reform with [[net.noresttherein.oldsql.sql.mechanics.Reform.impose impose]] method.
	  */
	def queryReform(implicit spelling :SQLSpelling) :QueryReform = spelling.queryReform.prohibit(self.permissions)

	/** A reform equivalent to this one, but using the specified `QueryReform` if one of the expressions
	  * is a ''compound select'', imposing this reform's restrictions on it
	  * using [[net.noresttherein.oldsql.sql.mechanics.ReformPermissions.prohibit prohibit]] method.
	  */
	def impose(reform :QueryReform) :Reform = decorate(new QueryReformOverrideDecorator(reform)(_))
...
//	}
	override def equals(that :Any) :Boolean = that match {
		case same :AnyRef if same eq this => true
...
object Reform {
	...
	object PassCount {
		def apply() :PassCount = new PassCount(0)
	}
...
//		@inline def hasPassed         :Boolean = toInt > 1
		...
		/** The current expression's `reform` method is called for the second time (exactly) with `other`
		  ...
		  */
		@inline def secondTime        :Boolean = toInt >= 2 & toInt <= 3

		/** The current expression's `reform` method is called for the third time (exactly) with `other`
		  * and `other.reform` has been called once or twice.
		  * @return `toInt > 1 & toInt <= 3`.
		  */
		@inline def thirdTime         :Boolean = toInt >= 4 & toInt <= 5
		...
		/** The other expression's `reform` method has been called exactly two times with this expression as the argument.
		  ...
		  */
		@inline def gaveTwoChances    :Boolean = toInt >= 3 & toInt <= 4

		/** The other expression's `reform` method has been called exactly three times with this expression
		  *  as the argument.
		  * @return `toInt >= 5 & toInt <= 6`
		  */
		@inline def gaveThreeChances  :Boolean = toInt >= 5 & toInt <= 6

		/** This expression's `reform` method is being called at least for the second time,
		  * the first time having passed control to the other expression's `reform` method.
		  * @return `toInt > 1`.
		  */
		@inline def passedOnceOrMore  :Boolean = toInt >= 2
		...
		/** This expression's `reform` method is being called at least for the third time,
		  ...
		  * @return `toInt > 3`.
		  */
		@inline def passedTwiceOrMore :Boolean = toInt >= 4

		@inline override def toString :String = toInt.toString
	}

	//Consider: increasing to 7:
	//  1. anything synthetic that *must* act first
	//  2. conversions, sure bets
	//  3. second choice
	//  4. SQLNull and last resort terms which have a high likelihood of failing later.
...
*/
	...
	/** A `Reform` mixin trait which overrides all reforming methods with paired arguments in order to perform
	  ...
	  * This check is not performed by standard implementations as in many cases it would amount to a cost comparable
	  * to attempting the unification. An alternative check which does not result in a creation of unified expression
	  * would require doubling the API with a validation method mirroring
	  * [[net.noresttherein.oldsql.sql.SQLExpression.realign reform]].
	  */
...
*/
	...
	{
		protected override def permit(mayReformLeft :Boolean, mayReformRight :Boolean) :Reform =
...
//		override def isSymmetrical :Boolean = decorated.isSymmetrical
		...
		override def queryReform(implicit spelling :SQLSpelling) :QueryReform = decorated.queryReform
//
...
//			decorated.decorate(decorator)
	...
	/** `SwappedReform` is a reform proxy which switches left nad right sides when delegating to the underlying instance.
	  ...
	  * for [[net.noresttherein.oldsql.sql.mechanics.Reform.self self]]): if the swapped `Reform`
	  * would pass `self` reference somewhere, the next call made with already swapped sides would swap them again,
	  * obliterating the sense of delegating all methods to `swap` before swapping the result - we want everything done
	  * as the decorated reform would, but with swapped sides. This changes if a `SwappedReform` is decorated:
	  * in that case we add swapping of the sides before the new decorator ''and after it'', so that `swap.self`
	  * retains correct parameter order. I don't know if it's the best, or even good, approach, but it is the only
	  * sensible one I can think of.
	  */
	private class SwappedReform(override val swap :Reform) extends Reform {
		...
		override def underlying :Reform = swap.underlying
		...
		override def decorations :Reform => Reform = swap.decorations

		override def redecoration :Reform => Reform =
			throw new UnsupportedOperationException("SwappedReform " + this + " is not present on the decorator stack.")
...
*/
		...
		override def allow(permissions :Permissions) :Reform = swap.allow(permissions.swap).swap
		...
		override def permit(permissions :Permissions) :Reform = swap.permit(permissions.swap).swap
...
*/
	...
	private class QueryReformOverrideDecorator(defaultReform :QueryReform)(override val decorated :Reform)
		extends ReformDecorator(new QueryReformOverrideDecorator(defaultReform)(_))
	{
		override def queryReform(implicit spelling :SQLSpelling) :QueryReform = defaultReform.prohibit(self.permissions)
	}
		...
		extends SimpleReform(mayAlterLeft, mayAlterRight)(wrap, constructor(_, _, _))
	{
		...
		override def apply[LF <: RowProduct, LS >: Grouped <: Single, LV, LE[v] <: ConvertibleSQL[LF, LS, v, LE],
				...
				:(leftResult.Expression[LF, LS, LE[U]], rightResult.Expression[RF, RS, RE[U]]) =
			(left, right) match {
				case (_ :LValueSQL.__ @unchecked | _ :IndexedSQL.__, _ :LValueSQL.__ @unchecked | _ :IndexedSQL.__) =>
					left.`->reform`[RF, RS, RV, RE, U](right)(noReform, new PassCount(0))
				case (_ :LValueSQL.__ @unchecked | _ :IndexedSQL.__, _) =>
					left.`->reform`[RF, RS, RV, RE, U](right)(noReformLeft, new PassCount(0))
				case (_, _ :LValueSQL.__ @unchecked | _ :IndexedSQL.__) =>
					left.`->reform`[RF, RS, RV, RE, U](right)(noReformRight, new PassCount(0))
				case _ =>
...
//		}
		...
		extends FlatValidatorReform(wrap, new ArityValidator(_))
	{
		...
		override def compatible[LF <: RowProduct, LS >: Grouped <: Single, LV,
				...
				:Boolean =
			try {
				(left eq right) || spelling.columnCount(left) == spelling.columnCount(right)
			} catch {
				case _ :UnsupportedOperationException | _ :InvalidSQLException => false
			}
...
*/
	...
	//  has more informative layout: LValueSQL first, then IndexedSQL, then anything other than a term.
...
*/
	...
	//todo: we need a reform implementation which will still reform non-default components, because in QueryReform,
	// in `query reform query`, the same expression will be reformed multiple times against various expressions.
	/** The default reforming strategy used to unify the columns of a pair of SQL expressions used within another
	  ...
	  * [[net.noresttherein.oldsql.sql.SQLExpression.realign reform]] methods and handling only the case
...
*/
		override def default[LF <: RowProduct, LM[O] <: MappingAt[O], RF <: RowProduct, RM[O] <: MappingAt[O], U]
			...
			if (left.mapping homomorphic right.mapping)
				(new DefaultComponentReformer[LF, LM, RF, RM, U, leftResult.Expression, rightResult.Expression](
	...
	private abstract class HomomorphicComponentReformer
		                   ...
		                   (val left :ComponentSQL[LF, LM], val right :ComponentSQL[RF, RM])(reform :Reform)
		                   (implicit leftResult  :SQLTransformation.Into[LM[Unit]#Subject, U, LR],
		                             rightResult :SQLTransformation.Into[RM[Unit]#Subject, U, RR], spelling :SQLSpelling)
	{   //different components, we need to compare actual column sets
		...
		private def reformFirst[F1 <: RowProduct, M1[O] <: MappingAt[O], F2 <: RowProduct, M2[O] <: MappingAt[O], U,
			...
			:conversion.Expression[F1, Single, LValueSQL[F1, M1, U]] =
		{
			val reformed =
		...
		protected def reformLeft :LR[LF, Single, LValueSQL[LF, LM, U]] =
			reformFirst(left, right)(
				leftDefaultColumns, rightDefaultColumns, leftCounterparts, leftDefaultCounterparts
			)(leftResult)

		protected def reformRight :RR[RF, Single, LValueSQL[RF, RM, U]] =
			reformFirst(right, left)(
				rightDefaultColumns, leftDefaultColumns, rightCounterparts, rightDefaultCounterparts
			)(rightResult)

		protected def convertLeft  :LR[LF, Single, LValueSQL[LF, LM, U]] = leftResult.convert(left)
		protected def convertRight :RR[RF, Single, LValueSQL[RF, RM, U]] = rightResult.convert(right)
	...
	private class DefaultComponentReformer
	              [LF <: RowProduct, LM[O] <: MappingAt[O], RF <: RowProduct, RM[O] <: MappingAt[O], U,
	               LR[-F <: RowProduct, -S >: Grouped <: Single, +E <: SQLExpression[F, S, U]] <: SQLExpression[F, S, U],
	               RR[-F <: RowProduct, -S >: Grouped <: Single, +E <: SQLExpression[F, S, U]] <: SQLExpression[F, S, U]]
		          (override val left :ComponentSQL[LF, LM], override val right :ComponentSQL[RF, RM])(reform :Reform)
		          (implicit leftResult  :SQLTransformation.Into[LM[Unit]#Subject, U, LR],
		                    rightResult :SQLTransformation.Into[RM[Unit]#Subject, U, RR],
		                    spelling :SQLSpelling)
		extends HomomorphicComponentReformer[LF, LM, RF, RM, U, LR, RR](left, right)(reform :Reform)(leftResult, rightResult, spelling)
	{
		...
		def apply() :(LR[LF, Single, LValueSQL[LF, LM, U]], RR[RF, Single, LValueSQL[RF, RM, U]]) =
			if (leftDefaultColumns == leftDefaultCounterparts)
				...
				if (mayAlterLeft && left.isAlterable && left.isDefault)
					(reformLeft, convertRight)
				else if (mayAlterRight && right.isAlterable && right.isDefault)
				...
				if (mayAlterLeft && left.isAlterable && left.isDefault && left.relation.isDefault)         //reform left
					(reformLeft, convertRight)
				else if (mayAlterRight && right.isAlterable && right.isDefault && right.relation.isDefault) //reform right
					(convertLeft, reformRight)
					...
					if (mayAlterLeft && left.isAlterable)
						(reformLeft, convertRight)
					else
						(convertLeft, reformRight)
					...
					if (mayAlterLeft && left.isAlterable && left.relation.isDefault)        //reform left
						(reformLeft, convertRight)
					else if (mayAlterRight && right.isAlterable && right.relation.isDefault) //reform right
						(convertLeft, reformRight)
					else
						fail()
				else
