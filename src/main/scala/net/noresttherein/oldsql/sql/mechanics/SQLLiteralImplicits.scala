...
import net.noresttherein.oldsql.sql.ast.{BoundParam, ChainTuple, EmptySQL, IndexedSQL, SeqSQL, SQLNull, SQLTerm}
...
private[sql] sealed trait Rank1SQLLiteralImplicits {
	implicit def implicitLiteral[T](value :T)(implicit factory :SQLTerm.Factory[T]) :factory.Res = factory(value)
...
private[sql] trait SQLLiteralImplicits extends Rank1SQLLiteralImplicits {
	...
	implicit final def noImplicitBoolean1(value :Boolean) :SingleBoolean[RowProduct] = if (value) True else False
	implicit final def noImplicitBoolean2(value :Boolean) :SingleBoolean[RowProduct] = if (value) True else False

	implicit final def implicitBooleanLiteral[B <: Boolean with Singleton](value :B) :SingleBoolean[RowProduct] =
		if (value) True else False

	implicit final def implicitNullTerm[T](n :Null)(implicit factory :SQLNull.Factory[T]) :factory.Res = factory(())

	implicit final def fromEmptyTuple(empty: @~) :EmptySQL = EmptySQL

	implicit final def boundParameterSQL[T, P <: BoundParam[T]]
	                                    (value :T)(implicit factory :BoundParamFactory[T, P]) :boundParameterSQL[T, P] =
		new boundParameterSQL[T, P](value)

	implicit final def commonTableExpressionName[A <: Label](name :A) :CTEName[A] = new CTEName[A](name)
	...
	/** Enriches the `null` literal with an `apply[X]` methods which create
	  ...
	  */
	implicit final def nullSQL(n :Null) :nullSQL = new nullSQL {}


	implicit final def seqExpression[F <: RowProduct, S >: Grouped <: Single, T]
	                                (expressions :Seq[SQLExpression[F, S, T]]) :SeqSQL[F, S, T] =
		SeqSQL(expressions :_*)

	implicit final def chainExpression[F <: RowProduct, S >: Grouped <: Single, Exprs <: Chain, E, I <: Chain, L]
	                                  (expressions :Exprs ~ E)
	                                  (implicit init :Exprs => ChainTuple[F, S, I], entry :E <:< SQLExpression[F, S, L])
			:ChainTuple[F, S, I ~ L] =
		init(expressions.init) ~ entry(expressions.last)

//	implicit def fromEmptyChain(empty: @~) :ChainTuple[RowProduct, Single, @~] = EmptySQL

	implicit final def listingColumnExpression //todo: test if it works, likely type inferer will fail
	                   [F <: RowProduct, S >: Grouped <: Single, Exprs <: Listing, Item <: K :~ L, E, I <: Listing, K <: Label, L]
	                   (expressions :Exprs |~ Item)
	                   (implicit init :Exprs => IndexedSQL[F, S, I], entry :Item <:< (K :~ LabeledValueSQL[F, S, L]),
	                             key :ValueOf[K])
			:IndexedSQL[F, S, I |~ (K :~ L)] =
		init(expressions.init) |~ :~[K](entry(expressions.last).value)
