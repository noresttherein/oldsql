...
import net.noresttherein.oldsql.sql.ast.{AdaptedSQL, ChainSQL, ComponentSQL, CompoundSelectSQL, InlineSQL, IndexedSQL, LValueSQL, QuerySQL, SelectIdSQL, SelectSQL}
...
/** A base trait for `Reform` implementations which do not perform a full unification
  ...
  * linear in computational complexity, as `Query.`[[net.noresttherein.oldsql.sql.Query.reformed reform]] methods
...
//			case (_, _ :ComponentLValueSQL[_, _, _]) => ???
			case (_ :IndexedSQL[_, _, _], _ :IndexedSQL[_, _, _]) =>
			...
			case (_ :InlineSQL[_, _, _] | _ :ChainSQL[_, _, _, _] | _ :SelectIdSQL[_, _, _],
				...
				super.apply[LF, LS, LV, LE, RF, RS, RV, RE, U](left, right)(leftResult, rightResult, spelling)
			case (_ :AdaptedSQL[_, _, _, _], _) =>
				super.apply[LF, LS, LV, LE, RF, RS, RV, RE, U](left, right)(leftResult, rightResult, spelling)
			case (_, _ :AdaptedSQL[_, _, _, _]) =>
				super.apply[LF, LS, LV, LE, RF, RS, RV, RE, U](left, right)(leftResult, rightResult, spelling)
			case _ =>
				fallback[LF, LS, LV, LE, RF, RS, RV, RE, U](left, right)(leftResult, rightResult, spelling) //ColumnSQL, CompositeSQL, MappingSQL, SQLTerm, QuerySQL
		}
