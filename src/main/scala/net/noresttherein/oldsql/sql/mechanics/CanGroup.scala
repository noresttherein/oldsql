...
import net.noresttherein.oldsql.sql.ast.{ComponentSQL, LabeledSQL}
...
//			}
	...
	implicit def groupByListing[O <: FromSome, F <: FromSome { type Generalized = O }, S <: Listing]
			:CanGroup[O, F, LabeledSQL[O, Single, S]] { type Result = F GroupBy IndexedMapping.of[S]#M } =
		new CanGroup[O, F, LabeledSQL[O, Single, S]] {
			override type Result = F GroupBy IndexedMapping.of[S]#M

			override def apply(clause :F, expr :LabeledSQL[O, Single, S]) =
	...
	implicit def byListing[F <: FromSome, G <: GroupingOfGeneralized[F], S <: Listing]
			:CanGroup[F, G, LabeledSQL[F, Single, S]] { type Result = G By IndexedMapping.of[S]#M } =
		new CanGroup[F, G, LabeledSQL[F, Single, S]] {
			override type Result = G By IndexedMapping.of[S]#M

			override def apply(clause :G, expr :LabeledSQL[F, Single, S]) :G By IndexedMapping.of[S]#M =
