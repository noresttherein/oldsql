...
object TableStatement {
	...
	class SetParamScope[X, M[O] <: MappingAt[O]](val domain :From[M] WithParam X) {
		val / :M[From[M]] = domain.left.right.row[From[M]]
		val row :M[RowProduct AndFrom M WithParam X] = domain.prev
//		val old :M[RowProduct AndFrom M WithParam X] = domain.prev
		def param :UnboundParam[X, RowProduct AndFrom ParamRelation[X]#Param] = ?
		val ? :UnboundParam[X, RowProduct AndFrom ParamRelation[X]#Param] = domain.last.mapping
	}
	...
	private[sql] trait SetClauseFactory[Arg, M[O] <: MappingAt[O], F >: From[M] WithParam Arg <: RowProduct, +Res]
		extends Any with SetClauseDomain[Arg, M, F]
	{
		protected[sql] final def `->set`(setter :From[M] := SetDomain) :Res = set(setter)
		...
		protected def setAll(setters :Seq[From[M] := SetDomain]) :Res
...
//		}
		...
		protected def set(setter :From[M] := SetDomain) :Res = setAll(PassedArray :+ setter)

		def set(setters :Seq[(M[From[M]], UnboundParam.Last[Arg]) => From[M] := SetDomain]) :Res = {
			val base = setDomain
		...
		def set(setter :(M[From[M]], UnboundParam.Last[Arg]) => From[M] := F) :Res = {
			val base = setDomain
	...
	private[sql] trait SupplantClauseFactory[Arg, M[O] <: MappingAt[O], F >: From[M] WithParam Arg <: RowProduct, +Res]
		extends Any with SetClauseDomain[Arg, M, F]
	{
		protected[sql] final def `->supplant`(update :From[M] := F) :Res = supplant(update)
		protected[sql] final def `->supplantAll`(updates :Seq[From[M] := F]) :Res =
			supplantAll(updates)
		...
		def supplant(setters :Seq[(M[From[M]], UnboundParam.Last[Arg]) => From[M] := F]) :Res = {
			val base = setDomain
			val row = table.row[From[M]]
			val param = base.last.mapping
	...
	class WhereParamScope[X, M[O] <: MappingAt[O]](val domain :From[M] WithParam X) {
		/** The row mapping for the modified table. */
		val row :M[RowProduct AndFrom M WithParam X] = domain.prev
		
		/** The mapping for the statement parameter `X`. 
		  * Same as [[net.noresttherein.oldsql.sql.TableStatement.WhereParamScope.param param]].
		  */
		val ? :UnboundParam[X, RowProduct AndFrom ParamRelation[X]#Param] = domain.last.mapping

		/** The mapping for the statement parameter `X`. 
		  * Same as [[net.noresttherein.oldsql.sql.TableStatement.WhereParamScope.? ?]].
		  */
		def param :UnboundParam[X, RowProduct AndFrom ParamRelation[X]#Param] = ?
	}
	...
	/** The API of factories - and complete [[net.noresttherein.oldsql.sql.Delete Delete]]`[Arg, M, Int]`
	  ...
	  *                               of the component.
	  * @define whereRowInfo         The function does not accept
...
//			where(condition(table.row).anchor(whereDomain))
		...
		def whereParam(condition :WhereParamScope[Arg, M] => SingleBoolean[WhereDomain]) :Res =
			where(condition(new WhereParamScope(whereDomain)).anchor(whereDomain))

		def whereParam(condition :(WhereParamScope[Arg, M], WhereParamScope[Arg, M]) => SingleBoolean[WhereDomain]) :Res = {
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade).anchor(base))
		}

		def whereParam(condition :(WhereParamScope[Arg, M], WhereParamScope[Arg, M], WhereParamScope[Arg, M])
		                          => SingleBoolean[WhereDomain]) :Res =
		{
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade, facade).anchor(base))
		}

		def whereParam(condition :(WhereParamScope[Arg, M], WhereParamScope[Arg, M], WhereParamScope[Arg, M],
		                           WhereParamScope[Arg, M])
		                          => SingleBoolean[WhereDomain]) :Res =
		{
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade, facade, facade).anchor(base))
		}

		def whereParam(condition :(WhereParamScope[Arg, M], WhereParamScope[Arg, M], WhereParamScope[Arg, M],
		                           WhereParamScope[Arg, M], WhereParamScope[Arg, M])
		                          => SingleBoolean[WhereDomain]) :Res =
		{
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade, facade, facade, facade).anchor(base))
		}

		def whereParam(condition :(WhereParamScope[Arg, M], WhereParamScope[Arg, M], WhereParamScope[Arg, M],
		                           WhereParamScope[Arg, M], WhereParamScope[Arg, M], WhereParamScope[Arg, M])
		                          => SingleBoolean[WhereDomain]) :Res =
		{
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade, facade, facade, facade, facade).anchor(base))
		}
		                          ...
		                          => SingleBoolean[WhereDomain]) :Res =
		{
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade, facade, facade, facade, facade, facade).anchor(base))
		}
		                          ...
		                          => SingleBoolean[WhereDomain]) :Res =
		{
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade, facade, facade, facade, facade, facade, facade).anchor(base))
		}
		                          ...
		                          => SingleBoolean[WhereDomain]) :Res =
		{
			val base = whereDomain
			val facade = new WhereParamScope(base)
			where(condition(facade, facade, facade, facade, facade, facade, facade, facade, facade).anchor(base))
		}
		...
		/** Creates a new, parameterless ''delete'' statement using the Boolean SQL
		  ...
		  * $whereRowInfo
		  *
		  * $whereRowOverloadInfo
		  * $examplesInfo
		  * @return a $DelLink $delResult
		  */ //an alternative name: whereOld
		def whereRow(condition :M[WhereDomain] => SingleBoolean[WhereDomain]) :Res =
			where(condition(table.row).anchor(whereDomain))
		...
		/** Creates a new, parameterless ''delete'' statement using the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def whereRow(condition :(M[WhereDomain], M[WhereDomain]) => SingleBoolean[WhereDomain]) :Res = {
			val mapping = table.row[WhereDomain]
			where(condition(mapping, mapping).anchor(whereDomain))
		}
		...
		/** Creates a new, parameterless ''delete'' statement using the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def whereRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain]) => SingleBoolean[WhereDomain]) :Res = {
			val mapping = table.row[WhereDomain]
		...
		/** Creates a new, parameterless ''delete'' statement using the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def whereRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain])
		                         => SingleBoolean[WhereDomain]) :Res =
		{
			val mapping = table.row[WhereDomain]
		...
		/** Creates a new, parameterless ''delete'' statement using the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def whereRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain])
		                         => SingleBoolean[WhereDomain]) :Res =
		{
			val mapping = table.row[WhereDomain]
		...
		{
			val mapping = table.row[WhereDomain]
			where(condition(mapping, mapping, mapping, mapping, mapping, mapping).anchor(whereDomain))
		}
		...
		{
			val mapping = table.row[WhereDomain]
			where(condition(mapping, mapping, mapping, mapping, mapping, mapping, mapping).anchor(whereDomain))
		}
		...
		{
			val mapping = table.row[WhereDomain]
			where(condition(mapping, mapping, mapping, mapping, mapping, mapping, mapping, mapping).anchor(whereDomain))
		}
		                         ...
		                         => SingleBoolean[WhereDomain]) :Res =
		{
			...
			where(condition(meta, meta, meta, meta, meta, meta, meta, meta, meta).anchor(whereDomain))
		}
	}
		...
		extends Any with WhereClauseDomain[Arg, M]
	{
		...
		/** Creates a new ''delete'' statement using a logical alternative of the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def orRow(condition :M[WhereDomain] => SingleBoolean[WhereDomain]) :Or =
			or(condition(table.row).anchor(whereDomain))
		...
		/** Creates a new ''delete'' statement using a logical alternative of the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def orRow(condition :(M[WhereDomain], M[WhereDomain]) => SingleBoolean[WhereDomain]) :Or =
		...
		/** Creates a new ''delete'' statement using a logical alternative of the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def orRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain]) => SingleBoolean[WhereDomain]) :Or = {
			val mapping = table.row[WhereDomain]
		...
		/** Creates a new ''delete'' statement using a logical alternative of the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def orRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain])
		...
		/** Creates a new ''delete'' statement using a logical alternative of the Boolean SQL
		  ...
		  * @return a $DelLink $delResult
		  */
		def orRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain])
		                      => SingleBoolean[WhereDomain]) :Or =
		{
			val mapping = table.row[WhereDomain]
		...
		extends Any with WhereAnyClauseFactory[Arg, M, Or]
	{
		...
		/** Creates a new ''delete'' statement with a new ''where'' clause,
		  ...
		  * @return a $DelLink $delResult
		  */
		def andRow(condition :M[WhereDomain] => SingleBoolean[WhereDomain]) :And =
			and(condition(table.row).anchor(whereDomain))
		...
		/** Creates a new ''delete'' statement with a new ''where'' clause,
		  ...
		  * @return a $DelLink $delResult
		  */
		def andRow(condition :(M[WhereDomain], M[WhereDomain]) => SingleBoolean[WhereDomain]) :And =
		...
		/** Creates a new ''delete'' statement with a new ''where'' clause,
		  ...
		  * @return a $DelLink $delResult
		  */
		def andRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain]) => SingleBoolean[WhereDomain]) :And = {
			val mapping = table.row[WhereDomain]
		...
		/** Creates a new ''delete'' statement with a new ''where'' clause,
		  ...
		  * @return a $DelLink $delResult
		  */
		def andRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain])
		...
		/** Creates a new ''delete'' statement with a new ''where'' clause,
		  ...
		  * @return a $DelLink $delResult
		  */
		def andRow(condition :(M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain], M[WhereDomain])
		                       => SingleBoolean[WhereDomain]) :And =
		{
			val mapping = table.row[WhereDomain]
			...
			:SingleBoolean[From[M] WithParam S] =
	{
		...
		//todo: probably should use primary key - or assume entity mappings buff everything else with NoFilterByDefault
