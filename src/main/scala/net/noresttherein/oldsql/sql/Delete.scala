...
object Delete { //todo: multiple unbound parameters with aliases, growing base FromClause.
	...
	def by[X] :DeleteParam[X] = new DeleteParam[X] {} //todo: make this accept a non-implicit form to differentiate it from Delete(table) by
	...
	object syntax {
			...
			extends AnyVal with WhereClauseFactory[Arg, M, DeleteWhereAll[Arg, S, M]]
		{
			...
			protected override def where(condition :SingleBoolean[From[M] WithParam Arg]) :DeleteWhereAll[Arg, S, M] =
			...
			extends ParamDelete[Arg, M] with WhereAnyClauseFactory[Arg, M, DeleteWhere[Arg, S, M]]
		{
			...
			override def or(condition :SingleBoolean[From[M] WithParam Arg]) :DeleteWhere[Arg, S, M] =
		...
		{
			override def and(condition :SingleBoolean[From[M] WithParam Arg]) :DeleteWhereAll[Arg, S, M] =
			...
			extends WhereClauseFactory[Arg, M, MultiDeleteWhereAll[Arg, S, M]]
		{
			...
			protected override def where(condition :SingleBoolean[From[M] WithParam Arg]) :MultiDeleteWhereAll[Arg, S, M] =
		...
		/** Shared implementation of [[net.noresttherein.oldsql.sql.Delete.syntax.MultiDeleteEntity MultiDeleteEntity]]
		  ...
		  * @tparam M       the mapping used by `table`.
		  */
		private[Delete] sealed trait MultiDelete[Arg, S, M[O] <: BaseMapping[S, O]] extends ParamDelete[Seq[Arg], M] {
		...
		/** A parameterized ''delete'' statement which contains `max` repetitions of the same condition matching
		  ...
		  * SQL Boolean [[net.noresttherein.oldsql.sql.SingleBoolean expression]] by `where` and `whereSelf` methods
...
//				mapper.deleteAny(this)
		...
		{
			protected override def or(condition :SingleBoolean[From[M] WithParam Arg]) :MultiDeleteWhere[Arg, S, M] =
		...
		{
			protected override def and(condition :SingleBoolean[WithParam[From[M], Arg]]) :MultiDeleteWhereAll[Arg, S, M] =
			...
			extends ParamDelete[S, M] with WhereClauseFactory[S, M, DeleteWhereAll[S, S, M]]
		{
			...
			protected override def where(condition :SingleBoolean[From[M] WithParam S]) :DeleteWhereAll[S, S, M] =
			...
			extends DeleteOne[S, M](table)
		{
			...
			/** A factory of statements parameterized with type `X`, using arbitrary
			  ...
			  * domain of the used [[net.noresttherein.oldsql.sql.SQLExpression SQLExpression]].
			  */
			def by[X :SQLForm] :DeleteWhereFactory[X, S, M] = new DeleteWhereFactory[X, S, M](From(table).param[X])
		}
			...
			extends AnyVal with WhereClauseFactory[Arg, M, DeleteManyWhereAll[Arg, S, M]]
		{
			...
			protected override def where(condition :SingleBoolean[From[M] WithParam Arg]) :DeleteManyWhereAll[Arg, S, M] =
		...
		/** A ''delete'' statement batch deleting all rows matching subsequent elements of the input collection `Seq[S]`
		  ...
		  * SQL Boolean [[net.noresttherein.oldsql.sql.SingleBoolean expression]] using `where` and `whereSelf` methods
			   ...
			   with WhereClauseFactory[S, M, DeleteManyWhereAll[S, S, M]]
		{
			...
			protected override def where(condition :SingleBoolean[WithParam[From[M], S]]) :DeleteManyWhereAll[S, S, M] =
				new DeleteManyWhereAll[S, S, M](dml.`->where`(condition))
//
...
//				dml.where(condition).batch
			   ...
			   with WhereAnyClauseFactory[Arg, M, DeleteManyWhere[Arg, S, M]]
		{
			...
			protected override def or(condition :SingleBoolean[From[M] WithParam Arg]) :DeleteManyWhere[Arg, S, M] =
		...
		{
			protected override def and(condition :SingleBoolean[From[M] WithParam Arg]) :DeleteManyWhereAll[Arg, S, M] =
