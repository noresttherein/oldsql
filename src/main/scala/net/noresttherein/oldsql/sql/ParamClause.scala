...
import net.noresttherein.oldsql.collection.{Opt, PassedArray, ReversedList, Unique}
...
//		   with FormBasedFactory[FunctionOf[P]#F, MappingAt[O]#TypedComponent, MappingAt[O]#Column]
	{
		...
		override object components
			extends EmptyUnique with Unique[Nothing] //EmptyUnique is protected
			   with FormBasedFactory[FunctionOf[P]#F, MappingAt[O]#TypedComponent, MappingAt[O]#TypedColumnComponent]
		{
...
//		override def equivalent(that :Mapping) :Boolean =
	...
	object UnboundParamSQL {
		...
		def unapply[F <: RowProduct, T[A] <: BaseMapping[E, A], E, M[A] <: BaseColumn[V, A], V, O >: F <: RowProduct]
...
object JoinParam {
	...
	private[sql] def apply[L <: FromSome, R[O] <: UnboundParam[X, O], X, A <: Label]
		...
		new JoinParam[clause.type, R] with AbstractExpanded[clause.type, R, X] {
			...
			override val last :LastRelation[R, X] { type FromLast = RowProduct AndFrom R } = param
