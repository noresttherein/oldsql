...
trait RowProduct extends RowProductTemplate[RowProduct] { thisClause =>
	...
	/** Subject types of the mappings of all relations in this clause, concatenated into a heterogeneous list.
	  ...
	  * Clauses with a [[net.noresttherein.oldsql.sql.GroupBy GroupBy]] component are treated specially:
	  * any mappings preceding the `GroupBy` extension class since the last
	...
	/** All relations in this clause as abstract, untyped mappings, in the reverse order of their appearance.
	  ...
	  * the [[net.noresttherein.oldsql.sql.RowProduct.FullRow FullRow]] type, which should be consistent with this method.
	  * @see [[net.noresttherein.oldsql.sql.RowProduct.fullRow]]
	  */
	...
	/** Number of relations contained in the explicit portion of this clause. This is equal to the number of mappings
	...
	/** All relations in the ''explicit'' section of this instance as generic, untyped mappings, in the reverse order
	  ...
	  * and `Implicit =:= RowProduct`), all not grouped relations are included. For the idea about what exact relations
	  * are included in the result, see the [[net.noresttherein.oldsql.sql.RowProduct.Row Row]] type,
...
//		new ProductRelation[SQLMapping.Expression[Generalized, GlobalScope, Row]#Projection, Row](this)
	...
	private[sql] def bridgeMatchWith[Y](matcher :RowProductMatcher[Y]) :Option[Y] = matchWith(matcher)

	protected def matchWith[Y](matcher :RowProductMatcher[Y]) :Option[Y] = matcher.rowProduct(this)
...
object RowProduct {
	...
	trait RowProductTemplate[+F <: RowProduct] { thisClause :F with RowProductTemplate[F] =>
		...
		/** The actual ''from'' and ''where'' clauses in this instance. Is an instance in which
	...
	type TopFrom = RowProduct { //consider: rename to TopRow
	...
	/** A ''from'' clause is a ''subclause'' of another clause `F` if its ''implicit'' portion is a supertype of `F`,
	  ...
	  */
	type SubclauseOf[-F <: RowProduct] = RowProduct {
...
//		type Paramless = Self
	...
	/** An upper bound for all ''from'' clauses which are known to contain at least one `JoinParam` 'join' in their
	  * static type. Note that a `RowProduct` not conforming to this type does not mean that it indeed contains
...
//		def apply(param :thisClause.LastParam) :thisClause.AppliedParam = thisClause.bind(param)
		...
		/** Checks if this clause InferTypeParams to [[net.noresttherein.oldsql.sql.RowProduct.TopFrom TopFrom]]
		  ...
		  */
		@inline def ifOuter[T](map :F { type Implicit = RowProduct } => T) :Option[T] =
		...
		/** Checks if this clause InferTypeParams to [[net.noresttherein.oldsql.sql.RowProduct.GroundFrom GroundFrom]]
		  ...
		  */
		@inline def ifFree[T](map :F with GroundFrom => T) :Option[T] =
