...
/** A [[java.sql.ResultSet ResultSet]] proxy which rearranges the columns of the underlying result set
  ...
  * [[net.noresttherein.oldsql.pixies.RearrangedIndexing RearrangedIndexing]]. It is the same as
  * [[net.noresttherein.oldsql.pixies.RearrangedResultSet RearrangedResultSet]], but assumes that the translation
  * is a [[net.noresttherein.oldsql.pixies.RearrangedIndexing.isInjection injection]], i.e. every read (exposed)
  ...
  *                    `indexing.`[[net.noresttherein.oldsql.pixies.RearrangedIndexing.apply RearrangedIndexing(_)]].
  ...
  */
private[pixies] class InjectionResultSet private[pixies] (rs :ResultSet, startOffset :Int, indices :RearrangedIndexing)
...
/** A [[java.sql.ResultSet ResultSet]] proxy which rearranges the columns of the underlying result set
  * according to the given index translation [[net.noresttherein.oldsql.pixies.RearrangedIndexing RearrangedIndexing]].
  * All access operations (`getXxx` and `updateXxx`) translate the given argument, 'exposed' index
  * to the underlying one before delegating to the same method of the adapted `ResultSet`.
  * The translation is calculated as:
  * {{{
  ...
  * @author Marcin Mościcki
  */
private[pixies] class RearrangedResultSet private (rs :ResultSet, startOffset :Int, indices :RearrangedIndexing)
	extends ResultSetProxy(rs)
{
...
private[oldsql] object RearrangedResultSet {
	/** A [[java.sql.ResultSet ResultSet]] proxy which rearranges the columns of the underlying result set
	  ...
	  */
	def apply(rs :ResultSet, startIndex :Int, indexing :RearrangedIndexing) :ResultSet =
	...
	def apply(rs :ResultSet, indexing :RearrangedIndexing) :ResultSet =
	...
	/** `ResultSetMetaData` proxy associated with [[net.noresttherein.oldsql.pixies.RearrangedResultSet RearrangedResultSet]].
	  ...
	  *                    to the mapping of the `RearrangedIndexing.apply` in order to obtain the actual index
	...
	private[oldsql] class RearrangedResultSetMetaData
	                      (startOffset :Int, indexing :RearrangedIndexing, meta :ResultSetMetaData)
