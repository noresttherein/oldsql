...
import net.noresttherein.oldsql.schema.Buff.{AutoInsert, AutoUpdate, BuffType, NoFilter, NoFilterByDefault, NoInsert, NoInsertByDefault, NoSelect, NoSelectByDefault, NoUpdate, NoUpdateByDefault, OptionalSelect, SelectPreset, Virtual}
...
import net.noresttherein.oldsql.sql.{RowProduct, SQLExpression, RowShape}
...
//		}
	...
	/** If two mappings `m1` and `m2` are homomorphic then they share the same subject and there is a bijection
	  ...
	  * @see [[net.noresttherein.oldsql.schema.ColumnMapping.ColumnSetExtension.homomorphic]]
...
//		(this isomorphic that) || (this != original || that != that.original) && (original homomorphic that.original)
	...
	/** If two mappings `m1` and `m2` are isomorphic then they share the same subject type and there is a bijection
	  ...
	  * @see [[net.noresttherein.oldsql.schema.ColumnMapping.ColumnSetExtension.isomorphic]]
	...
	/** Directly checks if this mapping recognizes the argument
	  ...
	  * `(this, `[[net.noresttherein.oldsql.schema.Mapping.original original]]`)`, `(original, that.orginal)` and
	...
	/** Two mappings are equivalent ''iff'' their [[net.noresttherein.oldsql.schema.Mapping.columns columns]],
	  ...
	  *   1. [[net.noresttherein.oldsql.schema.ColumnMapping.ColumnSetExtension.equivalent equivalent]]
...
//					included.columnsWithout(excludedBuff).map(counterpart(included, _))
		...
		def counterpart[T, O](other :MappingAt[O], component :TypedMapping[T, O]) :Comp[T, Origin] =
			if (component == other)
				...
				export(component.original.withOrigin[Origin])
			else if ((other isomorphic refine))
				subcomponents(other.subcomponents.sureIndexOf(other.export(component))).asInstanceOf[Comp[T, Origin]]
			else
				throw new IncompatibleMappingsException(
					"Cannot find a counterpart of component " + component + " from " + other + " in " + this + "."
				)
