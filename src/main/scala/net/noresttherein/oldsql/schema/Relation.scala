...
import net.noresttherein.oldsql.schema.bits.{ConstantMapping, ForeignKeyColumnMapping, JoinTableColumnsMapping, JoinTableMapping}
...
import net.noresttherein.oldsql.sql.ast.{BoundMappingParam, BoundParam, JoinedTable, MappingQuerySQL, QuerySQL, TableSQL}
import net.noresttherein.oldsql.sql.mechanics.{RelationOffset, SpelledSQL}
...
//		result(from, joined)
	...
	//todo: rename to `->defaultSpellingOf`
	...
	//todo: rename to `->defaultSpelling`
...
object Table {
	...
	implicit class TableExtension[M[O] <: MappingAt[O]](private val table :Table[M]) extends AnyVal {
		...
		/** An SQL expression for the entity mapped by this table when it is the last table listed by a ''from'' clause.
		  ...
		  * @return [[net.noresttherein.oldsql.sql.ast.TableSQL.LastTable TableSQL.LastTable]]`(this)`.
		  */
		def joined :JoinedTable[RowProduct AndFrom M, M] =
			table.expr.asInstanceOf[JoinedTable[RowProduct AndFrom M, M]]

		/** An SQL expression for the entity mapped by this table in an SQL ''select'' with ''from'' clause `F`.
		  * The table mapping must be the first one listed in type `F`.
		  */
		def at[F <: RowProduct](implicit index :RelationOffset[F, M] { type First = RowProduct AndFrom M })
				:JoinedTable[F, M] =
		{
			type T[O] = BaseMapping[Any, O]
			TableSQL[F, T, Any](
				table.castFrom[Table[M], Table[T]],
				index.castFrom[RelationOffset[F, M], RelationOffset[F, T] { type First = RowProduct AndFrom T }]
			).castFrom[JoinedTable[F, T], JoinedTable[F, M]]
		}
		...
		def from :From[M] = table.fromClause.asInstanceOf[From[M]]
	}
	...
	/** Extension methods of [[net.noresttherein.oldsql.schema.Table Table]]`[M]`
	  ...
	  */
	class TableExtraExtension[M[O] <: BaseMapping[S, O], S](private val table :Table[M]) extends AnyVal {
		...
		/** Creates a statement parameter whose [[net.noresttherein.oldsql.schema.SQLForm form]] is based on
		  * the mapping of this relation. Aside from providing the exact column composition of the parameter,
		  * the expression is a [[net.noresttherein.oldsql.sql.ast.MappingSQL MappingSQL]] and thus can be easily
		  * matched/aligned with another expression based on the same mapping.
		  */ //we don't have a variant for BoundColumnMappingParam, but they are not very useful.
		@inline def ?(value :S) :BoundMappingParam[M, S] =
			BoundMappingParam(table.row[Unit], value)
	}
	...
	@inline implicit def TableExtraExtension[M[O] <: MappingAt[O], T[O] <: BaseMapping[S, O], S]
			...
			:TableExtraExtension[T, S] =
		new TableExtraExtension[T, S](reveal(table))
...
//		def as[N <: Label](alias :N) :CommonTableAlias[N, M] = CommonTableAlias(alias, query)
...
//
//		def apply[M[O] <: MappingAt[O], V](select :MappingQuerySQL[RowProduct, M]) :TableExpression[M] =
//			new TableExpression[M] {
//				override type Row = M[Unit]#Subject
//				override val query = select
////				override def as[N <: Label](alias :N) :CommonTableAlias[N, M] = CommonTableAlias(alias, query)
//			}
...
object JoinTable {
	def apply[L[O] <: GenericEntityMapping[LM, LK, LE, O], LM[O] <: TypedMapping[LK, O], LE, LK,
	          R[O] <: GenericEntityMapping[RM, RK, RE, O], RM[O] <: TypedMapping[RK, O], RE, RK]
	         (name :String, buffs :Buffs[(Kin[LE], Kin[RE])] = Buffs.empty[(Kin[LE], Kin[RE])])
	         (left :RelVar[L], leftPrefix :String, leftBuffs :Buffs[Kin[LE]] = Buffs.empty[Kin[LE]])
	         (right :RelVar[R], rightPrefix :String, rightBuffs :Buffs[Kin[RE]] = Buffs.empty[Kin[RE]])
			:BaseTable[({ type M[O] = JoinTableMapping[L, LM, LK, Kin[LE], R, RM, RK, Kin[RE], (Kin[LE], Kin[RE]), O] })#M] =
		BaseTable(name,
			JoinTableMapping[L, LE, LM, LK, R, RE, RM, RK, Unit]
				            (left, (_ :L[Unit]).pk, leftPrefix + (_ :String), leftBuffs,
			                 right, (_ :R[Unit]).pk, rightPrefix + (_ :String), rightBuffs, buffs)
		)

	def columns[L[O] <: GenericEntityMapping[LM, LK, LE, O], LM[O] <: TypedColumn[LK, O], LE, LK,
	          R[O] <: GenericEntityMapping[RM, RK, RE, O], RM[O] <: TypedColumn[RK, O], RE, RK]
	         (name :String, buffs :Buffs[(Kin[LE], Kin[RE])] = Buffs.empty[(Kin[LE], Kin[RE])])
	         (left :RelVar[L], leftName :String, leftBuffs :Buffs[Kin[LE]] = Buffs.empty[Kin[LE]])
	         (right :RelVar[R], rightName :String, rightBuffs :Buffs[Kin[RE]] = Buffs.empty[Kin[RE]])
			:BaseTable[({ type M[O] = JoinTableColumnsMapping[L, LK, Kin[LE], R, RK, Kin[RE], (Kin[LE], Kin[RE]), O] })#M] =
		BaseTable(name,
			JoinTableColumnsMapping[L, LE, Kin[LE], LK, R, RE, Kin[RE], RK, Unit](
				ForeignKeyColumnMapping.kin[L, LK, LE, Unit, Unit](leftName, true, leftBuffs)(left, (_ :L[Unit]).pk),
				ForeignKeyColumnMapping.kin[R, RK, RE, Unit, Unit](rightName, true, rightBuffs)(right, (_ :R[Unit]).pk),
				buffs
			)
//			JoinTableMapping(left, (_ :L[Unit]).pk, leftName, leftBuffs,
//			                 right, (_ :R[Unit]).pk, rightName, rightBuffs, buffs)
		)
}
