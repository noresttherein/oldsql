...
trait RelatedMapping[S, O] extends BaseMapping[S, O] {
	...
	/** Default factory used in all foreign key and inverse foreign key mappings created by this mapping.
	  ...
	  * This can be overridden by subclasses and an alternative implementation is provided in the form of method
	...
	protected def tableKinFactory[M[A] <: TypedMapping[E, A], K, E, T]
	              (table :RelVar[M], key :M[_] => TypedMapping[K, _])
	              (implicit composition :T ComposedOf E, referencedType :TypeTag[E]) :KinFactory[K, E, T] =
		KinFactory.delay { TableKin.factory(table, key(table.row)).as[T] }
	...
	//todo: replace overloaded methods with factory objects. Will allow to have documentation in one place
	// and reduce the flooding of IDE hints with almost identical methods.
	...
	//consider: renaming to opt/option
...
//todo: rename supposed with presumed
...
*/
	...
	//todo: Seq, Map and custom collections. ComposedOf should refer to JE, not E
