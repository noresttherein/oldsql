...
import net.noresttherein.oldsql.schema.forms.{CustomColumnForm, DerivedMappedColumnForm, DerivedOptMappedColumnForm, LazyColumnForm, MappedColumnForm, OptMappedColumnForm, SQLForms}
...
object ColumnForm {
	...
	def jdbc[T :NullValue :ClassTag](name :String, sqlType :JDBCType = JDBCType.JAVA_OBJECT) :ColumnForm[T] = {
		val n = name
		new JDBCObjectForm[T](sqlType) with NamedColumnForm[T] {
			override val name = Got(n)
			override def text = name
			override def toString = string
			private[this] val string =
				if (this.sqlType == JDBCType.JAVA_OBJECT) text.get else text.get + "(" + this.sqlType + ")"
		}
	}
