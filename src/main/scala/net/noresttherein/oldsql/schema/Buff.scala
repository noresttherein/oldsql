...
trait Buff[T] extends Serializable {
	...
	/** Adapts this buff for a component type derived from this buff's type parameter. This method is used for
	  ...
	  */ //consider: a better name (derive? impose? adopt? bestow?)
...
object Buff {
	...
	/** This column/component is not included in the default select statement and must be specified explicitly,
	  * if at all possible. Unlike [[net.noresttherein.oldsql.schema.Buff.ExplicitSelect ExplicitSelect]],
	  * it ''does not'' provide a default value, so it is used only if
	  *   1. The enclosing component can deal with a situation in which a value for the annotated component cannot
	  *      be assembled;
	  *   1. To explicitly exclude a component already marked with
	  *      [[net.noresttherein.oldsql.schema.Buff.OptionalSelect OptionalSelect]];
	  *   1. On 'symlink' components - components wrapping a component already present in the some of the enclosing
	  *      components, outside the annotated component's subcomponent tree. This can be the case, for example,
	  *      with [[net.noresttherein.oldsql.schema.bits.ForeignKeyMapping inverse foreign key]] components,
	  *      which typically wrap the primary key of the outer entity.
	  *
	  * Implied by [[net.noresttherein.oldsql.schema.Buff.NoSelect NoSelect]],
	  * [[net.noresttherein.oldsql.schema.Buff.OptionalSelect OptionalSelect]], and
	  * [[net.noresttherein.oldsql.schema.Buff.NonDefault NonDefault]].
	  */
	...
	/** This column/component is not included in the default insert statement and must be specified explicitly,
	  * if at all possible. Is is implied [[net.noresttherein.oldsql.schema.Buff.ExplicitInsert ExplicitInsert]],
	  * and the framework checks for its presence, rather than the former, as it catches also situations in which
	  * a component already marked with an [[net.noresttherein.oldsql.schema.Buff.OptionalInsert OptionalInsert]],
	  * is explicitly excluded.
	  *
	  * Implied by [[net.noresttherein.oldsql.schema.Buff.NoInsert NoInsert]] and
	  * [[net.noresttherein.oldsql.schema.Buff.OptionalInsert OptionalInsert]],
	  * and [[net.noresttherein.oldsql.schema.Buff.NoWriteByDefault NoWriteByDefault]].
	  */
	...
	/** This column/component is not included in the default update statement and must be specified explicitly,
	  * if at all possible. Is is implied [[net.noresttherein.oldsql.schema.Buff.ExplicitUpdate ExplicitUpdate]],
	  * and the framework checks for its presence, rather than the former, as it catches also situations in which
	  * a component already marked with an [[net.noresttherein.oldsql.schema.Buff.OptionalUpdate OptionalUpdate]],
	  * is explicitly excluded.
	  *
	  * Implied by [[net.noresttherein.oldsql.schema.Buff.NoUpdate NoUpdate]],
	  * [[net.noresttherein.oldsql.schema.Buff.OptionalUpdate OptionalUpdate]], and
	  * [[net.noresttherein.oldsql.schema.Buff.NoWriteByDefault NoWriteByDefault]].
	  */
	...
	/** Excludes the given component from ''insert'' and ''update'' statements, unless explicitly included.
	  * It is implied by [[net.noresttherein.oldsql.schema.Buff.ExplicitWrite ExplicitWrite]], and the framework
	  * checks for its presence, rather than the latter, as it catches also situations such as excluding a component
	  * already marked with a [[net.noresttherein.oldsql.schema.Buff.OptionalWrite OptionalWrite]] buff.
	  *
	  * Implies: [[net.noresttherein.oldsql.schema.Buff.NoInsertByDefault NoInsertByDefault]] and
	  * [[net.noresttherein.oldsql.schema.Buff.NoUpdateByDefault NoUpdateByDefault]].
	  */
	case object NoWriteByDefault extends ComboBuffType(NoInsertByDefault, NoUpdateByDefault)
	...
	/** This column/component is never written to the database by the application. Note that 'read only' means here
	  ...
	  * [[net.noresttherein.oldsql.schema.Buff.NoWriteByDefault NoWriteByDefault]].
	  */
	case object ReadOnly extends ComboFlag(NoInsert, NoUpdate, NoWriteByDefault)
	...
	/** Marks a column/component as not included in the insert/update statements by default and needing to be included
	  ...
	  * [[net.noresttherein.oldsql.schema.Buff.OptionalWrite OptionalWrite]], and
	  * [[net.noresttherein.oldsql.schema.Buff.NoWriteByDefault NoWriteByDefault]]
	  */
	case object ExplicitWrite extends ComboFlag(ExplicitInsert, ExplicitUpdate, OptionalWrite, NoWriteByDefault)
	...
	/** Excludes the given component from all statements, unless explicitly included.
	  * This is roughly equivalent to [[net.noresttherein.oldsql.schema.Buff.Explicit Explicit]],
	  * except it ''does not'' provide a default value or any means of obtaining it. For this reason,
	  * it is used only if either the owner of the component can deal with a situation in which no value
	  * for the component exists, or to exclude a component already marked with
	  * a [[net.noresttherein.oldsql.schema.Buff.Optional Optional]] buff.
	  *
	  * Implies: [[net.noresttherein.oldsql.schema.Buff.NoSelectByDefault NoSelectByDefault]],
	  * [[net.noresttherein.oldsql.schema.Buff.NoFilterByDefault NoFilterByDefault]], and
	  * [[net.noresttherein.oldsql.schema.Buff.NoWriteByDefault NoWriteByDefault]].
	  */
	case object NonDefault extends ComboFlag(NoSelectByDefault, NoFilterByDefault, NoWriteByDefault)
	...
	/** A buff type which marks columns ignored by the enclosing mapping. It is useful when a column is still
	  ...
	  * [[net.noresttherein.oldsql.schema.Buff.NoSelect NoSelect]] and
	  * [[net.noresttherein.oldsql.schema.Buff.NonDefault NonDefault]].
	  * @see [[net.noresttherein.oldsql.schema.Buff.Virtual Virtual]]
	  * @see [[net.noresttherein.oldsql.schema.Buff.Partition Partition]]
	  */
	case object Ignored extends ComboFlag(ReadOnly, NoFilter, NoSelect, NonDefault)
	...
	/** Signifies that a column/component must be listed explicitly in order to be included in any database operation
	  ...
	  * [[net.noresttherein.oldsql.schema.Buff.ExplicitFilter ExplicitFilter]],
	  * [[net.noresttherein.oldsql.schema.Buff.ExplicitWrite ExplicitWrite]], and
	  * [[net.noresttherein.oldsql.schema.Buff.NonDefault NonDefault]].
	  */
	case object Explicit extends ComboValueBuffType(Optional, ExplicitSelect, ExplicitFilter, ExplicitWrite, NonDefault)
...
//	case object Extra extends ComboSQLBuffType(ExtraSelect, ExtraFilter, ExtraWrite)
	...
	/** Marks a component or column as a part of the owning table's primary key. */
	case object PrimaryKey extends FlagBuffType with Cascading with UnrelatedBuffType
...
//		}
	...
	/** Mixing trait for [[net.noresttherein.oldsql.schema.Buff.BuffType buff types]] which marks them
	  ...
	  */ //todo: rename to Branching or StemBuff, or DerivingBuff, or Inheritable, or Derivable
...
//			@inline def apply[T](buffs :Buffs[T]) :Opt[T] = buffs(ValueBuffType.this).map(_.value)
		...
		override def > :ValueBuffType = cascading
		private val cascading :ValueBuffType = if (cascades) this else ValueBuffType(toString + ">")(this)
	}
