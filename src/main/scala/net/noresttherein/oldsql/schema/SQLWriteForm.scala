...
import net.noresttherein.oldsql.collection.{ConstSeq, Opt, PassedArray}
...
import net.noresttherein.oldsql.schema.forms.{CaseSQLWriteForm, CustomOptSQLWriteForm, CustomSQLWriteForm, DerivedMappedSQLWriteForm, DerivedOptMappedSQLWriteForm, LazyWriteForm, MappedSQLWriteForm, OptMappedSQLWriteForm, ReorderedWriteForm, RepeatedSQLWriteForm, SQLForms, SQLWriteFormSeq, UnspecifiedForm}
...
trait SQLWriteForm[-T] extends UnspecifiedForm { outer =>
	...
	/** An adapter to this form which writes columns out of order, possibly only a subset, or ignoring some
	  ...
	  *
	  * Note that if `order` is not an [[net.noresttherein.oldsql.pixies.Rearrangement.isInjection injection]],
	  * the parameters set by this form will not be consecutive parameters of the underlying statement -
	  * some parameters within the form's translated column
	  * [[net.noresttherein.oldsql.pixies.Rearrangement.underlyingColumnCount range]] will be set
	  * automatically to `NULL` by the form. On the other hand, if the index translation
	  * is not a [[net.noresttherein.oldsql.pixies.Rearrangement.isSurjection surjection]],
	  * then the form's [[net.noresttherein.oldsql.schema.SQLReadForm.shape shape]] will omit some columns
	  * of this form's `shape`.
	  ...
	  *              Its [[net.noresttherein.oldsql.pixies.Rearrangement.columnCount columnCount]]
	  *              must equal this form's [[net.noresttherein.oldsql.schema.SQLWriteForm.columnCount columnCount]].
	  */
	def reorder(order :Rearrangement) :SQLWriteForm[T] = ReorderedWriteForm(this, order)
...
object SQLWriteForm { //todo: instead of columns :Int, take shape :RowShape
