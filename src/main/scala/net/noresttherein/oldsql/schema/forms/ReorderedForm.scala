...
import net.noresttherein.oldsql.pixies.{RearrangedCallableStatement, RearrangedIndexing, RearrangedPreparedStatement, RearrangedResultSet}
...
private[oldsql] trait ReorderedUnspecifiedForm extends UnspecifiedForm {
	...
	protected[oldsql] def indexing :RearrangedIndexing
...
object ReorderedReadForm {
	/** An adapter to an implicit `SQLReadForm[T]` applying it to [[java.sql.ResultSet ResultSet]]s
	  ...
	  * by `form` to column `indexing.`[[net.noresttherein.oldsql.pixies.RearrangedIndexing.apply indexing]]`(n)`
	  * in the wrapped `ResultSet`, or a virtual column returning always `null`
	  * if `!indexing.`[[net.noresttherein.oldsql.pixies.RearrangedIndexing.isMapped isMapped]]`(n)`.
	  ...
	  * `indexing.`[[net.noresttherein.oldsql.pixies.RearrangedIndexing.underlyingColumnCount underlyingColumnCount]].
	  */
	def apply[T :SQLReadForm](indexing :RearrangedIndexing) :SQLReadForm[T] =
	...
	/** An adapter to an `SQLReadForm[T]` applying it to [[java.sql.ResultSet ResultSet]]s
	  ...
	  * `indexing.`[[net.noresttherein.oldsql.pixies.RearrangedIndexing.underlyingColumnCount underlyingColumnCount]].
	  */
	def apply[T](form :SQLReadForm[T], indexing :RearrangedIndexing) :SQLReadForm[T] = {
		...
		class StandardReorderedReadForm(override val form :SQLReadForm[T], override val indexing :RearrangedIndexing)
	...
	def splice[T](form :SQLReadForm[T], gaps :IndexedSeq[(Int, Int)]) :SQLReadForm[T] =
		ReorderedReadForm(form, RearrangedIndexing.splice(gaps, form.columnCount))
	...
	def shuffle[T](form :SQLReadForm[T], permutation :IndexedSeq[Int]) :SQLReadForm[T] =
		if (permutation.length != form.columnCount)
			...
			ReorderedReadForm(form, RearrangedIndexing.permutation(permutation))
	...
	def inverse[T](form :SQLReadForm[T], permutation :IndexedSeq[Int]) :SQLReadForm[T] =
		if (permutation.length != form.columnCount)
	        ...
	        ReorderedReadForm(form, RearrangedIndexing.inverse(permutation))
}
...
object ReorderedWriteForm {
	def apply[T :SQLWriteForm](indexing :RearrangedIndexing) :SQLWriteForm[T] =
		ReorderedWriteForm(SQLWriteForm[T], indexing)

	def apply[T](form :SQLWriteForm[T], indexing :RearrangedIndexing) :SQLWriteForm[T] = {
	...
	def splice[T](form :SQLWriteForm[T], gaps :IndexedSeq[(Int, Int)]) :SQLWriteForm[T] =
		ReorderedWriteForm(form, RearrangedIndexing.splice(gaps, form.columnCount))
	...
	def shuffle[T](form :SQLWriteForm[T], permutation :IndexedSeq[Int]) :SQLWriteForm[T] =
		if (permutation.length != form.columnCount)
			...
			ReorderedWriteForm(form, RearrangedIndexing.permutation(permutation))
}




private[oldsql] class ReorderedForm[T](override val form :SQLForm[T], override val indexing :RearrangedIndexing)
...
object ReorderedForm {
	def apply[T :SQLForm](indexing :RearrangedIndexing) :SQLForm[T] =
		ReorderedForm(SQLForm[T], indexing)

	def apply[T](form :SQLForm[T], indexing :RearrangedIndexing) :SQLForm[T] =
	...
	def splice[T](form :SQLForm[T], gaps :IndexedSeq[(Int, Int)]) :SQLForm[T] =
		ReorderedForm(form, RearrangedIndexing.splice(gaps, form.columnCount))
	...
	def shuffle[T](form :SQLForm[T], permutation :IndexedSeq[Int]) :SQLForm[T] =
		if (permutation.length != form.columnCount)
			...
			ReorderedForm(form, RearrangedIndexing.permutation(permutation))
}
