...
object ColumnMapping extends Rank1ColumnMappingImplicits {
	...
	@inline implicit def columnMappingOriginProjection[M <: ColumnOf[S], S](implicit subject :M <:< ColumnOf[S])
			:OriginProjection[M, S] with ExactProjection[M] { type WithOrigin[O] = BaseColumn[S, O] } =
		OriginProjection.projectAs[M, ColumnOf[S]#TypedColumnProjection]
...
//		protected final val superUpdatedByDefault  :Unique[Col[_, O]] = super.updatedByDefault
		...
		override def apply[T](column :Column[T]) :like[Col]#Extract[T] = apply(column :Component[T])
	}
