...
import net.noresttherein.oldsql.schema.bits.LabelPath.{/, ~/, Concat, Label, LabelPathPrefix, Split}
...
/** A type class marking the type `P` as a ''label path'' - a sequence of one or more `Label`s (string literal types)
  ...
  *      through type alias [[net.noresttherein.oldsql.schema.bits.LabelPath.Label Label]]s;
  *   1. All types `P `[[net.noresttherein.oldsql.schema.bits.LabelPath./ /]]` L` such that
  ...
  *      and a `LabelPath`. Importing the [[net.noresttherein.oldsql.schema.bits.LabelPath./ /]] type
  ...
  * starting with object [[net.noresttherein.oldsql.schema.bits.LabelPath.~/ ~/]] and continuing with
  * [[net.noresttherein.oldsql.schema.bits.LabelPath.LabelPathPrefix./ /]].
  *
...
sealed trait LabelPath[P] extends Any with LabelPathPrefix {
	...
	@inline final def /[A, B <: Label](next :A / B)(implicit concat :Concat[P, A / B]) :concat.Path =
		concat.result

	/** Creates a new path being an inlined concatenation of this path and the suffix path `S`
	  * provided as an implicit parameter. */
	@inline final def /[S](implicit suffix :LabelPath[S], concat :Concat[P, S]) :concat.Path = concat.result

	@inline final def split(implicit split :Split[P]) :(split.First, split.Suffix) =
		(split.first, split.suffix.path)

	override def toSeq :Seq[Label] = toList

	override def toList :List[Label] = {
...
//todo: move to morsels or collection (together with Listing)
object LabelPath {
	...
	def fromSeq(path :Seq[String]) :LabelPath[_] = path match {
		...
		case list :LinearSeq[String] =>
			def rec(parent :LabelPath[_], rest :LinearSeq[String]) :LabelPath[_] =
			...
			rec(~/(hd), list.tail)
		case indexed :IndexedSeq[String] =>
	...
	/** Creates a `LabelPath` for a given `String` representation.
	  ...
	  * ([[net.noresttherein.oldsql.schema.bits.LabelPath.~/ ~/]]). Otherwise each its occurrence delimits the next
	  * label in the path.
	  * @param path a non empty `String`.
	  * @param separator any chracter to use as a separator (`/` if not specified).
	  */
	...
	/** A common super type of [[net.noresttherein.oldsql.schema.bits.LabelPath LabelPath]]
	  * and constructor object [[net.noresttherein.oldsql.schema.bits.LabelPath.~/$ ~/]],
	  * serving as a 'possibly empty path'.
	  */
	sealed trait LabelPathPrefix extends Any with Serializable {
		/** True if this prefix is non empty, i.e., it is a [[net.noresttherein.oldsql.schema.bits.LabelPath LabelPath]]. */
		final def nonEmpty :Boolean = this.isInstanceOf[LabelPath[_]]

		/** True if this prefix is empty, i.e., it equals [[net.noresttherein.oldsql.schema.bits.LabelPath.~/ ~/]]. */
	...
	class ~/[L <: Label] private[LabelPath] (val last :L) extends AnyVal with LabelPath[L] {
		...
		override def reverseList :List[Label] = last::Nil
	...
	sealed trait /[P, L <: Label] extends LabelPath[P / L] {
		...
		override def reverseList :List[Label] = last::init.reverseList
...
//		def apply[L <: Label](label :L): ~/[L] = ~/(label)
	...
	/** An implicit ''conversion'' from a string singleton to a single element path. Can be also used to create
	  ...
	  * to use [[net.noresttherein.oldsql.schema.bits.LabelPath!./ LabelPath./]] as an extension method of `Label`
	...
	@inline implicit def concatWithLabel[A :LabelPath, B <: Label :ValueOf] :Concat[A, B] { type Path = A / B } =
		new Composite[A, B](LabelPath[A], valueOf[B]) with Concat[A, B] {
			override type Path = A / B
			@inline override def typeClass :A / B = this
		}

	@inline implicit def concatWithPath[A, B, C <: Label :ValueOf](implicit concat :Concat[A, B])
			:Concat[A, B / C] { type Path = concat.Path / C } =
		new Composite[concat.Path, C](concat.typeClass, valueOf[C]) with Concat[A, B / C] {
	...
	@inline implicit def splitIntoTwoLabels[A <: Label, B <: Label](implicit a :ValueOf[A], b :ValueOf[B])
		...
		new Split[A/B] {
			...
			override def join(first :A, suffix :B) :A / B = (first :A) / suffix
		}

	@inline implicit def splitIntoLabelAndPath[P, L <: Label](implicit split :Split[P], last :ValueOf[L])
		...
		new Split[P / L] {
			...
			override def join(first :split.First, suffix :Suffix) :LabelPath[P / L] =
				split.join(first, suffix.prefix) / last.value
		}
