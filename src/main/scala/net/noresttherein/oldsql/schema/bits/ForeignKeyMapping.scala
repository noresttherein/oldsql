...
import net.noresttherein.oldsql.model.{GenericKinFactory, Kin, KinFactory, RelatedEntityFactory}
...
import net.noresttherein.oldsql.schema.{Buff, Buffs, ColumnExtract, ColumnForm, ColumnMapping, Mapping, MappingExtract, RelVar, composeColumnExtracts, composeExtracts}
...
object ForeignKeyMapping {
	...
	/** Create a mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  * @param buffs buffs of the created mapping.
	  */
	def apply[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, T, R <: Kin[T], MO, O]
	         (rename :String => String, factory :GenericKinFactory[K, E, T, R], buffs :Buffs[R])
	         (table :RelVar[M], pk :M[MO] => C[MO]) :ForeignKeyMapping[M, C, K, R, O] =
		apply(rename, factory.narrow, buffs)(table, pk)
	...
	/** Create a mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  * @param buffs buffs of the created mapping.
	  */
	def apply[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, T, R <: Kin[T], X, O]
	         (rename :String => String, factory :GenericKinFactory[K, E, T, R], buffs :Buff[R]*)
	         (table :RelVar[M], pk :M[X] => C[X]) :ForeignKeyMapping[M, C, K, R, O] =
		apply(rename, factory.narrow, buffs :_*)(table, pk)
	...
	/** Create a mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  */
	def apply[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, T, R, MO, O]
	         (columnPrefix :String, factory :RelatedEntityFactory[K, E, T, R], buffs :Buffs[R])
	         (table :RelVar[M], pk :M[MO] => C[MO]) :ForeignKeyMapping[M, C, K, R, O] =
		apply[M, C, K, E, T, R, MO, O](columnPrefix + _, factory, buffs)(table, pk)
	...
	/** Create a mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  */
	def apply[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, T, R <: Kin[T], MO, O]
	         (columnPrefix :String, factory :GenericKinFactory[K, E, T, R], buffs :Buffs[R])
	         (table :RelVar[M], pk :M[MO] => C[MO]) :ForeignKeyMapping[M, C, K, R, O] =
		apply(columnPrefix, factory.narrow, buffs)(table, pk)
	...
	/** Create a mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  */
	def apply[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, T, R <: Kin[T], MO, O]
	         (columnPrefix :String, factory :GenericKinFactory[K, E, T, R], buffs :Buff[R]*)
	         (table :RelVar[M], pk :M[MO] => C[MO]) :ForeignKeyMapping[M, C, K, R, O] =
		apply(columnPrefix, factory.narrow, buffs :_*)(table, pk)



	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`.
	  * Referencing happens by matching the subjects `K` of the component from `table`, returned by function `pk`,
	  * (which needs not to be a primary key), and the single direct component of the returned mapping.
	  * The `Kin` created are [[net.noresttherein.oldsql.schema.bits.TableKin TableKin]].
	  *
	  * @tparam M  the mapping for the referenced table, mapped to type `E`.
	  * @tparam C  the mapping for the referenced component in `table`, mapped to type `K`.
	  * @tparam K  the key type: the subject type of both the local key (subcomponent of the returned mapping)
	  ...
	  * @param rename  a function mapping column names from the referenced component to column names
	  *                in the created mapping.
	  * @param notNull is the underlying foreign key nullable?
	  *                If so, `TableKin.`[[net.noresttherein.oldsql.schema.bits.TableKin.nonexistent nonexistent]]
	  *                will be used for `null` keys.
	  * @param buffs   buffs of the created mapping.
	  */
	def kin[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (rename :String => String, notNull :Boolean, buffs :Buffs[Kin[E]])(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Kin[E], O] =
	{
		val factory =
			if (notNull) KinFactory.delay(TableKin.required(table, pk(table[MO])))
			else KinFactory.delay(TableKin.factory(table, pk(table[MO])))
		apply[M, C, K, E, E, Kin[E], MO, O](rename, factory, buffs)(table, pk)
	}
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`.
	  ...
	  */
	def kin[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (rename :String => String, notNull :Boolean, buffs :Buff[Kin[E]]*)(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Kin[E], O] =
		kin(rename, notNull, Buffs(buffs:_*))(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`.
	  ...
	  */
	def kin[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (columnPrefix :String, notNull :Boolean = true, buffs :Buffs[Kin[E]] = Buffs.empty[Kin[E]])
	       (table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Kin[E], O] =
		kin(columnPrefix + _, notNull, buffs)(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`.
	  ...
	  */
	def kin[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (columnPrefix :String, notNull :Boolean, buffs :Buff[Kin[E]]*)(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Kin[E], O] =
		kin(columnPrefix + _, notNull, Buffs(buffs :_*))(table, pk)



	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  * through a non null foreign key. Referencing happens by matching the subjects `K` of the component from `table`,
	  * returned by function `pk`, (which needs not to be a primary key), and the single direct component
	  * of the returned mapping. The `Kin` created are [[net.noresttherein.oldsql.schema.bits.TableKin TableKin]].
	  *
	  * @tparam M  the mapping for the referenced table, mapped to type `E`.
	  * @tparam C  the mapping for the referenced component in `table`, mapped to type `K`.
	  * @tparam K  the key type: the subject type of both the local key (subcomponent of the returned mapping)
	  ...
	  * @param buffs   buffs of the created mapping.
	  */
	def one[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (rename :String => String, buffs :Buffs[One[E]])(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, One[E], O] =
		apply(rename, KinFactory.delay(TableKin.required(table, pk(table[MO]))).narrow, buffs)(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def one[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (rename :String => String, buffs :Buff[One[E]]*)(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, One[E], O] =
		one(rename, Buffs(buffs:_*))(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def one[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (columnPrefix :String, buffs :Buffs[One[E]])(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, One[E], O] =
		one(columnPrefix + _, buffs)(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def one[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (columnPrefix :String, buffs :Buff[One[E]]*)(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, One[E], O] =
		one(columnPrefix + _, Buffs(buffs :_*))(table, pk)



	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  * through a nullable foreign key. Referencing happens by matching the subjects `K` of the component from `table`,
	  * returned by function `pk`, (which needs not to be a primary key), and the single direct component
	  * of the returned mapping. The `Kin` created are [[net.noresttherein.oldsql.schema.bits.TableKin TableKin]].
	  *
	  * @tparam M  the mapping for the referenced table, mapped to type `E`.
	  ...
	  * @param buffs   buffs of the created mapping.
	  */
	def opt[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (rename :String => String, buffs :Buffs[Option[One[E]]])(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Option[One[E]], O] =
		apply(rename, KinFactory.delay(TableKin.required(table, pk(table[MO]))).narrow.optional, buffs)(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def opt[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (rename :String => String, buffs :Buff[Option[One[E]]]*)(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Option[One[E]], O] =
		opt(rename, Buffs(buffs:_*))(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def opt[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (columnPrefix :String, buffs :Buffs[Option[One[E]]])(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Option[One[E]], O] =
		opt(columnPrefix + _, buffs)(table, pk)
	...
	/** Create a mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def opt[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, MO, O]
	       (columnPrefix :String, buffs :Buff[Option[One[E]]]*)(table :RelVar[M], pk :M[MO] => C[MO])
			:ForeignKeyMapping[M, C, K, Option[One[E]], O] =
		opt(columnPrefix + _, Buffs(buffs :_*))(table, pk)
	...
	/** Create a column mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  * @param buffs buffs for the created column.
	  */
	def column[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	          (name :String, factory :GenericKinFactory[K, E, T, R], buffs :Buffs[R])
	          (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO]) :ForeignKeyColumnMapping[M, K, R, O] =
		column(name, factory.narrow, buffs)(table, pk)
	...
	/** Create a column mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  */
	def column[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	          (name :String, factory :GenericKinFactory[K, E, T, R], buffs :Buff[R]*)
	          (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO]) :ForeignKeyColumnMapping[M, K, R, O] =
		column(name, factory.narrow, buffs :_*)(table, pk)
	...
	/** Create a mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  *
	  * Note that, in order to avoid including the `key` component multiple times in every database operation
	  * (which, in case of ''insert'' and ''update'' would result in an error), the component should be marked
	  * with [[net.noresttherein.oldsql.schema.Buff.NonDefault NonDefault]] buff, or similar.
	...
	/** Create a mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  * @param fk a function returning the foreign key subcomponent of the table referenced by the created mapping.
	  */
	def inverse[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, T, R <: Kin[T], MO, O]
	           (key :C[O], factory :GenericKinFactory[K, E, T, R], buffs :Buffs[R])
	           (table :RelVar[M], fk :M[MO] => ForeignKeyMapping[MappingAt, C, K, _, MO])
			:JoinedEntityComponent[M, C, K, R, O] =
		inverse(key, factory.narrow, buffs)(table, fk)
	...
	/** Create a mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  */
	def inverse[M[A] <: TypedMapping[E, A], C[A] <: TypedMapping[K, A], K, E, T, R <: Kin[T], MO, O]
	           (key :C[O], factory :GenericKinFactory[K, E, T, R], buffs :Buff[R]*)
	           (table :RelVar[M], fk :M[MO] => ForeignKeyMapping[MappingAt, C, K, _, MO])
			:JoinedEntityComponent[M, C, K, R, O] =
		inverse[M, C, K, E, T, R, MO, O](key, factory.narrow, buffs :_*)(table, fk)
	...
	/** Create a mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  * containing the original referenced key.
	  *
	  * Note that, in order to avoid including the `key` column multiple times in every database operation
	  * (which, in case of ''insert'' and ''update'' would result in an error), `buffs` should contain
	  * a [[net.noresttherein.oldsql.schema.Buff.NonDefault NonDefault]] buff, or similar.
	  * @tparam M the mapping type for rows of the table containing the foreign key mapping referencing `key`.
	  * @tparam K the key type: the subject of the referenced key,
	  *           the [[net.noresttherein.oldsql.schema.bits.ForeignKeyMapping.key key]] component of the foreign key
	  *           returned by `fk`, and the `key` component of the returned mapping.
	...
	/** Create a mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  * @param fk a function returning the foreign key column of the table referenced by the created column.
	  */
	def inverseColumn[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	                 (key :TypedColumn[K, O], factory :GenericKinFactory[K, E, T, R], buffs :Buffs[R])
	                 (table :RelVar[M], fk :M[MO] => ForeignKeyColumnMapping[MappingAt, K, _, MO])
			:JoinedEntityColumn[M, K, R, O] =
		inverseColumn(key, factory, buffs)(table, fk)
	...
	/** Create a mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  */
	def inverseColumn[M[A] <: TypedMapping[E, A], K, E, T, R, MO, O]
	                 (key :TypedColumn[K, O], factory :RelatedEntityFactory[K, E, T, R], buffs :Buff[R]*)
	                 (table :RelVar[M], fk :M[MO] => ForeignKeyColumnMapping[MappingAt, K, _, MO])
			:JoinedEntityColumn[M, K, R, O] =
		new InverseForeignKeyColumnMapping[M, K, E, T, R, MO, O](key, factory, buffs)(table, fk)
	...
	/** Create a mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  */
	def inverseColumn[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	                 (key :TypedColumn[K, O], factory :GenericKinFactory[K, E, T, R], buffs :Buff[R]*)
	                 (table :RelVar[M], fk :M[MO] => ForeignKeyColumnMapping[MappingAt, K, _, MO])
			:JoinedEntityColumn[M, K, R, O] =
		inverseColumn(key, factory, buffs :_*)(table, fk)
...
object ForeignKeyColumnMapping {
	/** Create a column mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  * as some type `X` derived from `E`. Referencing happens by matching the key type `K` of the column
	  * from `table` returned by function `pk` (which needs not to be a primary key), and the single 'subcolumn'
	  * of the returned column. See the documentation of
	  ...
	  * @param buffs buffs for the created column.
	  */
	def apply[M[A] <: TypedMapping[E, A], K, E, T, R, MO, O]
	         (name :String, factory :RelatedEntityFactory[K, E, T, R], buffs :Buffs[R])
	         (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO]) :ForeignKeyColumnMapping[M, K, R, O] =
		ForeignKeyMapping.column(name, factory, buffs)(table, pk)
	...
	/** Create a column mapping for type `R`, referencing the subjects `E` of `table` with mapping `M[_]`
	  ...
	  * @param buffs buffs for the created column.
	  */
	def apply[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	         (name :String, factory :GenericKinFactory[K, E, T, R], buffs :Buffs[R])
	         (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO]) :ForeignKeyColumnMapping[M, K, R, O] =
		apply(name, factory.narrow, buffs)(table, pk)
	...
	def apply[M[A] <: TypedMapping[E, A], K, E, T, R, MO, O]
	         (name :String, factory :RelatedEntityFactory[K, E, T, R], buffs :Buff[R]*)
	         (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO]) :ForeignKeyColumnMapping[M, K, R, O] =
		ForeignKeyMapping.column(name, factory, buffs :_*)(table, pk)
	...
	def apply[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	         (name :String, factory :GenericKinFactory[K, E, T, R], buffs :Buff[R]*)
	         (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO]) :ForeignKeyColumnMapping[M, K, R, O] =
		apply(name, factory.narrow, buffs :_*)(table, pk)



	/** Create a column mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  * through a foreign key column. Referencing happens by matching the subjects `K` of the column from `table`
	  * returned by function `pk` (which needs not to be a primary key), and the single direct column
	  * of the returned mapping. The `Kin` created are [[net.noresttherein.oldsql.schema.bits.TableKin TableKin]].
	  *
	  * @tparam M  the mapping for the referenced table, mapped to type `E`.
	  * @tparam K  the key type: the subject type of both the local key (subcomponent of the returned mapping)
	  ...
	  * @param buffs   buffs of the created mapping.
	  */
	def kin[M[A] <: TypedMapping[E, A], K, E, MO, O]
	       (name :String, notNull :Boolean, buffs :Buffs[Kin[E]])
	       (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO]) :ForeignKeyColumnMapping[M, K, Kin[E], O] =
	{
		val factory =
			if (notNull) KinFactory.delay(TableKin.required(table, pk(table[MO])))
			else KinFactory.delay(TableKin.factory(table, pk(table[MO])))
		apply[M, K, E, E, Kin[E], MO, O](name, factory, buffs)(table, pk)
	}
	...
	/** Create a column mapping for type `Kin[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def kin[M[A] <: TypedMapping[E, A], K, E, MO, O]
	       (name :String, notNull :Boolean, buffs :Buff[Kin[E]]*)
	       (table :RelVar[M], pk :M[MO] => TypedColumn[K, MO])
			:ForeignKeyColumnMapping[M, K, Kin[E], O] =
		kin[M, K, E, MO, O](name, notNull, Buffs(buffs :_*))(table, pk)


	/** Create a column mapping for type `One[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  * through a non null foreign key column. Referencing happens by matching the subjects `K` of the column
	  * from `table` returned by function `pk` (which needs not to be a primary key), and the single direct column
	  * of the returned mapping. The `Kin` created are [[net.noresttherein.oldsql.schema.bits.TableKin TableKin]].
	  * @tparam M  the mapping for the referenced table, mapped to type `E`.
	  * @tparam K  the key type: the subject type of both the local key (subcomponent of the returned mapping)
	  *            and referenced key in `table` (returned by `pk`).
	  ...
	  * @param buffs   buffs of the created mapping.
	  */
	def one[M[A] <: TypedMapping[E, A], K, E, MO, O]
	       (name :String, buffs :Buffs[One[E]])(table :RelVar[M], pk :M[MO] => TypedColumn[K, MO])
			:ForeignKeyColumnMapping[M, K, One[E], O] =
		apply(name, KinFactory.delay(TableKin.required(table, pk(table[MO]))), buffs)(table, pk)
	...
	/** Create a column mapping for type `One[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  */
	def one[M[A] <: TypedMapping[E, A], K, E, MO, O]
	       (name :String, buffs :Buff[One[E]]*)(table :RelVar[M], pk :M[MO] => TypedColumn[K, MO])
			:ForeignKeyColumnMapping[M, K, One[E], O] =
		one(name, Buffs(buffs:_*))(table, pk)
	...
	/** Create a column mapping for type `One[E]`, referencing an entity mapped to `table` with mapping `M[_]`
	  ...
	  * @param buffs   buffs of the created mapping.
	  */
	def opt[M[A] <: TypedMapping[E, A], K, E, MO, O]
	       (name :String, buffs :Buffs[Option[One[E]]])(table :RelVar[M], pk :M[MO] => TypedColumn[K, MO])
			:ForeignKeyColumnMapping[M, K, Option[One[E]], O] =
		apply(name, KinFactory.delay(TableKin.required(table, pk(table[MO]))).narrow.optional, buffs)(table, pk)

	/** Create a column mapping for type `Option[One[E]]`, referencing an entity mapped to `table` with mapping `M[_]`
	  * through a nullable foreign key column. Referencing happens by matching the subjects `K` of the column
	  * from `table` returned by function `pk` (which needs not to be a primary key), and the single direct column
	  * of the returned mapping. The `Kin` created are [[net.noresttherein.oldsql.schema.bits.TableKin TableKin]].
	  * @tparam M  the mapping for the referenced table, mapped to type `E`.
	  * @tparam K  the key type: the subject type of both the local key (subcomponent of the returned mapping)
	  ...
	  */
	def opt[M[A] <: TypedMapping[E, A], K, E, MO, O]
	       (name :String, buffs :Buff[Option[One[E]]]*)(table :RelVar[M], pk :M[MO] => TypedColumn[K, MO])
			:ForeignKeyColumnMapping[M, K, Option[One[E]], O] =
		opt(name, Buffs(buffs:_*))(table, pk)


	/** Create a column mapping for the other side of a relationship defined by a foreign key mapping.
	  * It will, similarly to the original foreign key mapping, use some reference type `R` to the table containing
	  * the foreign key column. It represents following the referenced 'primary key' `key` to all rows
	  * containing a matching foreign key. The returned mapping can only be used in the table
	  * containing the original referenced key.
	  ...
	  * @tparam R the reference type created by the passed factory and the subject type of the created column.
	  * @tparam O the origin type of the created mapping and the whole table mapping containing the referenced key.
	  * @param key the original key column (typically the primary key) referenced by the foreign key returned by `fk`.
	  * @param factory a factory for references to collections of the entities mapped to `table`.
	  * @param buffs the buffs for the created column. As the column will have `key` as its subcolumn, most likely
	  *              duplicating its occurrence in the owning table, it almost certainly
	  *              should contain [[net.noresttherein.oldsql.schema.Buff.Ignored Ignored]] buff.
	  ...
	  * @param fk a function returning the foreign key column of the table referenced by the created column.
	  */
	def inverse[M[A] <: TypedMapping[E, A], K, E, T, R, MO, O]
	           (key :TypedColumn[K, O], factory :RelatedEntityFactory[K, E, T, R], buffs :Buffs[R])
	           (table :RelVar[M], fk :M[MO] => ForeignKeyColumnMapping[MappingAt, K, _, MO])
			:JoinedEntityColumn[M, K, R, O] =
		ForeignKeyMapping.inverseColumn(key, factory, buffs)(table, fk)
	...
	/** Create a column mapping for the other side of a relationship defined by a foreign key mapping.
	  ...
	  * @param fk a function returning the foreign key column of the table referenced by the created column.
	  */
	def inverse[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	           (key :TypedColumn[K, O], factory :GenericKinFactory[K, E, T, R], buffs :Buffs[R])
	           (table :RelVar[M], fk :M[MO] => ForeignKeyColumnMapping[MappingAt, K, _, MO])
			:JoinedEntityColumn[M, K, R, O] =
		inverse(key, factory, buffs)(table, fk)
	...
	def inverse[M[A] <: TypedMapping[E, A], K, E, T, R, MO, O]
	           ...
	           (table :RelVar[M], fk :M[MO] => ForeignKeyColumnMapping[MappingAt, K, _, MO])
			:JoinedEntityColumn[M, K, R, O] =
		ForeignKeyMapping.inverseColumn(key, factory, buffs :_*)(table, fk)
	...
	def inverse[M[A] <: TypedMapping[E, A], K, E, T, R <: Kin[T], MO, O]
	           ...
	           (table :RelVar[M], fk :M[MO] => ForeignKeyColumnMapping[MappingAt, K, _, MO])
			:JoinedEntityColumn[M, K, R, O] =
		inverse(key, factory, buffs :_*)(table, fk)
