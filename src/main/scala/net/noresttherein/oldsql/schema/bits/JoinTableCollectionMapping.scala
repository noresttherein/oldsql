...
import net.noresttherein.oldsql.schema.{composeExtracts, Buffs, MappingExtract, RelVar}
...
/** A mapping interface for join tables used to implement ''many-to-many'' relationships.
  ...
  * @tparam EL the 'left' entity in the relationship, the subject type of its mapping `L`.
  * @tparam R  the mapping of the referenced table of the 'right' side of this relationship.
  * @tparam ER the 'right' entity in the relationship, the subject type of its mapping `L`.
  ...
  */
trait JoinTableMapping[L[A] <: TypedMapping[EL, A], R[A] <: TypedMapping[ER, A],
                       CL[A] <: TypedMapping[KL, A], CR[A] <: TypedMapping[KR, A], KL, KR, EL, ER, S, O]
...
/** Base trait for mappings of standard ''many-to-many'' relationships using a join table, that is one where
  ...
  * (at least to the minimal required extent) solely based on the relationship. While a component of an entity
  * on one end of the relationship, the underlying columns are already typically already mapped by another component `C`
  * (like the table's primary key), and thus it is a candidate for marking with buffs preventing from its duplicate
  * inclusion in ''insert'' and ''update'' statements. Instead, update of the value of this component triggers
  * a cascading ''delete''/''insert'' to the ''join table'' with mapping `J`. It also specifies the arity and
