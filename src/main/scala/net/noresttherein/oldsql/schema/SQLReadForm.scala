...
import java.sql.{CallableStatement, JDBCType, ResultSet, SQLException}
...
import net.noresttherein.oldsql.pixies.{CallableStatementOutParams, Rearrangement}
import net.noresttherein.oldsql.schema.SQLForm.NullValue
import net.noresttherein.oldsql.schema.SQLForm.NullValue.NotNull
import net.noresttherein.oldsql.schema.SQLReadForm.{FallbackSQLReadForm, FlatMappedSQLReadForm, IgnoringReadForm, NotNullReadFormProxy, ReadFormAdapter}
import net.noresttherein.oldsql.schema.forms.{CaseSQLReadForm, CustomOptSQLReadForm, CustomSQLReadForm, DerivedMappedSQLReadForm, DerivedOptMappedSQLReadForm, LazySQLReadForm, MappedSQLReadForm, OptMappedSQLReadForm, ReorderedReadForm, RepeatedSQLReadForm, SQLReadFormSeq, UnspecifiedForm}
...
trait SQLReadForm[+T] extends UnspecifiedForm {
	...
	def register(call :CallableStatement, position :Int) :Unit =
		try {
			val types = columnTypes.toIndexedSeq
			var i = position + columnCount - 1
			while (i >= position) {
				call.registerOutParameter(position, types(i)); i -= 1
			}
		} catch {
			case _ :UnsupportedOperationException =>
				var i = position + columnCount - 1
				while (i >= position) {
					call.registerOutParameter(position, JDBCType.OTHER); i -= 1
				}
		}
...
//		override def columnTypes :Seq[JDBCType] = SQLReadForm.this.columnTypes
	...
	/** A form which first reads a value `x` from the `ResultSet` with this form, and then uses form `f(x)`
	  * to read, starting from `position + this.columnCount`, the final value. Note that many form methods
	  * have no access to a `ResultSet` and an actual value to use as the function's argument; they are implemented
	  * by applying `f` to `this.`[[net.noresttherein.oldsql.schema.SQLReadForm.nullValue nullValue]].
	  * This requires that:
	  *   1. `this.nullValue` does not throw an exception;
	  *   2. `f(this.nullValue)` does not throw an exception.
	  */
	@throws[NullValueException]("if this.nullValue throws this exception.")
	@throws[NullPointerException]("if f(nullValue) throws this exception.")
	@throws[NoSuchElementException]("if f(nullValue) throws this exception.")
	def flatMap[X](f :T => SQLReadForm[X]) :SQLReadForm[X] = new FlatMappedSQLReadForm(this, f)
	...
	/** An adapter to this form which reads columns out of order, possibly ignoring some additional columns
	  ...
	  * is not a [[net.noresttherein.oldsql.pixies.Rearrangement.isInjection injection]], then `null`
	  * will be returned for columns missing in the underlying `ResultSet`.
	  * If it is not a [[net.noresttherein.oldsql.pixies.Rearrangement.isSurjection surjection]], then
	  * the form's [[net.noresttherein.oldsql.schema.SQLReadForm.shape shape]] will include `JDBCType.NULL` columns
	  * interspersed with columns of this form.
	  ...
	  */ //consider: renaming to project; but what with reorder(IndexedSeq)?
	def reorder(order :Rearrangement) :SQLReadForm[T] = ReorderedReadForm(this, order)
	...
	/** An adapter to this form which adds `shift` to the read column indices before delegating to this form.
	  ...
	  */ //consider: making it reorder(Rearrangement.shift(shift)), as its columnCount would be this.columnCount + shift
...
//	def <<(shift :Int) :SQLReadForm[T] = this >> -shift
	...
	//todo: review all usages to check if they use compatible column shapes.
...
//		nullValue("UNSUPPORTED")(0)(NullValue.Unsupported(message))
	...
	trait ReadFormNullValue[+T] extends SQLReadForm[T] {
		override def nullValue :T = this.nulls.value
	}
		...
		extends SQLReadForm[T] with ReadFormNullValue[T]
	{
		...
		private[oldsql] lazy val cachedString :String = super.toString
		override def toString :String = cachedString
	}

	//todo: migrate from columnCount parameters to shape parameter
	abstract class ShapedSQLReadForm[+T](override val shape :RowShape, protected override val text :Opt[String] = Lack)
	                                    (implicit override val nulls :NullValue[T])
		extends AbstractSQLReadForm[T](shape.columnCount, text)
	{
		override def columnTypes :Seq[JDBCType] = shape.toSeq
	}
...
//		override def hashCode :Int = form.hashCode
	...
	private[schema] trait DerivedReadForm[+T] extends ReadFormAdapter[T] {
		assert(name.isDefined, "Empty name in a " + this.className + " instance.")
...
//		override def notNull :SQLReadForm[T] = super[ReadFormAdapter].notNull
		...
		override def equals(that :Any) :Boolean = that match {
			...
			case other :DerivedReadForm[_] if other canEqual this =>
				name == other.name && form == other.form && this.nulls == other.nulls
			case _ => false
		}
	...
	private[schema] class FlatMappedSQLReadForm[X, +Y](init :SQLReadForm[X], f :X => SQLReadForm[Y])
		extends SQLReadForm[Y]
	{
		private val nullForm = f(init.nullValue)
		private[this] val initSize = init.columnCount
		override def columnCount = init.columnCount + nullForm.columnCount
		override def columnTypes :Seq[JDBCType] = init.columnTypes ++: nullForm.columnTypes

		override def opt(res :ResultSet, position :Int) :Opt[Y] =
			init.opt(res, position) match {
				case Got(x) => f(x).opt(res, position + initSize)
				case _ => Lack
			}
		override def apply(res :ResultSet, position :Int) :Y =
			f(init(res, position))(res, position + initSize)

		override def nulls = nullForm.nulls
		override def nullValue :Y = nullForm.nullValue

		override def equals(that :Any) :Boolean = that match {
			case self :AnyRef if this eq self => true
			case other :FlatMappedSQLReadForm[_, _] if other canEqual this =>
				source == other.source && function == other.function
			case _ => false
		}
		private def source = init
		private def function = f
		override def hashCode :Int = init.hashCode * 31 + f.hashCode
	}
		...
		extends AbstractSQLReadForm[T](columns, text) with IgnoringReadForm[T]
	{
		...
		override def equals(that :Any) :Boolean = that match {
			...
			case const :ConstSQLReadForm[_] if const canEqual this =>
				value == const.result && columnCount == const.columnCount && this.nulls == const.nulls
			case _ => false
		}
		override def hashCode :Int = value.hashCode

		private[oldsql] override lazy val cachedString = if (text.isDefined) text.get + ">" else "=" + value + ">"
	}
		...
		extends AbstractSQLReadForm[T](columns, text)(NullValue.eval(value)) with IgnoringReadForm[T]
	{
		...
		override def equals(that :Any) :Boolean = that match {
			...
			case other :EvalSQLReadForm[_] if other canEqual this =>
				result == other.result && columnCount == other.columnCount && this.nulls == other.nulls
			case _ => false
		}
		override def hashCode :Int = (result.hashCode * 31 + columnCount.hashCode) * 31 + this.nulls.hashCode

		private[oldsql] override lazy val cachedString =
	...
	private[schema] class EvalNullValueSQLReadForm[+T :NullValue](columns :Int = 0,
		...
		extends AbstractSQLReadForm[T](columns, text) with IgnoringReadForm[T]
	{
		override def apply(res :ResultSet, position :Int) :T = this.nulls.value
		override def opt(res :ResultSet, position :Int) :Opt[T] = Opt(this.nulls.value)

		override def notNull :SQLReadForm[T] =
			if (this.nulls == NotNull) this else super.notNull

		override def equals(that :Any) :Boolean = that match {
			case self :AnyRef if this eq self => true
			case other :EvalNullValueSQLReadForm[_] if other canEqual this =>
				columnCount == other.columnCount && this.nulls == other.nulls
			case _ => false
		}
		override def hashCode :Int = columnCount.hashCode * 31 + this.nulls.hashCode

		private[oldsql] override lazy val cachedString = if (text.isDefined) text.get + ">" else "=" + this.nulls + ">"
	}
		...
		extends AbstractSQLReadForm[T](columns, text) with IgnoringReadForm[T]
	{
		...
		private[oldsql] override lazy val cachedString :String =
			if (text.isDefined) text.get + ">"
			else if (columns == 1) this.nulls.toString + ">"
			else this.nulls.toString + columnCount + ">"
	}
