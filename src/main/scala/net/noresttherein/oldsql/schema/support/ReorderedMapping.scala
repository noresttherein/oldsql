...
//	private[this] val order = RearrangedIndexing.permutation(permutation)
...
object ReorderedMapping {
	...
	private[oldsql] def permutation[O](columns :IndexedSeq[TypedColumn[_, O]])
	                                  (precedes :(TypedColumn[_, O], TypedColumn[_, O]) => Boolean) :IndexedSeq[Int] =
	{
		val sorted = topologicalSort(columns)(precedes) to Unique
		columns.map(sorted.sureIndexOf)
	}

	/** Creates a permutation of indices `0..allColumns.size-1` such that applying it to `allColumns` yields
	  * a column sequence `columns` in which `selectedColumns` form a (non consecutive) subsequence.
	  * More precisely,
	  * {{{
	  *     permutation.map(allColumns(_)).filter(selectedColumns.contains) == selectedColumns)
	  * }}}
	  */
	private[oldsql] def subseqPermutation[O](allColumns :Unique[TypedColumn[_, O]],
	                                         selectedColumns :Unique[TypedColumn[_, O]]) :IndexedSeq[Int] =
	{
		val indices = selectedColumns.toIndexedSeq.map(allColumns.indexOf).sorted
		IndexedSeq.tabulate(allColumns.size) { n =>
			selectedColumns.indexOf(allColumns(n)) match {
				case -1 => n
				case i => indices(i)
			}
		}
...
private[oldsql] class RearrangedMapping[+M <: TypedMapping[S, O], S, O](backer :M, order :RearrangedIndexing)
	extends LazyMapping[S, O]
{
	...
	private def reorder(columns :Unique[TypedColumn[_, O]], backing :Unique[TypedColumn[_, O]]) :RearrangedIndexing =
		new RearrangedIndexing {
			...
			override def get(index :Int)       = Got(columns.indexOf(backing(index))).filter(_ >= 0)
			...
			override def find(index :Int)      = Got(backing.indexOf(columns(index))).filter(_ >= 0)
