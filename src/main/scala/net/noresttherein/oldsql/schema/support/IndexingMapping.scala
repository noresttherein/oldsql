...
/** A stateful proxy to another mapping which maintains a collection of indices which map the values
  ...
  */
class IndexingMapping[S, O](protected override val backer :TypedMapping[S, O], indices :Iterable[TableIndex[_, S]])
	extends DirectProxy[S, O]
{
	...
	override def assemble(pieces :Pieces) :Opt[S] = optionally(pieces) //will not be called in normal flow, but possible with proxies.
...
/** A stateful proxy to another mapping which maintains a cache of all previously read values
  ...
  * Additionally, like [[net.noresttherein.oldsql.schema.support.IndexingMapping IndexedMapping]], it maintains
  ...
  * specifically for mapping a single `ResultSet` or a single ''haul''.
  */
class IndexedEntityMapping[K, S, O] private (protected override val backer :TypedMapping[S, O],
...
object IndexingMapping {

	def apply[S, O](cache :TableCache[S]) :IndexingMapping[S, O] = new IndexingMapping[S, O](cache)
