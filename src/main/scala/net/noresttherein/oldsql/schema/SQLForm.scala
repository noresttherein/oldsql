...
import net.noresttherein.oldsql.schema.forms.{CustomSQLForm, DerivedMappedSQLForm, DerivedOptMappedSQLForm, LazySQLForm, MappedSQLForm, OptMappedSQLForm, ReorderedForm, RepeatedSQLForm, SQLForms, SQLFormSeq}
...
trait SQLForm[T] extends SQLReadForm[T] with SQLWriteForm[T] { outer =>
	...
	/** An adapter to this form which reads columns out of order, possibly ignoring some additional columns
	  ...
	  * will be returned for columns missing in the underlying `ResultSet`, and this form's attempts to set
	  * these parameters on a [[java.sql.PreparedStatement PreparedStatement]] will be silently ignored.
	  * On the other hand, if it is not an [[net.noresttherein.oldsql.pixies.Rearrangement.isInjection injection]],
	  * then extra intermittent columns in a `ResultSet` will be ignored, while the corresponding statement parameters
	  * will be set to `NULL`
	  ...
	  */
	override def reorder(order :Rearrangement) :SQLForm[T] =
...
//			implicit final val EmptyTreeSet = NullValue(TreeSet.empty)
	...
	{
		private[oldsql] lazy val cachedString = if (text.isDefined) "<" + text.get + ">" else super.toString
		...
		extends SQLForm[T] with ReadFormNullValue[T]
	{
		...
		private[oldsql] lazy val cachedString :String = if (text.isDefined) "<" + text.get + ">" else super.toString
		override def toString :String = cachedString
	}
		...
		extends SQLForm[T] with ReadFormAdapter[T] with WriteFormAdapter[T] with ReadFormNullValue[T]
	{
		...
		private[oldsql] lazy val cachedString :String =
	...
	private[schema] trait EmptyForm[T] extends SQLForm[T] with IgnoringReadForm[T] with EmptyWriteForm[T] {
		...
		override def equals(that :Any) :Boolean = that match {
			case other :EmptyForm[_] =>
				(other eq this) || other.canEqual(this) && (this.nulls == other.nulls)
			case _ => false
		}
		override def hashCode :Int = this.nulls.hashCode
	...
	{
		override def equals(that :Any) :Boolean = that match {
			case other :EmptySQLForm[_] => (other eq this) || (other canEqual this) && this.nulls == other.nulls
			case _ => false
		}
		override def hashCode :Int = this.nulls.hashCode

		private[oldsql] override lazy val cachedString = text match {
	...
	{
		//consider: moving this check to columnTypes definition, partially because it is faster in CombinedColumnForms
