...
//			acc
		...
		/** Traverses the collection, applying a folding function `op`, checking if the condition `pred` holds
		  ...
		  * {{{
		  *     foldLeftUntil((start, false)){ (a, e) => (op(a._1, e), !pred(a._1, e)); }(_._2)
		  * }}}
		...
		private def forall(f :T => Boolean) :Boolean = items match {
			case it :Iterable[T] => it forall f
			case _ => items.iterator forall f
		}
	}
		...
		extends AnyVal
	{
		...
		/** Equivalent to `this.zip(that).forall { case (a, b) => p(a, b) }`, but does not build an intermediate
		  ...
		  * `this.corresponds(that)(p)` is that, if one collection is larger than the other, excess elements
		  * are ignored.
		  */
