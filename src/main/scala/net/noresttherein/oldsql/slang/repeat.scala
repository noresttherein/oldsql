...
/** A group of higher order functions and syntax extensions repeatedly or recursively applying other functions.
  ...
  * between `repeat` and [[net.noresttherein.oldsql.slang.repeat.RepeatUntil.until until]]. You can also pass as the condition
  ...
  * }}}
  *
  * Importing [[net.noresttherein.oldsql.slang.repeat.repeatMethod repeatMethod]] extends values of any type with
  * a similar [[net.noresttherein.oldsql.slang.repeat.repeatMethod.repeat repeat]] method, accepting as the body
...
object repeat {
	/** Emulates the `repeat { ... } until ( ... )` loop, repeating the execution of the passed block
	  * until the condition passed to the [[net.noresttherein.oldsql.slang.repeat.RepeatUntil.until until]] method
	...
	/** Extends the preceding block with [[net.noresttherein.oldsql.slang.repeat.RepeatUntil.until until]] methods
...
//	}
	...
	/** Extends any value with method [[net.noresttherein.oldsql.slang.repeat.repeatMethod.repeat repeat]], creating
