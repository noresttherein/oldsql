...
import net.noresttherein.oldsql.morsels.witness.Maybe.NoImplicit
...
/** A type for which an implicit value is always present, however, if an implicit value for `T` can be found,
  * it is exposed as `Got[T]` through this instances [[net.noresttherein.oldsql.morsels.witness.Maybe.opt opt]] method.
  */
class Maybe[+T] private[witness] (private val content :AnyRef) extends AnyVal {
	@inline def opt :Opt[T] = if (content == NoImplicit) Lack else Got(content.asInstanceOf[T])
...
object Maybe extends MaybeNoImplicit {
	/** A wrapper type for a type constructor `F` allowing to use a `Maybe` for a generic type as a type class
	  ...
	  */
	type TypeClass[F[_]] = {
		/** A type constructor for [[net.noresttherein.oldsql.morsels.witness.Maybe Maybe]]`[F[T]]`,
		  * where `F` is a type class/type constructor specified as the preceding argument to `TypeCass`.
		  */
		type Maybe[X] = witness.Maybe[F[X]]
	}

	/** A shorter alternative for [[net.noresttherein.oldsql.morsels.witness.Maybe.TypeClass TypeClass]]`[F]` composing
	  * [[net.noresttherein.oldsql.morsels.witness.Maybe! Maybe]] with a specified a type constructor
