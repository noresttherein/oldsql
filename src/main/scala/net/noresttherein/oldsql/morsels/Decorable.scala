...
/** A mixin trait for any interface `Self` we might want to decorate, aware what its decorator is.
  ...
  */
private[oldsql] trait Decorable[Self] { this :Self =>
	...
	/** [[net.noresttherein.oldsql.morsels.Decorable.redecorate Redecorates]] the underlying instance
	  ...
	  * the [[net.noresttherein.oldsql.morsels.Decorable.Decorator.decorated decorated]] instance, but compose it
...
private[oldsql] object Decorable {
	...
	/** Full implementation of `Decorable`, requiring only mixing in an implementation of $Self.
	  * @param decorators  a constructor function for [[net.noresttherein.oldsql.morsels.Decorable.self self]]:
	  *                    it accepts the bottom implementation (`this`) as an argument, and wraps it in a sequence
	  *                    of zero or more decorators.
