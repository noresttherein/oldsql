...
class OldSQLException protected (val msg :String, init :Throwable,
                                 enableSuppression :Boolean, writableStackTrace :Boolean)
	extends Exception(msg, init, enableSuppression, writableStackTrace)
{
	...
	def causeQueue :Seq[Throwable] = {
		...
		@tailrec def push(e :Throwable, result :List[Throwable]) :Seq[Throwable] = e.getCause match {
			...
			case cause if dejaVu(cause) =>
				val cycle = new OldSQLException(s"Cycle detected in the exception stack: ${e.getCause} already present.")
				cycle::e::result
	...
	val message :String = if (msg == null) "" else msg
}
