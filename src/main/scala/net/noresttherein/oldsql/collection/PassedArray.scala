...
import net.noresttherein.oldsql.collection.PassedArray.{InitSize, OwnerField}
import net.noresttherein.oldsql.slang.classMethods
...
/** An array-backed immutable sequence with amortized `O(1)` random access, `slice` and first `append/prepend`.
  * It combines the idea behind the classic growing and shrinking mutable buffer, a shared sequence view,
  ...
  * [[net.noresttherein.oldsql.collection.PassedArray$ PassedArray]]` `[[net.noresttherein.oldsql.collection.PassedArray$.:+ :+]]` e1 `[[net.noresttherein.oldsql.collection.PassedArray!.:+ :+]]` e2`.
  *
	...
	extends AbstractSeq[E] with PassedArray[E]
{
	...
	override def elementType :Class[_] = array.getClass.getComponentType
	private[collection] def elems[U >: E] :Array[U] = array.asInstanceOf[Array[U]]
	private[collection] def startIndex :Int = offset
	...
	override def drop(n :Int) :PassedArray[E] = slice(n, len)

	override def takeRight(n :Int) :PassedArray[E] =
		if (n <= 0) PassedArray.empty
		else slice(len - n, len)
	...
	//todo: replace synchronized with VarHandle
	//extracted for specialization
	private[this] def newInstance(array :Array[E], offset :Int, len :Int) =
		new PassedArrayPlus(array, offset, len, true)
	...
	override def copyToArray[B >: E](xs :Array[B], start :Int, len :Int) :Int =
		if (start >= xs.length | len <= 0)
			...
			val copied = Math.min(this.len, Math.min(len, Math.max(xs.length - start, 0)))
			Array.copy(array, offset, xs, start, copied)
			copied
		}
}
...
object PassedArray extends StrictOptimizedSeqFactory[PassedArray] {
	...
	@inline final def :+[A](elem :A) :PassedArray[A] = single(elem)
