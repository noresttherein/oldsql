...
import net.noresttherein.oldsql.collection.Opt.{Got, Lack, NoContent, WithFilter}
	...
	extends AnyVal with IterableOnce[A] with Product with Equals with Serializable
{
	...
	/** The wrapped value. It is the same as [[net.noresttherein.oldsql.collection.Opt.get get]], but this method
	  * is only possible to call on instances for which it is statically known that the value exists based on
	  * [[net.noresttherein.oldsql.collection.Opt.isEmpty! isEmpty]] member type.
	...
	/** Gets the element in the `Opt` or throws the exception given as the type parameter with the given message.
	  ...
	  * @see [[net.noresttherein.oldsql.collection.Opt.orNoSuch orNoSuch]]
	  * @see [[net.noresttherein.oldsql.collection.Opt.orIllegal orIllegal]] */
	...
	/** Gets the element in this `Opt` or throws a `NoSuchElementException` with the given message.
	  * @see [[net.noresttherein.oldsql.collection.Opt.orThrow orThrow]] */
	...
	/** Returns the result of applying `f` to the value of this `Opt` if it is non empty,
	  ...
	  * [[net.noresttherein.oldsql.collection.Opt.mapOrElse mapOrElse]] instead.
	...
	/** Equivalent to `this.`[[net.noresttherein.oldsql.collection.Opt.filter filter]]`(p)` - a variant for use
...
object Opt {
	...
	/** A refinement of [[net.noresttherein.oldsql.collection.Opt Opt]] marking it through a member flag type
	  * as non-empty. [[net.noresttherein.oldsql.collection.Opt$ Opt]] factory object creates instances
	...
	object Got {
		/** Creates a non-empty [[net.noresttherein.oldsql.collection.Opt Opt]] wrapping the given value. */
		@inline def apply[T](x :T) :Got[T] = new Opt(x.asInstanceOf[AnyRef]).asInstanceOf[Got[T]]

		/** Matches non-empty [[net.noresttherein.oldsql.collection.Opt Opt]] instances. */
		@inline def unapply[T](opt :Opt[T]) :Opt[T] = if (opt == null : @nowarn) Lack else opt

		/** Implicitly lifts any value `T` to [[net.noresttherein.oldsql.collection.Opt Opt]]`[T]`. */
		@inline def implicitWrapping[T](x :T) :Opt[T] = new Opt(x.asInstanceOf[AnyRef])
	}

	/** A refinement of [[net.noresttherein.oldsql.collection.Opt Opt]] marking it through a member flag type
	  * as empty. [[net.noresttherein.oldsql.collection.Opt.Lack Opt.Lack]] is an instance of this type.
	  */
	type Lack = Opt[Nothing] { type isEmpty = true }

	/** An equivalent of `None` - a special, empty instance of [[net.noresttherein.oldsql.collection.Opt Opt]]
	  * which conforms to any `Opt[T]` type. It is represented by wrapping a special, private singleton object
	  * and all `isEmpty` tests check for referential equality of the wrapped value with this object.
	  * While normally inaccessible from scala code, it is however still possible to obtain its reference,
	  * which makes this class unsuitable for security conscious applications.
