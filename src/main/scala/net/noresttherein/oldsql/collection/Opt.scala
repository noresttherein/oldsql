...
import net.noresttherein.oldsql.collection.Opt.{NoContent, Lack}
...
final class Opt[+T] private[Opt] (private val ref :AnyRef) extends AnyVal with Serializable {
	...
	/** Gets the element in the `Opt` or throws the exception given as the type parameter with the given message.
	  ...
	  * @see [[net.noresttherein.oldsql.collection.Opt.orFail orFail]]
	  */
...
object Opt {
	...
	/** A special, empty instance of [[net.noresttherein.oldsql.collection.Opt Opt]] which conforms to any `Opt[T]` type.
	...
	private object NoContent extends (Any => AnyRef) with Serializable {
